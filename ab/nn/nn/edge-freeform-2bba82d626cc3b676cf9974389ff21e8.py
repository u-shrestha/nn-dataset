...
def inverted_residual_block(in_channels, out_channels, stride, expand_ratio=1):
    ...
    layers.append(nn.LazyConv2d(expanded_channels, kernel_size=3, stride=stride, padding=1, groups=expanded_channels, bias=False))
    layers.append(nn.LazyBatchNorm2d())
    layers.append(nn.ReLU6(inplace=True))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Parallel_Triple_Fractal'
