...
def dsconv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=bias), nn.ReLU6(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expansion_factor=6, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(dsconv3x3_block(in_channels, hidden_dim, stride=stride, padding=1, bias=False), dsconv3x3_block(hidden_dim, out_channels, stride=1, padding=1, bias=False, dropout_prob=dropout_prob))
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(InvertedResidualBlock(in_ch_ij, out_channels, stride=1, expansion_factor=6, dropout_prob=dropout_prob))
            blocks.append(level)
