...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            if i == 0:
                continue
            unit = FractalUnit(curr_ch, out_ch, 2, 0.15, dropout_prob)
            self.features.add_module(f'unit{i + 1}', unit)
            curr_ch = out_ch
        self.backbone_b = TorchVision('resnet18', weights='DEFAULT', in_channels=3).to(device)
    ...
    def infer_dimensions_dynamically(self, in_shape, num_classes):
        ...
        with torch.no_grad():
            ...
            outputFeat = torch.cat([output_feat_a, output_feat_b], dim=1)
            dim_fused = outputFeat.shape[1]
