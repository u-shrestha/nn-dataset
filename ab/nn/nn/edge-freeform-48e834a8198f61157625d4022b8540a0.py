...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
    ...
    layers = [nn.Conv2d(in_channels, expanded_dim, kernel_size=1, bias=False), nn.BatchNorm2d(expanded_dim), nn.SiLU(inplace=True), nn.Conv2d(expanded_dim, expanded_dim, kernel_size=3, stride=stride, groups=expanded_dim, bias=False), nn.BatchNorm2d(expanded_dim), nn.SiLU(inplace=True), nn.Conv2d(expanded_dim, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels)]
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, (out_ch, stride) in enumerate(zip(channels, strides)):
            self.features.add_module(f'unit{i + 1}', inverted_residual_block(curr_ch, out_ch, stride, expand_ratio=4, dropout_prob=dropout_prob))
            curr_ch = out_ch
