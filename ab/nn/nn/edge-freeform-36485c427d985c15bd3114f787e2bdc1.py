...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, use_se=False, se_ratio=0.25):
        super().__init__()
        self.stride = stride
        hidden_dim = int(in_channels * expansion_factor)
        self.conv1 = nn.LazyConv2d(hidden_dim, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn1 = nn.LazyBatchNorm2d()
        self.conv2 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False)
        self.bn2 = nn.LazyBatchNorm2d()
        self.se = SEBlock(hidden_dim, round(hidden_dim * se_ratio)) if use_se else nn.Identity()
...
class SEBlock(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        ...
        self.fc1 = nn.LazyConv2d(reduced_dim, kernel_size=1, stride=1, padding=0)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.LazyConv2d(in_channels, kernel_size=1, stride=1, padding=0)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidualBlock(3, 64, expansion_factor=6, stride=2, use_se=True), InvertedResidualBlock(64, 64, expansion_factor=6, stride=1, use_se=True), InvertedResidualBlock(64, 128, expansion_factor=6, stride=2, use_se=True), InvertedResidualBlock(128, 128, expansion_factor=6, stride=1, use_se=True)).to(device)
