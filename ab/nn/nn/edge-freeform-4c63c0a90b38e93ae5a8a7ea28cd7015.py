...
def drop_depthwise_separable_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.BatchNorm2d(in_channels), nn.SiLU(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=bias), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())

class DepthwiseSeparableUnit(nn.Module):

    def __init__(self, in_channels, out_channels, num_blocks, dropout_prob):
        super().__init__()
        blocks = []
        for _ in range(num_blocks):
            blocks.append(drop_depthwise_separable_block(in_channels, out_channels, dropout_prob=dropout_prob))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', DepthwiseSeparableUnit(curr_ch, out_ch, num_blocks=2, dropout_prob=dropout_prob))
            curr_ch = out_ch
