...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_small', weights='DEFAULT', in_channels=3).to(device)
        self.backbone_b = TorchVision('squeezenet1_1', weights='DEFAULT', in_channels=3).to(device)
        self.features = nn.Sequential(FractalUnit(3, 32, 2, 0.15, dropout_prob), FractalUnit(32, 64, 2, 0.15, dropout_prob), FractalUnit(64, 128, 2, 0.15, dropout_prob))
