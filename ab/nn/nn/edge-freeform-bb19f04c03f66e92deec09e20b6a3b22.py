...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Hybrid_DS_IR_SE_Fractal'
        ...
        for i, out_ch in enumerate(channels):
            if i == 0:
                self.features.add_module(f'ds_unit{i + 1}', DepthwiseSeparableBlock(curr_ch, out_ch, stride=2, expansion_factor=6))
            elif i == 1:
                self.features.add_module(f'inverted_residual_unit{i + 1}', InvertedResidualBlock(curr_ch, out_ch, stride=2, expansion_factor=6))
            else:
                self.features.add_module(f'se_unit{i + 1}', InvertedResidualBlock(curr_ch, out_ch, stride=2, expansion_factor=6))
            curr_ch = out_ch
