...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1):
        ...
        self.dw_bn = nn.BatchNorm2d(self.expanded_channels)
        self.pw_conv = nn.Conv2d(self.expanded_channels, out_channels, kernel_size=1, bias=False)
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(InvertedResidual(in_ch_ij, out_channels, stride=1))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=3, truncate=1).to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        channels = [24, 48, 96]
        for i, out_ch in enumerate(channels):
            if i == 0:
                self.features.add_module(f'dsconv{i + 1}', DepthwiseSeparableBlock(curr_ch, out_ch, stride=2))
            else:
                self.features.add_module(f'irb{i + 1}', InvertedResidual(curr_ch, out_ch, stride=2))
            self.features.add_module(f'se{i + 1}', SqueezeExcitation(out_ch, out_ch // 16))
            curr_ch = out_ch
