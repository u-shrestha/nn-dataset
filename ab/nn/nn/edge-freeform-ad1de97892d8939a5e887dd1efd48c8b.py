...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, (out_ch, stride, exp_ratio) in enumerate(zip(channels, strides, expand_ratios)):
            self.features.add_module(f'inverted_residual_{i + 1}', inverted_residual_block(curr_ch, out_ch, stride, exp_ratio, dropout_prob))
            self.features.add_module(f'seblock_{i + 1}', SEBlock(out_ch))
            self.features.add_module(f'depthwise_separable_conv_{i + 1}', DepthwiseSeparableConv(out_ch, out_ch, dropout_prob=dropout_prob))
            curr_ch = out_ch
