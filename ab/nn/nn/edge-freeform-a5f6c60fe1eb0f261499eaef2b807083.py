...
class SEInvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(DepthwiseSeparableConv(in_channels, hidden_dim, stride=1, padding=1, dropout_prob=dropout_prob))
        layers.extend([DepthwiseSeparableConv(hidden_dim, out_channels, stride=stride, padding=1, dropout_prob=dropout_prob), SqueezeExcitation(out_channels)])
