...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        super().__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(reduced_dim, kernel_size=1, bias=True), nn.ReLU(inplace=True), nn.LazyConv2d(in_channels, kernel_size=1, bias=True), nn.Hardsigmoid(inplace=True))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(inverted_residual_block(24, 32, stride=2), DepthwiseSeparableConv(32, 64), SqueezeExcitation(64, 16), nn.MaxPool2d(kernel_size=2, stride=2))
