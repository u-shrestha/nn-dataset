    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            if i == 0:
                ...
                self.features.add_module(f'gelu{i + 1}', nn.GELU())
            else:
                self.features.add_module(f'unit{i + 1}', InvertedResidualBlock(curr_ch, out_ch, stride=2))
            curr_ch = out_ch
