...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=4, dropout_prob=0.0):
    ...
    if stride == 1 and in_channels == out_channels:
        ...
        layers = [nn.Conv2d(in_channels, hidden_dim, kernel_size=1, stride=stride, padding=0, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=1, padding=1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(out_channels), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()]
    return nn.Sequential(*layers)

def se_block(in_channels, reduction_ratio=16):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(in_channels, in_channels // reduction_ratio), nn.ReLU(inplace=True), nn.Linear(in_channels // reduction_ratio, in_channels), nn.Sigmoid(), nn.Unflatten(1, (in_channels, 1, 1)))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, (out_ch, stride) in enumerate(zip(channels, strides)):
            ...
            self.features.add_module(f'se_{i + 1}', se_block(out_ch))
            curr_ch = out_ch
