...
def dsconv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False):
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('shufflenet_v2_x1_0', in_channels=3, truncate=6).to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        channels = [24, 48, 96]
        for i, out_ch in enumerate(channels):
            if i == 0:
                self.features.add_module(f'dsconv{i + 1}', dsconv3x3_block(curr_ch, out_ch, stride=2))
            else:
                self.features.add_module(f'resblock{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=6, stride=2, dropout_prob=dropout_prob))
