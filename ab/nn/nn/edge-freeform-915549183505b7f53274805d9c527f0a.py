...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidualBlock(576, 6, 96, 1), SqueezeExcitation(96, 24), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), DepthwiseSeparableConv(96, 128, 1), InvertedResidualBlock(128, 6, 128, 2), SqueezeExcitation(128, 32), DepthwiseSeparableConv(128, 128, 1), nn.AdaptiveAvgPool2d((1, 1)), nn.Flatten(), nn.Dropout(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.Linear(128, out_shape[0]))
