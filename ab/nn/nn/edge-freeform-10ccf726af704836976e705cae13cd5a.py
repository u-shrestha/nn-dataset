...
class DepthwiseSeparableConv(nn.Module):
    ...
    def forward(self, x):
        return self.dropout(self.relu(self.pointwise(self.depthwise(x))))

class InvertedResidualBlock(nn.Module):

    def __init__(self, in_channels, out_channels, expansion_factor=2, stride=1, dropout_prob=0.0):
        super().__init__()
        expanded_channels = in_channels * expansion_factor
        self.conv1 = nn.LazyConv2d(expanded_channels, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn1 = nn.LazyBatchNorm2d()
        self.conv2 = DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride, padding=1, bias=False, dropout_prob=dropout_prob)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'HybridDepthwiseInvertedResidual'
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inv_res_{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=2, stride=2, dropout_prob=dropout_prob))
            curr_ch = out_ch
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        x_f = self.features(x)
        if x_f.dim() == 2:
            x_f = x_f.unsqueeze(-1).unsqueeze(-1)
        if x_f.shape[-1] < 14:
            x_f = torch.nn.functional.interpolate(x_f, size=(14, 14), mode='nearest')
        f_f = adaptive_pool_flatten(x_f)
        fused = torch.cat([f_a, f_f], dim=1)
        if is_probing:
            return fused
