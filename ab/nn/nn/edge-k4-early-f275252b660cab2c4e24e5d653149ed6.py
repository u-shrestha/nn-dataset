...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        inverted_residual_setting: Optional[List[List[int]]] = [[1, 8, 1, 1], [6, 16, 2, 2], [6, 24, 3, 2], [6, 48, 4, 2], [6, 96, 3, 1], [6, 192, 3, 2], [6, 384, 1, 1]]
        ...
        self.last_channel = _make_divisible(last_channel * max(width_mult, 1.0), 8)
        features: List[nn.Module] = [Conv2dNormActivation(in_shape[1], input_channel, stride=2, norm_layer=nn.BatchNorm2d, activation_layer=nn.ReLU6)]
        for t, c, n, s in inverted_residual_setting:
            output_channel = _make_divisible(c * width_mult, 8)
            for i in range(n):
                stride = s if i == 0 else 1
                features.append(InvertedResidual(input_channel, output_channel, stride, expand_ratio=t, norm_layer=nn.BatchNorm2d))
                input_channel = output_channel
        features.append(Conv2dNormActivation(input_channel, self.last_channel, kernel_size=1, norm_layer=nn.BatchNorm2d, activation_layer=nn.ReLU6))
