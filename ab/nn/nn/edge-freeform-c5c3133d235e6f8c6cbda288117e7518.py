...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res{i + 1}', InvertedResidualBlock(curr_ch, expansion_factor=6, out_channels=out_ch, stride=2 if i > 0 else 1))
            curr_ch = out_ch
            if i < len(channels) - 1:
                self.features.add_module(f'dsconv{i + 1}', DSConv(out_ch, out_ch))
        self.to(self.device)
