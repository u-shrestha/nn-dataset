...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, inp: int, oup: int, stride: int, expand_ratio: int, norm_layer: Optional[Callable[..., nn.Module]]=None) -> None:
        ...
        layers.extend([Conv2dNormActivation(hidden_dim, hidden_dim, stride=stride, groups=hidden_dim, norm_layer=norm_layer, activation_layer=nn.ReLU6), SqueezeExcitation(hidden_dim, hidden_dim // 4), nn.Conv2d(hidden_dim, oup, 1, 1, 0, bias=False), norm_layer(oup)])
...
class Net(nn.Module):
    ...
    def infer_dimensions_dynamically(self, dummy: Tensor):
        with torch.no_grad():
            ...
            x = self.avgpool(x)
            self.classifier[1] = nn.Linear(x.view(-1).shape[0], self.classifier[1].out_features)
