...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.ReLU6(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(nn.Sequential(drop_conv3x3_block(in_ch_ij, out_channels, dropout_prob=dropout_prob), SEBlock(out_channels)))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Optimized_Multi_Scale_Fusion'
