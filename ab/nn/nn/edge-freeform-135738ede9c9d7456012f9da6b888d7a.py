...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expansion=6):
        super().__init__()
        self.stride = stride
        self.hidden_dim = round(in_channels * expansion)
        self.residual = self.stride == 1 and in_channels == out_channels
        self.conv = nn.Sequential(nn.Conv2d(in_channels, self.hidden_dim, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(self.hidden_dim), nn.ReLU6(inplace=True), DepthwiseSeparableConv(self.hidden_dim, self.hidden_dim, stride=self.stride), nn.Conv2d(self.hidden_dim, out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidualBlock(3, 32, stride=1, expansion=4), DepthwiseSeparableConv(32, 64, stride=2), nn.MaxPool2d(kernel_size=2, stride=2))
