...
def drop_conv3x3_dsc(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.BatchNorm2d(in_channels), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=bias), nn.BatchNorm2d(out_channels), nn.GELU())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(drop_conv3x3_dsc(in_ch_ij, out_channels, dropout_prob=dropout_prob))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        if x_bf.shape[-1] < 14:
            x_bf = nn.functional.interpolate(x_bf, size=(14, 14), mode='nearest')
