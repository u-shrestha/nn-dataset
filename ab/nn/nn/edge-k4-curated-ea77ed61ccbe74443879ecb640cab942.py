...
class DSConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expansion=1, dropout_prob=0.0):
        super().__init__()
        self.conv = nn.Sequential(nn.Conv2d(in_channels, in_channels * expansion, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(in_channels * expansion), nn.SiLU(inplace=True), nn.Conv2d(in_channels * expansion, in_channels * expansion, kernel_size=3, stride=stride, padding=1, groups=in_channels * expansion, bias=False), nn.BatchNorm2d(in_channels * expansion), nn.SiLU(inplace=True), nn.Conv2d(in_channels * expansion, out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expansion, dropout_prob=0.0):
        super().__init__()
        self.stride = stride
        self.use_res_connect = self.stride == 1 and in_channels == out_channels
        self.expansion = expansion
        self.conv = nn.Sequential(DSConv(in_channels, in_channels * expansion, stride=1, expansion=expansion, dropout_prob=dropout_prob), DSConv(in_channels * expansion, out_channels, stride=stride, expansion=1, dropout_prob=dropout_prob))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidualBlock(3, 64, stride=1, expansion=1, dropout_prob=dropout_prob), SEBlock(channel=64, reduction=4))
