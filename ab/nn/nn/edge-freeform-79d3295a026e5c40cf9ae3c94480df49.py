        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(InvertedResidualBlock(in_ch_ij, out_channels, expansion_factor=6, stride=1))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('squeezenet1_1', in_channels=3, truncate=10).to(device)
