...
def inverted_residual_block(in_channels, out_channels, stride=1, expansion_factor=6):
    return nn.Sequential(nn.LazyConv2d(in_channels * expansion_factor, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.Conv2d(in_channels * expansion_factor, in_channels * expansion_factor, kernel_size=3, stride=stride, padding=1, groups=in_channels * expansion_factor, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d())

class FractalBlock(nn.Module):

    def __init__(self, in_channels, out_channels, num_columns, dropout_prob=0.0):
        super().__init__()
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(inverted_residual_block(in_ch_ij, out_channels, stride=1, expansion_factor=2))
            blocks.append(level)
