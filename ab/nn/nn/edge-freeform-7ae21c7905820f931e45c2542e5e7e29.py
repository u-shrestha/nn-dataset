...
def inverted_residual_block(in_channels, out_channels, stride=1, expansion_factor=6, dropout_prob=0.0):
    ...
    if dropout_prob > 0:
        layers.insert(4, nn.Dropout2d(p=dropout_prob))
    return nn.Sequential(*layers)

def se_block(in_channels, reduction_ratio=16):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(in_channels // reduction_ratio, kernel_size=1), nn.SiLU(inplace=True), nn.LazyConv2d(in_channels, kernel_size=1), nn.Sigmoid())
