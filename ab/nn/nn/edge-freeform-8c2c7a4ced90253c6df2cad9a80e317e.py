...
class DepthwiseSeparableBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, dropout_prob=0.0):
        super().__init__()
        self.depthwise = nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False), nn.BatchNorm2d(in_channels), nn.ReLU6(inplace=True))
        self.pointwise = nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.ReLU6(inplace=True))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
        ...
        layers.extend([nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True)])
        layers.append(nn.Conv2d(hidden_dim, out_channels, kernel_size=1, bias=False))
        layers.append(nn.BatchNorm2d(out_channels))
        if dropout_prob > 0 and self.use_res_connect:
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        self.fc = nn.Sequential(nn.Conv2d(in_channels, in_channels // reduction_ratio, kernel_size=1), nn.ReLU(inplace=True), nn.Conv2d(in_channels // reduction_ratio, in_channels, kernel_size=1), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(DepthwiseSeparableBlock(3, 64, stride=2, dropout_prob=dropout_prob), InvertedResidualBlock(64, 64, stride=1, expand_ratio=2, dropout_prob=dropout_prob), SqueezeExcitation(64), InvertedResidualBlock(64, 128, stride=2, expand_ratio=2, dropout_prob=dropout_prob), SqueezeExcitation(128))
