...
def depthwise_separable_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.LazyConv2d(out_channels, kernel_size=1, bias=bias), nn.LazyBatchNorm2d(), nn.ReLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
    ...
    if stride != 1 or in_channels != out_channels:
        layers.append(nn.LazyConv2d(out_channels, kernel_size=1, stride=stride, bias=False))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('squeezenet1_1', in_channels=3, truncate=7).to(device)
        self.features = nn.Sequential(inverted_residual_block(in_channels=3, out_channels=32, stride=1, expand_ratio=2, dropout_prob=dropout_prob), depthwise_separable_block(in_channels=32, out_channels=16, stride=2, padding=1, bias=False, dropout_prob=dropout_prob), squeeze_excitation_block(in_channels=16, reduction_ratio=16))
