...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        super().__init__()
        self.fc = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(reduced_dim, 1, bias=True), nn.ReLU(inplace=True), nn.LazyConv2d(in_channels, 1, bias=True), nn.Sigmoid())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(nn.Sequential(inverted_residual_block(in_ch_ij, out_channels), SqueezeExcitation(out_channels, out_channels // 16)))
            blocks.append(level)
