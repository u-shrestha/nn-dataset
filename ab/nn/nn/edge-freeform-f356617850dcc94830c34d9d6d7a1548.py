...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=1, dropout_prob=0.0):
    hidden_dim = int(round(in_channels * expand_ratio))
    layers = [nn.LazyConv2d(hidden_dim, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True)]
    layers.extend([nn.LazyConv2d(hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d()])
...
def se_block(in_channels, reduction_ratio=16):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyLinear(in_channels // reduction_ratio, bias=False), nn.ReLU(inplace=True), nn.LazyLinear(in_channels, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('shufflenet_v2_x1_0', in_channels=3, truncate=8).to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        channels = [16, 32, 64]
        strides = [1, 2, 2]
        expand_ratios = [1, 4, 6]
        for i, (out_ch, stride, exp_ratio) in enumerate(zip(channels, strides, expand_ratios)):
            self.features.add_module(f'unit{i + 1}', inverted_residual_block(curr_ch, out_ch, stride, exp_ratio, dropout_prob))
            self.features.add_module(f'seblock{i + 1}', se_block(out_ch))
            curr_ch = out_ch
