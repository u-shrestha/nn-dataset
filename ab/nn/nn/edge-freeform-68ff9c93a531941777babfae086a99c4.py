...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        super().__init__()
        self.conv = nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=1, groups=in_channels), nn.BatchNorm2d(in_channels), nn.SiLU(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1), nn.BatchNorm2d(out_channels), nn.SiLU(inplace=True))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1):
        super().__init__()
        self.shortcut = nn.Identity() if stride == 1 and in_channels == out_channels else None
        expanded_channels = in_channels * expansion_factor
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_channels, kernel_size=1), nn.BatchNorm2d(expanded_channels), nn.SiLU(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride), nn.Conv2d(expanded_channels, out_channels, kernel_size=1), nn.BatchNorm2d(out_channels))
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction=4):
        super().__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)), nn.Conv2d(channels, channels // reduction, kernel_size=1), nn.SiLU(inplace=True), nn.Conv2d(channels // reduction, channels, kernel_size=1), nn.Sigmoid())
...
class SEInvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, reduction=4):
        super().__init__()
        self.shortcut = nn.Identity() if stride == 1 and in_channels == out_channels else None
        expanded_channels = in_channels * expansion_factor
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_channels, kernel_size=1), nn.BatchNorm2d(expanded_channels), nn.SiLU(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride), SqueezeExcitation(expanded_channels, reduction), nn.Conv2d(expanded_channels, out_channels, kernel_size=1), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Hybrid_Backbone_SEIR'
        ...
        self.features = nn.Sequential(SEInvertedResidualBlock(3, 32, stride=2), SEInvertedResidualBlock(32, 32, stride=1), SEInvertedResidualBlock(32, 64, stride=2), SEInvertedResidualBlock(64, 64, stride=1), SEInvertedResidualBlock(64, 128, stride=2), SEInvertedResidualBlock(128, 128, stride=1))
