...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, expansion_factor, out_channels, stride, use_se=False):
        super(InvertedResidualBlock, self).__init__()
        expanded_channels = in_channels * expansion_factor
        self.identity_skip = stride == 1 and in_channels == out_channels
        layers = [nn.Conv2d(in_channels, expanded_channels, kernel_size=1, bias=False), nn.BatchNorm2d(expanded_channels), nn.ReLU(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride)]
        if use_se:
            layers.append(SEModule(expanded_channels))
        layers.extend([nn.Conv2d(expanded_channels, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels)])
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('mobilenet_v3_small', in_channels=3, truncate=7).to(device)
        self.features = nn.Sequential(nn.Conv2d(3, 32, kernel_size=3, stride=2, padding=1, bias=False), nn.BatchNorm2d(32), nn.ReLU(inplace=True), InvertedResidualBlock(32, 1, 16, 1), InvertedResidualBlock(16, 6, 24, 2, use_se=True), InvertedResidualBlock(24, 6, 24, 1, use_se=True), InvertedResidualBlock(24, 6, 32, 2), InvertedResidualBlock(32, 6, 32, 1), InvertedResidualBlock(32, 6, 32, 1), InvertedResidualBlock(32, 6, 64, 2), InvertedResidualBlock(64, 6, 64, 1), InvertedResidualBlock(64, 6, 64, 1), InvertedResidualBlock(64, 6, 96, 1), InvertedResidualBlock(96, 6, 96, 1), InvertedResidualBlock(96, 6, 160, 2), InvertedResidualBlock(160, 6, 160, 1), InvertedResidualBlock(160, 6, 160, 1), InvertedResidualBlock(160, 6, 320, 1), nn.Conv2d(320, 1280, kernel_size=1, bias=False), nn.BatchNorm2d(1280), nn.ReLU(inplace=True))
