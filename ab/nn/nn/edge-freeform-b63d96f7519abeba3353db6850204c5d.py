...
def se_block(channel, reduction=16):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(channel, channel // reduction, kernel_size=1, padding=0, bias=False), nn.ReLU(inplace=True), nn.Conv2d(channel // reduction, channel, kernel_size=1, padding=0, bias=False), nn.Sigmoid())
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, self.expanded_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(self.expanded_channels), nn.ReLU6(inplace=True), DepthwiseSeparableConv(self.expanded_channels, self.expanded_channels, stride=stride, padding=1, bias=False, dropout_prob=dropout_prob), nn.Conv2d(self.expanded_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(out_channels))
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(InvertedResidual(in_ch_ij, out_channels, expansion_factor=6, stride=1, dropout_prob=dropout_prob))
            blocks.append(level)
