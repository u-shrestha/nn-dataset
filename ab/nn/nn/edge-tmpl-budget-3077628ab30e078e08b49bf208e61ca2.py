...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce = nn.Conv2d(in_channels, 256, 1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(256)
        self.depthwise1 = nn.Conv2d(256, 256, 3, padding=1, groups=256, bias=False)
        ...
        self.depthwise2 = nn.Conv2d(256, 256, 3, padding=1, groups=256, bias=False)
        self.bn_depthwise2 = nn.BatchNorm2d(256)
        self.pointwise = nn.Conv2d(256, 128, 1, bias=False)
        self.bn_pointwise = nn.BatchNorm2d(128)
