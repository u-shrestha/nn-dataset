...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=1, dropout_prob=0.0):
    ...
    conv_layer = nn.Conv2d if stride == 1 else nn.Conv2d
    conv_kwargs = {'kernel_size': 3, 'stride': stride, 'padding': 1, 'bias': False}
    return nn.Sequential(nn.Conv2d(in_channels, hidden_dim, 1, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), conv_layer(hidden_dim, hidden_dim, groups=hidden_dim, **conv_kwargs), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, out_channels, 1, bias=False), nn.BatchNorm2d(out_channels), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inv_res{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=2, expand_ratio=1, dropout_prob=dropout_prob))
            curr_ch = out_ch
