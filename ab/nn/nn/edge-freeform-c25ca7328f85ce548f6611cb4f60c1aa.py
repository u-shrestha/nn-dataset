...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=2, dropout_prob=0.0):
...
def squeeze_excitation_block(in_channels, reduction_ratio=16):
    ...
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(reduced_dim, 1, bias=False), nn.ReLU(inplace=True), nn.LazyConv2d(in_channels, 1, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('shufflenet_v2_x1_0', in_channels=3, truncate=5).to(device)
        self.features = nn.Sequential(depthwise_separable_block(3, 32, dropout_prob=dropout_prob), inverted_residual_block(32, 64, stride=2, dropout_prob=dropout_prob), squeeze_excitation_block(64), inverted_residual_block(64, 128, stride=2, dropout_prob=dropout_prob), squeeze_excitation_block(128))
