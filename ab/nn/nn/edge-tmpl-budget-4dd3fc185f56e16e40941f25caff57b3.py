...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.dw_conv1 = nn.Conv2d(64, 64, kernel_size=3, padding=1, groups=64, bias=False)
        self.bn_dw1 = nn.BatchNorm2d(64)
        self.se_block = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(64, 8, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(8, 64, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
        self.conv_expand = nn.Conv2d(64, 128, kernel_size=1, bias=False)
