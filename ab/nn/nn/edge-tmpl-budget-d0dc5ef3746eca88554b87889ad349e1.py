...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.dw_conv1 = nn.Conv2d(160, 160, kernel_size=3, padding=1, groups=160, bias=False)
        self.bn_dw1 = nn.BatchNorm2d(160)
        self.se_block = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(160, 20, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(20, 160, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
        self.dw_conv2 = nn.Conv2d(160, 160, kernel_size=3, padding=1, groups=160, bias=False)
