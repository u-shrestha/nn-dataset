...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.se = nn.Sequential(
            ...
            nn.Conv2d(32, 192, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
        self.squeeze = nn.Conv2d(192, 96, kernel_size=1, bias=False)
        self.bn_squeeze = nn.BatchNorm2d(96)
        self.expand = nn.Conv2d(96, 192, kernel_size=1, bias=False)
        self.bn_expand = nn.BatchNorm2d(192)
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.drop = nn.Dropout(p)
        self.fc = nn.LazyLinear(num_classes)

    def forward(self, x):
        x = self.bn_reduce(self.reduce(x))
        x = self.bn_depthwise(self.depthwise(x))
        se_weights = self.se(x)
        x = x * se_weights
        x = self.bn_squeeze(self.squeeze(x))
        x = self.bn_expand(self.expand(x))
...
class Net(nn.Module):
    def __init__(self, in_shape, out_shape, prm, device):
        ...
        self.backbone = _edge_backbone('shufflenet_v2_x1_0', device)
