...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.SiLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.SiLU(inplace=True))

def dw_conv_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
        ...
        self.layers = nn.Sequential(dw_conv_block(in_channels, hidden_dim, stride), dw_conv_block(hidden_dim, out_channels, stride=1))
