...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio):
        ...
        self.expand = nn.Sequential(nn.Conv2d(in_channels, hidden_dim, 1, 1, 0, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True))
        self.dw_conv = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True))
        self.project = nn.Sequential(nn.Conv2d(hidden_dim, out_channels, 1, 1, 0, bias=False), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(DepthwiseSeparableConv(3, 64, 1), InvertedResidualBlock(64, 64, 1, 1), SEBlock(64), DepthwiseSeparableConv(64, 128, 2), InvertedResidualBlock(128, 128, 1, 1), SEBlock(128), DepthwiseSeparableConv(128, 256, 2), InvertedResidualBlock(256, 256, 1, 1), SEBlock(256))
