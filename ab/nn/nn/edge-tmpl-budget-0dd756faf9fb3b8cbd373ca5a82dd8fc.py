...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.conv_reduce = nn.Conv2d(in_channels, 192, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(192)
        self.relu = nn.ReLU(inplace=True)
        self.dw_conv1 = nn.Conv2d(192, 192, kernel_size=3, padding=1, groups=192, bias=False)
        self.se_block = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(192, 24, kernel_size=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(24, 192, kernel_size=1),
            nn.Sigmoid()
        )
        self.dw_conv2 = nn.Conv2d(192, 192, kernel_size=3, padding=1, groups=192, bias=False)
        self.bn2 = nn.BatchNorm2d(192)
        self.conv_expand = nn.Conv2d(192, 320, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        x = self.relu(self.bn1(self.conv_reduce(x)))
        x = self.dw_conv1(x)
        se_weights = self.se_block(x)
        x = x * se_weights
        x = self.relu(self.bn2(self.dw_conv2(x)))
        x = self.relu(self.bn3(self.conv_expand(x)))
