...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, expansion_factor, out_channels, stride=1, dropout_prob=0.0):
        ...
        layers.append(DepthwiseSeparableConv(hidden_dim, hidden_dim, stride=stride, dropout_prob=dropout_prob))
        layers.append(nn.Conv2d(hidden_dim, out_channels, kernel_size=1, stride=1, padding=0, bias=False))
        layers.append(nn.BatchNorm2d(out_channels))
        self.conv = nn.Sequential(*layers)
        self.dropout = nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()
        self.shortcut = nn.Sequential() if self.use_residual else nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False), nn.BatchNorm2d(out_channels))

    def forward(self, x):
        out = self.conv(x)
        out = self.dropout(out)
        out += self.shortcut(x) if self.use_residual else 0
        return out
...
class FractalBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_blocks, stride=1, expansion_factor=6, se_ratio=4):
        super(FractalBlock, self).__init__()
        layers = [InvertedResidualBlock(in_channels, expansion_factor, out_channels, stride), SEBlock(out_channels, reduction_ratio=se_ratio)]
        for _ in range(1, num_blocks):
            layers.extend([InvertedResidualBlock(out_channels, expansion_factor, out_channels, stride=1), SEBlock(out_channels, reduction_ratio=se_ratio)])
...
class FractalUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_blocks, stride=1, expansion_factor=6, se_ratio=4):
        ...
        self.fractal = FractalBlock(in_channels, out_channels, num_blocks, stride, expansion_factor, se_ratio)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(DepthwiseSeparableConv(in_shape[1], 32, stride=1, padding=1, dropout_prob=dropout_prob), FractalUnit(32, 64, num_blocks=2, stride=2, expansion_factor=6, se_ratio=4), FractalUnit(64, 128, num_blocks=2, stride=2, expansion_factor=6, se_ratio=4))
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=3, truncate=3).to(device)
        self.backbone_b = TorchVision('shufflenet_v2_x1_0', in_channels=3, truncate=2).to(device)
