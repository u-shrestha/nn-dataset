...
class DepthwiseSeparableBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, dropout_prob=0.0):
        super().__init__()
        self.block = nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False), nn.BatchNorm2d(in_channels), nn.SiLU(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels), nn.SiLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        ...
        self.fc = nn.Sequential(nn.Linear(channels, channels // reduction_ratio, bias=False), nn.SiLU(inplace=True), nn.Linear(channels // reduction_ratio, channels, bias=False), nn.Sigmoid())
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1, dropout_prob=0.0):
        ...
        self.block = nn.Sequential(nn.Conv2d(in_channels, hidden_dim, kernel_size=1, bias=False), nn.BatchNorm2d(hidden_dim), nn.SiLU(inplace=True), DepthwiseSeparableBlock(hidden_dim, hidden_dim, stride), SqueezeExcitation(hidden_dim), nn.Conv2d(hidden_dim, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('shufflenet_v2_x0_5', in_channels=3, truncate=7).to(device)
        self.features = nn.Sequential(InvertedResidualBlock(in_shape[1], 32, expansion_factor=1, stride=1, dropout_prob=dropout_prob), InvertedResidualBlock(32, 64, expansion_factor=6, stride=2, dropout_prob=dropout_prob), InvertedResidualBlock(64, 128, expansion_factor=6, stride=2, dropout_prob=dropout_prob))
