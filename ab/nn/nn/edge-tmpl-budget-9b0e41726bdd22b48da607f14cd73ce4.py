...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.pointwise = nn.Conv2d(256, 128, kernel_size=1, bias=False)
        self.bn_pointwise = nn.BatchNorm2d(128)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(128, 8, kernel_size=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(8, 128, kernel_size=1, bias=True),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        x = self.bn_depthwise(F.relu(self.depthwise(x)))
        x = self.bn_pointwise(F.relu(self.pointwise(x)))
