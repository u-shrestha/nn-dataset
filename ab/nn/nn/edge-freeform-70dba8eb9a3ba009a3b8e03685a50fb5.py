...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            if i == 0:
                self.features.add_module(f'depthwise_separable_{i + 1}', depthwise_separable_block(curr_ch, out_ch, stride=2, dropout_prob=dropout_prob))
            else:
                self.features.add_module(f'inverted_residual_{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=2, dropout_prob=dropout_prob))
            self.features.add_module(f'se_block_{i + 1}', squeeze_excitation_block(out_ch))
            curr_ch = out_ch
