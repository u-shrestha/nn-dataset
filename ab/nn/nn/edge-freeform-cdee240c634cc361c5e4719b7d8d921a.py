...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride):
        ...
        self.conv = nn.Sequential(nn.LazyConv2d(expanded_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Efficient_Dual_Backbone'
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res_block_{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=2, stride=2))
            curr_ch = out_ch
        self.backbone_b = TorchVision('efficientnet_b0', in_channels=3, unwrap=True, truncate=1).to(device)
