...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=6):
    hidden_dim = int(round(in_channels * expand_ratio))
    layers = []
    if in_channels != out_channels or stride != 1:
        layers += [nn.Conv2d(in_channels, hidden_dim, 1, 1, 0, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True)]
    layers += [nn.Conv2d(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, out_channels, 1, 1, 0, bias=False), nn.BatchNorm2d(out_channels)]
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(inverted_residual_block(3, 16, stride=2, expand_ratio=1), inverted_residual_block(16, 24, stride=1, expand_ratio=6), inverted_residual_block(24, 40, stride=2, expand_ratio=6), depthwise_separable_conv(40, 80, stride=1), depthwise_separable_conv(80, 80, stride=1), depthwise_separable_conv(80, 80, stride=1), depthwise_separable_conv(80, 160, stride=2)).to(device)
