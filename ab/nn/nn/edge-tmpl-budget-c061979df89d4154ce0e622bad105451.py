...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.residual_dw = nn.Conv2d(192, 192, kernel_size=3, padding=1, groups=192, bias=False)
        self.bn_residual_dw = nn.BatchNorm2d(192)
    ...
    def forward(self, x):
        ...
        x = self.depthwise(x)
        se = self.squeeze_excitation(x)
        x = x * se
        residual = self.residual_dw(x)
        x = F.relu(self.bn_depthwise(residual + x))
