...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.ReLU6(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class SqueezeExcitationBlock(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        ...
        self.sequential = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(reduced_dim, kernel_size=1, stride=1, padding=0, bias=True), nn.ReLU(inplace=True), nn.LazyConv2d(in_channels, kernel_size=1, stride=1, padding=0, bias=True), nn.Sigmoid())
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1):
        ...
        self.block = nn.Sequential(nn.LazyConv2d(expanded_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), DepthwiseSeparableBlock(expanded_channels, expanded_channels, stride=stride, padding=1), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Optimized_Efficient_Fusion'
        ...
        self.features = nn.Sequential(InvertedResidualBlock(3, 16, expansion_factor=2, stride=1), nn.MaxPool2d(kernel_size=2, stride=2), InvertedResidualBlock(16, 32, expansion_factor=4, stride=1), nn.MaxPool2d(kernel_size=2, stride=2), InvertedResidualBlock(32, 64, expansion_factor=2, stride=1), SqueezeExcitationBlock(64, reduced_dim=16), nn.MaxPool2d(kernel_size=2, stride=2), InvertedResidualBlock(64, 128, expansion_factor=2, stride=1))
