...
def inverted_residual_block(in_channels, out_channels, stride=1, expansion_factor=6, dropout_prob=0.0):
    ...
    layers = [nn.Conv2d(in_channels, hidden_dim, kernel_size=1, stride=1, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, out_channels, kernel_size=1, stride=1, bias=False), nn.BatchNorm2d(out_channels)]
    if dropout_prob > 0 and stride == 1 and (in_channels == out_channels):
        layers.append(nn.Dropout2d(p=dropout_prob))
    return nn.Sequential(*layers)

def depthwise_separable_conv(in_channels, out_channels, stride=1, padding=1, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=False), nn.BatchNorm2d(in_channels), nn.ReLU6(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels), nn.ReLU6(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(inverted_residual_block(3, 16, stride=1, expansion_factor=1, dropout_prob=dropout_prob), inverted_residual_block(16, 32, stride=2, expansion_factor=6, dropout_prob=dropout_prob), squeeze_excitation(32), inverted_residual_block(32, 64, stride=2, expansion_factor=6, dropout_prob=dropout_prob), squeeze_excitation(64), depthwise_separable_conv(64, 128, stride=1, dropout_prob=dropout_prob)).to(device)
