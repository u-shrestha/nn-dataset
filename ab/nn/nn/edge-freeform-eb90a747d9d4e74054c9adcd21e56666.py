...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(nn.LazyConv2d(hidden_dim, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True), DepthwiseSeparableConv(hidden_dim, hidden_dim, stride=stride), nn.LazyConv2d(out_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d())
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduction_ratio=4):
        ...
        self.fc = nn.Sequential(nn.LazyLinear(hidden_dim, bias=False), nn.ReLU(inplace=True), nn.LazyLinear(in_channels, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Efficient_Mobile_Ensemble_SE'
