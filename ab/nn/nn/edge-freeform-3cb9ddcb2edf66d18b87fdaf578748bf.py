...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.ReLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.LazyConv2d(out_channels, kernel_size=3, stride=1, padding=1, bias=bias), nn.ReLU(inplace=True))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride):
        ...
        self.conv = nn.Sequential(nn.LazyConv2d(expanded_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d())
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction=4):
        ...
        self.fc = nn.Sequential(nn.LazyLinear(channels // reduction, bias=False), nn.ReLU(inplace=True), nn.LazyLinear(channels, bias=False), nn.Sigmoid())
...
class EfficientBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride):
        ...
        self.conv = nn.Sequential(InvertedResidualBlock(in_channels, out_channels, expansion_factor, stride), SqueezeExcitation(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(EfficientBlock(1600, 128, 2, 1), EfficientBlock(128, 128, 2, 1), EfficientBlock(128, 128, 2, 1))
