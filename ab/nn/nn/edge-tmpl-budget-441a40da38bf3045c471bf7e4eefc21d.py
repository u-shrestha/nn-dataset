...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.dw_conv = nn.Conv2d(192, 192, kernel_size=3, padding=1, groups=192, bias=False)
        self.bn_dw = nn.BatchNorm2d(192)
    ...
    def forward(self, x):
        ...
        identity = x
        x = self.relu(self.bn_dw(self.dw_conv(x)))
        se_weights = self.se(x)
        x = x * se_weights + identity
