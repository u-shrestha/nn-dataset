...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1):
        ...
        self.expand_conv = nn.LazyConv2d(self.expanded_channels, kernel_size=1, bias=False)
        self.expand_bn = nn.LazyBatchNorm2d()
        self.dw_conv = nn.Conv2d(self.expanded_channels, self.expanded_channels, kernel_size=3, stride=stride, padding=1, groups=self.expanded_channels, bias=False)
        self.dw_bn = nn.LazyBatchNorm2d()
        self.pw_conv = nn.LazyConv2d(out_channels, kernel_size=1, bias=False)
        self.pw_bn = nn.LazyBatchNorm2d()
        self.relu = nn.ReLU6(inplace=True)
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        ...
        self.fc = nn.Sequential(nn.LazyLinear(channels // reduction_ratio, bias=False), nn.ReLU(inplace=True), nn.LazyLinear(channels, bias=False), nn.Sigmoid())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(nn.Sequential(InvertedResidual(in_ch_ij, out_channels, stride=1, expansion_factor=4), SqueezeExcitation(out_channels)))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Hybrid_Efficient_Model'
        ...
        self.backbone_b = TorchVision('shufflenet_v2_x0_5', in_channels=3, truncate=10).to(device)
        ...
        for i, out_ch in enumerate(channels):
            ...
            curr_ch = out_ch
            if i < len(channels) - 1:
                self.features.add_module(f'dsconv{i + 1}', DepthwiseSeparableConv(out_ch, out_ch))
