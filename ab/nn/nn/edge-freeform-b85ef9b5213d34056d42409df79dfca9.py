...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expansion=6, reduction_ratio=16, dropout_prob=0.0):
        super().__init__()
        self.stride = stride
        hidden_dim = round(in_channels * expansion)
        self.conv1 = nn.Conv2d(in_channels, hidden_dim, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(hidden_dim)
        self.dws_conv = DepthwiseSeparableConv(hidden_dim, hidden_dim, stride=stride, dropout_prob=dropout_prob)
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        super().__init__()
        self.fc = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)), nn.Conv2d(channels, channels // reduction_ratio, kernel_size=1, bias=False), nn.ReLU(inplace=True), nn.Conv2d(channels // reduction_ratio, channels, kernel_size=1, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(nn.Conv2d(3, 16, kernel_size=3, stride=2, padding=1, bias=False), nn.BatchNorm2d(16), nn.ReLU6(inplace=True), InvertedResidualBlock(16, 24, stride=1, expansion=6, reduction_ratio=4, dropout_prob=dropout_prob), InvertedResidualBlock(24, 24, stride=1, expansion=6, reduction_ratio=4, dropout_prob=dropout_prob), InvertedResidualBlock(24, 40, stride=2, expansion=6, reduction_ratio=4, dropout_prob=dropout_prob), InvertedResidualBlock(40, 40, stride=1, expansion=6, reduction_ratio=4, dropout_prob=dropout_prob), InvertedResidualBlock(40, 40, stride=1, expansion=6, reduction_ratio=4, dropout_prob=dropout_prob))
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=3, truncate=5).to(device)
        self.backbone_b = TorchVision('shufflenet_v2_x1_0', in_channels=3, truncate=3).to(device)
