...
class PixelUnshuffleBlock(nn.Module):
    """PixelUnshuffle block for reducing spatial dimensions and increasing channel dimensions."""
...
class Net(nn.Module):
    """Enhanced Net combining ELUAvgBlock and PixelUnshuffleBlock for improved denoising performance."""

    def __init__(self, in_shape=(1, 3, 256, 256), out_shape=None, prm={'lr': 0.0001}, device='cuda'):
        super().__init__()
        self.device = device
        Block = ELUAvgBlock
        self.in_conv = nn.Conv2d(3, 16, 3, padding=1)
        self.eb0 = Block(16)
        self.unshuf0 = PixelUnshuffleBlock(2)
        self.down0 = nn.Conv2d(64, 32, 3, padding=1)
        self.eb1 = Block(32)
        self.unshuf1 = PixelUnshuffleBlock(2)
        self.down1 = nn.Conv2d(128, 64, 3, padding=1)
        self.eb2 = Block(64)
        self.unshuf2 = PixelUnshuffleBlock(2)
