...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=4, dropout_prob=0.0):
    ...
    if dropout_prob > 0:
        layers.insert(6, nn.Dropout2d(p=dropout_prob))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res{i + 1}', inverted_residual_block(curr_ch, out_ch, dropout_prob=dropout_prob))
            self.features.add_module(f'depthwise_separable{i + 1}', depthwise_separable_conv(out_ch, out_ch))
            curr_ch = out_ch
