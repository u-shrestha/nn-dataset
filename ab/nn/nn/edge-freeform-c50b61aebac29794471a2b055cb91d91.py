...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio, dropout_prob):
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(drop_conv3x3_block(in_channels, hidden_dim, bias=False))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=3, unwrap=False).to(device)
        self.backbone_a.head = nn.Identity()
