...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.dwconv = nn.Conv2d(192, 192, kernel_size=3, padding=1, groups=192, bias=False)
        self.bn_dw = nn.BatchNorm2d(192)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(192, 12, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(12, 192, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
