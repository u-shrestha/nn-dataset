...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_channels, kernel_size=1, bias=False), nn.BatchNorm2d(expanded_channels), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride, dropout_prob=dropout_prob), nn.Conv2d(expanded_channels, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels))
...
class EfficientBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dropout_prob=0.0, reduction_ratio=16):
...
class EfficientNetLike(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_blocks, expansion_factors, strides, dropout_prob=0.0, reduction_ratios=None):
        super(EfficientNetLike, self).__init__()
        if reduction_ratios is None:
            reduction_ratios = [16] * num_blocks
        layers = []
        for i in range(num_blocks):
            stride = strides[i] if isinstance(strides, list) else stride
            reduction_ratio = reduction_ratios[i] if isinstance(reduction_ratios, list) else reduction_ratio
            layers.append(EfficientBlock(in_channels, out_channels, expansion_factors[i], stride, dropout_prob, reduction_ratio))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = EfficientNetLike(in_channels=1984, out_channels=64, num_blocks=2, expansion_factors=[6, 6], strides=[1, 2], dropout_prob=dropout_prob)
