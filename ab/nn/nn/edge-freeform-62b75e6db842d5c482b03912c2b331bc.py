...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dropout_prob=0.0):
        ...
        self.conv1 = nn.LazyConv2d(expanded_dim, kernel_size=1, bias=False)
        self.bn1 = nn.LazyBatchNorm2d()
        self.conv2 = nn.Conv2d(expanded_dim, expanded_dim, kernel_size=3, stride=stride, padding=1, groups=expanded_dim, bias=False)
        self.bn2 = nn.LazyBatchNorm2d()
        self.conv3 = nn.LazyConv2d(out_channels, kernel_size=1, bias=False)
        self.bn3 = nn.LazyBatchNorm2d()
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        ...
        x = self.se(x)
        x = self.dropout(x)
        x += self.shortcut(identity) if self.stride == 1 else 0
        return x
...
class SEBlock(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        ...
        self.fc1 = nn.LazyConv2d(channels // reduction_ratio, kernel_size=1, stride=1, padding=0)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.LazyConv2d(channels, kernel_size=1, stride=1, padding=0)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(DepthwiseSeparableBlock(576, 128, stride=2, expansion_factor=2, dropout_prob=dropout_prob), InvertedResidualBlock(128, 256, expansion_factor=6, stride=2, dropout_prob=dropout_prob), SEBlock(256), DepthwiseSeparableBlock(256, 512, stride=2, expansion_factor=2, dropout_prob=dropout_prob))
