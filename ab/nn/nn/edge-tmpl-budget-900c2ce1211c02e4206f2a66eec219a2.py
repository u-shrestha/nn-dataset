...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce_dim = nn.Conv2d(in_channels, 256, 1, bias=False)
        self.bn1 = nn.BatchNorm2d(256)
        self.relu1 = nn.ReLU(inplace=True)
        self.dw_conv1 = nn.Conv2d(256, 256, 3, padding=1, groups=256, bias=False)
        ...
        self.conv1x1 = nn.Conv2d(256, 128, 1, bias=False)
        self.bn2 = nn.BatchNorm2d(128)
        self.relu2 = nn.ReLU(inplace=True)
        self.dw_conv2 = nn.Conv2d(128, 128, 3, padding=1, groups=128, bias=False)
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.drop = nn.Dropout(p)
        self.fc = nn.LazyLinear(num_classes)

    def forward(self, x):
        x = self.relu1(self.bn1(self.reduce_dim(x)))
        x = self.dw_conv1(x)
        se_weights = self.se_block(x)
        x = x * se_weights
        x = self.relu2(self.bn2(self.conv1x1(x)))
        x = self.dw_conv2(x)
