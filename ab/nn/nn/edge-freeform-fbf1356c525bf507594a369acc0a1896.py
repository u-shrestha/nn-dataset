...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio=4):
        ...
        layers.append(DepthwiseSeparableConv(in_channels, expanded_dim, stride=1))
        layers.append(DepthwiseSeparableConv(expanded_dim, out_channels, stride=stride))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(nn.Conv2d(3, 32, kernel_size=3, stride=2, padding=1, bias=False), nn.BatchNorm2d(32), nn.ReLU6(inplace=True), InvertedResidual(32, 64, stride=2), InvertedResidual(64, 64, stride=1), InvertedResidual(64, 128, stride=2), InvertedResidual(128, 128, stride=1), InvertedResidual(128, 256, stride=2), InvertedResidual(256, 256, stride=1)).to(device)
