...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_channels, kernel_size=1, stride=1, bias=False), nn.BatchNorm2d(expanded_channels), nn.ReLU(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride), nn.Conv2d(expanded_channels, out_channels, kernel_size=1, stride=1, bias=False), nn.BatchNorm2d(out_channels))
...
class SqueezeExcitationBlock(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=4):
        ...
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(channels, reduced_channels, kernel_size=1, stride=1), nn.ReLU(inplace=True), nn.Conv2d(reduced_channels, channels, kernel_size=1, stride=1), nn.Sigmoid())
...
class EfficientBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride, reduction_ratio):
        super().__init__()
        self.conv = InvertedResidualBlock(in_channels, out_channels, expansion_factor, stride)
        self.se = SqueezeExcitationBlock(out_channels, reduction_ratio)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', EfficientBlock(curr_ch, out_ch, expansion_factor=6, stride=2, reduction_ratio=4))
            curr_ch = out_ch
