...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce_dim = nn.Conv2d(in_channels, 160, kernel_size=1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(160)
        self.depthwise = nn.Conv2d(160, 160, kernel_size=3, padding=1, groups=160, bias=False)
        self.bn_depthwise = nn.BatchNorm2d(160)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(160, 16, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(16, 160, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
        self.expand = nn.Conv2d(160, 320, kernel_size=1, bias=False)
        self.bn_expand = nn.BatchNorm2d(320)
    ...
    def forward(self, x):
        ...
        x = self.se(x) * x + identity
        x = F.relu(self.bn_expand(self.expand(x)))
...
class Net(nn.Module):
    def __init__(self, in_shape, out_shape, prm, device):
        ...
        self.backbone = _edge_backbone('mnasnet0_75', device)
