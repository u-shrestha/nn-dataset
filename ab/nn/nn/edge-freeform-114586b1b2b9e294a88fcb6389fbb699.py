...
class FractalUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_blocks, stride=1, expansion_factor=6):
        ...
        self.fractal = FractalBlock(in_channels, out_channels, num_blocks, stride, expansion_factor)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(FractalUnit(3, 32, num_blocks=2, stride=1, expansion_factor=6), FractalUnit(32, 64, num_blocks=2, stride=2, expansion_factor=6))
    ...
    def infer_dimensions_dynamically(self, in_shape, num_classes):
        ...
        with torch.no_grad():
            ...
            f_feat = adaptive_pool_flatten(self.features(dummy))
            f_backbone = adaptive_pool_flatten(self.backbone_a(dummy))
            fused = torch.cat([f_feat, f_backbone], dim=1)
            self.dim_fused = fused.shape[1]
        self.train()

    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        x = x.to(self.device)
        f_feat = adaptive_pool_flatten(self.features(x))
        f_backbone = adaptive_pool_flatten(self.backbone_a(x))
        fused = torch.cat([f_feat, f_backbone], dim=1)
        if is_probing:
            return fused
