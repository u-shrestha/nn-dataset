...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce_channels = nn.Conv2d(in_channels, 256, 1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(256)
        self.depthwise_conv = nn.Conv2d(256, 256, 3, padding=1, groups=256, bias=False)
        self.bn_depthwise = nn.BatchNorm2d(256)
        self.pointwise_conv = nn.Conv2d(256, 128, 1, bias=False)
        self.bn_pointwise = nn.BatchNorm2d(128)
        self.se_block = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(128, 16, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(16, 128, 1, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        x = F.relu(self.bn_reduce(self.reduce_channels(x)))
        x = F.relu(self.bn_depthwise(self.depthwise_conv(x)))
        x = F.relu(self.bn_pointwise(self.pointwise_conv(x)))
