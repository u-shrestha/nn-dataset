...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.ReLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.SiLU(inplace=True))
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=1, use_se=False):
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm2d(32), nn.ReLU6(inplace=True), InvertedResidual(32, 16, stride=1, expand_ratio=1, use_se=True), InvertedResidual(16, 24, stride=2, expand_ratio=6, use_se=True), InvertedResidual(24, 24, stride=1, expand_ratio=6, use_se=True), nn.Conv2d(24, 48, kernel_size=1, bias=False), nn.BatchNorm2d(48), nn.ReLU6(inplace=True), InvertedResidual(48, 64, stride=2, expand_ratio=6, use_se=True), InvertedResidual(64, 64, stride=1, expand_ratio=6, use_se=True), InvertedResidual(64, 64, stride=1, expand_ratio=6, use_se=True)).to(device)
