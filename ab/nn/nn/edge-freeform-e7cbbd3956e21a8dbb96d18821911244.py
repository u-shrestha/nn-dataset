...
def depthwise_separable_conv(in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.ReLU(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=bias), nn.Dropout(dropout_prob))
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
        ...
        layers.extend([nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels), SEBlock(out_channels)])
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm2d(32), nn.ReLU6(inplace=True), depthwise_separable_conv(32, 64, dropout_prob=dropout_prob), InvertedResidual(64, 128, stride=2, expand_ratio=2, dropout_prob=dropout_prob), InvertedResidual(128, 128, expand_ratio=2, dropout_prob=dropout_prob))
    ...
    def infer_dimensions_dynamically(self, in_shape, num_classes):
        ...
        with torch.no_grad():
            ...
            self.dim_fused = x_f.size(1) + x_a.size(1) + x_b.size(1)
        self.train()
