...
def depthwise_separable_conv3x3_block(in_channels, out_channels, stride=1, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False), nn.LazyBatchNorm2d(), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(nn.LazyConv2d(hidden_dim, kernel_size=1, stride=1, padding=0, bias=False))
            layers.append(nn.LazyBatchNorm2d())
            layers.append(nn.ReLU6(inplace=True))
        layers.append(nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False))
        layers.append(nn.LazyBatchNorm2d())
        layers.append(nn.ReLU6(inplace=True))
        layers.append(nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False))
        layers.append(nn.LazyBatchNorm2d())
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction=4):
        ...
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(channels // reduction, kernel_size=1, stride=1, padding=0), nn.ReLU(inplace=True), nn.LazyConv2d(channels, kernel_size=1, stride=1, padding=0), nn.Sigmoid())

    def forward(self, x):
        return x * self.se(x)

class ModifiedFractalBlock(nn.Module):
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(InvertedResidual(in_ch_ij, out_channels, stride=1, expand_ratio=6, dropout_prob=dropout_prob))
            blocks.append(level)
    ...
    def forward(self, x):
        ...
        return self.se(outs[0])

class ModifiedFractalUnit(nn.Module):

    def __init__(self, in_channels, out_channels, num_columns, loc_drop_prob, dropout_prob):
        super().__init__()
        self.block = ModifiedFractalBlock(in_channels, out_channels, num_columns, loc_drop_prob, dropout_prob)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', ModifiedFractalUnit(curr_ch, out_ch, 3, 0.15, dropout_prob))
            curr_ch = out_ch
