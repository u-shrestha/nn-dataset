...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride, dropout_prob=0.0):
        ...
        expanded_dim = int(in_channels * expansion_factor)
        self.layers = nn.Sequential(nn.LazyConv2d(expanded_dim, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_dim, expanded_dim, stride), nn.LazyConv2d(out_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d())
        self.dropout = nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()

    def forward(self, x):
        if self.use_res_connect:
            return self.dropout(x + self.layers(x))
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        super().__init__()
        reduced_dim = max(1, channels // reduction_ratio)
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(reduced_dim, kernel_size=1, bias=True), nn.ReLU(inplace=True), nn.LazyConv2d(channels, kernel_size=1, bias=True), nn.Sigmoid())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(nn.Sequential(InvertedResidualBlock(in_ch_ij, out_channels, expansion_factor=4, stride=1, dropout_prob=dropout_prob), SqueezeExcitation(out_channels)))
            blocks.append(level)
...
class EfficientFractalUnit(nn.Module):

    def __init__(self, in_channels, out_channels, num_columns, loc_drop_prob, dropout_prob):
        super().__init__()
        self.block = EfficientFractalBlock(in_channels, out_channels, num_columns, loc_drop_prob, dropout_prob)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Efficient_Encoder_Decoder'
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', EfficientFractalUnit(curr_ch, out_ch, 2, 0.15, dropout_prob))
            curr_ch = out_ch
