...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('efficientnet_b0', in_channels=3, truncate=5).to(device)
        self.backbone_b = TorchVision('mobilenet_v3_small', in_channels=3, truncate=6).to(device)
        self.features = nn.Sequential(InvertedResidual(3, 1, 64, 2), nn.Dropout2d(dropout_prob), SqueezeExcitation(64), InvertedResidual(64, 2, 128, 2), nn.Dropout2d(dropout_prob), SqueezeExcitation(128), InvertedResidual(128, 2, 256, 2), nn.Dropout2d(dropout_prob), SqueezeExcitation(256))
