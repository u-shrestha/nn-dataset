...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            if i == 0:
                self.features.add_module(f'dsconv{i + 1}', nn.Conv2d(curr_ch, out_ch, kernel_size=3, stride=2, padding=1, groups=curr_ch, bias=False))
                self.features.add_module(f'pwconv{i + 1}', nn.LazyConv2d(out_ch, kernel_size=1, bias=False))
