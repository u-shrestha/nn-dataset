...
def dw_sep_conv(in_channels, out_channels, stride=1):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False), nn.BatchNorm2d(in_channels), nn.ReLU6(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=False), nn.BatchNorm2d(out_channels), nn.ReLU6(inplace=True))
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio):
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(dw_sep_conv(in_channels, hidden_dim))
        layers.extend([dw_sep_conv(hidden_dim, hidden_dim, stride), nn.Conv2d(hidden_dim, out_channels, kernel_size=1, stride=1, bias=False), nn.BatchNorm2d(out_channels)])
...
class SEBlock(nn.Module):
    ...
    def __init__(self, in_channels, reduction=4):
        ...
        self.fc = nn.Sequential(nn.Linear(in_channels, in_channels // reduction, bias=False), nn.ReLU(inplace=True), nn.Linear(in_channels // reduction, in_channels, bias=False), nn.Sigmoid())
...
class EfficientMobileBackbone(nn.Module):

    def __init__(self, in_channels, width_mult=1.0, inverted_residual_setting=None):
        super(EfficientMobileBackbone, self).__init__()
        block = InvertedResidual
        input_channel = 32
        input_channel = int(input_channel * width_mult)
        features = [dw_sep_conv(in_channels, input_channel)]
        ...
        features.append(dw_sep_conv(input_channel, 1280))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = EfficientMobileBackbone(in_channels=3).to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        channels = [64 * 2 ** i for i in range(2)]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', InvertedResidual(curr_ch, out_ch, stride=1, expand_ratio=1.5))
            self.features.add_module(f'se{i + 1}', SEBlock(out_ch))
            curr_ch = out_ch
