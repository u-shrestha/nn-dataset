...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Hybrid_Dual_Backbone'
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res_block_{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=2 if i == 0 else 1, dropout_prob=dropout_prob))
            curr_ch = out_ch
