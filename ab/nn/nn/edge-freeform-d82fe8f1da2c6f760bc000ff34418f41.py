...
def inverted_residual_block(in_channels, out_channels, stride, expand_ratio=6, dropout_prob=0.0):
    ...
    if stride != 1 or in_channels != out_channels:
        layers.extend([nn.LazyConv2d(hidden_dim, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True)])
    layers.extend([nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d()])
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, (out_ch, stride) in enumerate(zip(channels, strides)):
            self.features.add_module(f'inverted_residual_{i + 1}', inverted_residual_block(curr_ch, out_ch, stride, expand_ratio=4, dropout_prob=dropout_prob))
            self.features.add_module(f'se_block_{i + 1}', SEBlock(out_ch))
            curr_ch = out_ch
        for i in range(2):
            self.features.add_module(f'depthwise_separable_conv_{i + 1}', DepthwiseSeparableConv(curr_ch, curr_ch, dropout_prob=dropout_prob))
