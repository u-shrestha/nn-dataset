...
def depthwise_separable_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=bias), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=2, expand_ratio=6, dropout_prob=dropout_prob))
            curr_ch = out_ch
        self.features.add_module('dsconv1', depthwise_separable_block(curr_ch, curr_ch * 2, stride=2, dropout_prob=dropout_prob))
