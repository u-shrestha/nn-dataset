...
class StatNormBlock(nn.Module):
    """Explicit statistical normalisation (sub mean / div std) + CELU."""
    ...
    def forward(self, x):
        ...
        sd = torch.sqrt(x.var(dim=(2, 3), keepdim=True) + self.eps)
        h = (x - mu) / sd
...
class Net(nn.Module):
    """StatNorm — introduces op types absent from the pool; crossover-capable channel-preserving block."""
