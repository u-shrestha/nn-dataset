...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce_channels = nn.Conv2d(in_channels, 256, kernel_size=1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(256)
        self.depthwise_conv = nn.Conv2d(256, 256, kernel_size=3, padding=1, groups=256, bias=False)
        self.bn_dw = nn.BatchNorm2d(256)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.bn_dw(self.depthwise_conv(x)))
        se_weights = self.squeeze_excitation(x)
        x = x * se_weights
        x = F.relu(self.bn_final(self.conv1x1(x)))
