...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=1, expand_ratio=6, dropout_prob=dropout_prob))
            curr_ch = out_ch
