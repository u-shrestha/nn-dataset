...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce = nn.Conv2d(in_channels, 128, 1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(128)
        self.depthwise = nn.Conv2d(128, 128, 3, padding=1, groups=128, bias=False)
        ...
        self.conv1x1 = nn.Conv2d(128, 64, 1, bias=False)
        self.bn_conv1x1 = nn.BatchNorm2d(64)
    ...
    def forward(self, x):
        ...
        x = x * se_weights
        x = self.bn_conv1x1(self.conv1x1(x))
