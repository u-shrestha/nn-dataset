...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        ...
        self.fc = nn.Sequential(nn.LazyLinear(reduced_dim, bias=False), nn.ReLU(inplace=True), nn.LazyLinear(channels, bias=False), nn.Sigmoid())
...
class SEInvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1, dropout_prob=0.0, se_reduction=16):
        super().__init__()
        expanded_dim = in_channels * expansion_factor
        self.conv = nn.Sequential(nn.LazyConv2d(expanded_dim, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.Conv2d(expanded_dim, expanded_dim, kernel_size=3, stride=stride, padding=1, groups=expanded_dim, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(SEInvertedResidualBlock(in_ch_ij, out_channels, expansion_factor=2, dropout_prob=dropout_prob))
            blocks.append(level)
