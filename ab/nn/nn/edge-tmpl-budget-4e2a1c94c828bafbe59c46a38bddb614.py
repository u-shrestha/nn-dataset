...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce_dim = nn.Conv2d(in_channels, 96, kernel_size=1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(96)
        self.relu = nn.ReLU(inplace=True)
        self.dw_conv1 = nn.Conv2d(96, 96, kernel_size=3, padding=1, groups=96, bias=False)
        self.bn_dw1 = nn.BatchNorm2d(96)
        self.se_block = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(96, 16, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(16, 96, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
        self.conv_expand = nn.Conv2d(96, 192, kernel_size=1, bias=False)
        self.bn_expand = nn.BatchNorm2d(192)
        self.dw_conv2 = nn.Conv2d(192, 192, kernel_size=3, padding=1, groups=192, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.relu(self.bn_dw1(self.dw_conv1(x)))
        se_weights = self.se_block(x)
        x = x * se_weights
        x = self.relu(self.bn_expand(self.conv_expand(x)))
        x = self.relu(self.bn_dw2(self.dw_conv2(x)))
