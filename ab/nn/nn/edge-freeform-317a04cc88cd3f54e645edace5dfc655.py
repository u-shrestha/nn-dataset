...
def inverted_residual_block(in_channels, out_channels, stride, expand_ratio=1):
...
def depthwise_separable_block(in_channels, out_channels, stride=1, expansion_factor=1):
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            if i % 2 == 0:
                self.features.add_module(f'ds_unit{i + 1}', depthwise_separable_block(curr_ch, out_ch, stride=2))
            else:
                self.features.add_module(f'ir_unit{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=2))
            curr_ch = out_ch
