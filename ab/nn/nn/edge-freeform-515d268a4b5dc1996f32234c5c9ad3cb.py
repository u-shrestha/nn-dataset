...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction=16):
        ...
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(channels // reduction, kernel_size=1, padding=0), nn.ReLU(inplace=True), nn.LazyConv2d(channels, kernel_size=1, padding=0), nn.Sigmoid())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(nn.Sequential(drop_conv3x3_block(in_ch_ij, out_channels, dropout_prob=dropout_prob), SqueezeExcitation(out_channels)))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('shufflenet_v2_x0_5', in_channels=3, truncate=5).to(device)
        self.backbone_b = TorchVision('mobilenet_v3_large', in_channels=3, truncate=9).to(device)
        self.features = nn.Sequential(FractalUnit(240, 128, 2, 0.15, dropout_prob), DepthwiseSeparableConv(128, 64))
