...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dropout_prob=0.0):
        ...
        self.dropout = nn.Dropout2d(p=dropout_prob)
        self.shortcut = nn.Identity() if self.use_residual else nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone = TorchVision('mobilenet_v3_small', in_channels=in_shape[1]).to(device)
        self.features = nn.Sequential(DepthwiseSeparableConv(in_shape[1], 32, stride=1, dropout_prob=dropout_prob), SqueezeExcitation(32), InvertedResidual(32, 64, stride=2, dropout_prob=dropout_prob), DepthwiseSeparableConv(64, 64, stride=1, dropout_prob=dropout_prob), SqueezeExcitation(64), InvertedResidual(64, 128, stride=2, dropout_prob=dropout_prob))
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        x_a = adaptive_pool_flatten(self.backbone(x))
