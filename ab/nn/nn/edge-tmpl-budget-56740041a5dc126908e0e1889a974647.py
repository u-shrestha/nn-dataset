...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.squeeze_excitation = nn.Sequential(
            ...
            nn.Conv2d(256, 32, kernel_size=1, bias=False), nn.ReLU(inplace=True),
            nn.Conv2d(32, 256, kernel_size=1, bias=False), nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        identity = x
        x = self.depthwise(x)
        x = self.squeeze_excitation(x) * identity
