...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('resnet34', in_channels=3, weights='DEFAULT').to(device)
        self.backbone_b = TorchVision('resnet34', in_channels=3, weights='DEFAULT').to(device)
        curr_ch = 3
        channels = [64, 128, 256, 512][:-1]
        self.features = nn.Sequential()
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', FractalUnit(curr_ch, out_ch, num_columns, 0.15, dropout_prob))
            curr_ch = out_ch
