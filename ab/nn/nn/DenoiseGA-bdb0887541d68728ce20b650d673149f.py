...
class Net(nn.Module):
    ...
    def __init__(self, in_shape=(1, 3, 256, 256), out_shape=None, prm={}, device="cuda"):
        ...
        self.bridge = nn.Conv2d(f, f, 1)      # carries stage-1 FEATURES into stage 2
        self.s2a = HINBlock(f)
        self.s2b = HINBlock(f)
    ...
    def learn(self, train_data):
        ...
        for noisy, clean in train_data:
            ...
            # Without a loss on stage 1 the bridge carries unsupervised features and the two
            # subnetworks degenerate into one deeper network -- the same failure the
            # multi-stage seed guards against.
            if self._mid is not None:
                loss = loss + self.criterion_l1(
                    torch.clamp(self._mid, 0.0, 1.0), clean) * 30
