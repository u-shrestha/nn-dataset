...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=4, stride=1, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(nn.LazyConv2d(hidden_dim, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True), DepthwiseSeparableConv(hidden_dim, hidden_dim, stride=stride, dropout_prob=dropout_prob), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Ensemble_Backbones_with_Inverted_Residual'
        ...
        self.features = nn.Sequential(InvertedResidual(24, 96, expansion_factor=6, stride=1, dropout_prob=dropout_prob), InvertedResidual(96, 192, expansion_factor=6, stride=2, dropout_prob=dropout_prob), InvertedResidual(192, 384, expansion_factor=6, stride=2, dropout_prob=dropout_prob))
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        combined_features = torch.cat([f_a, f_b], dim=1)
        features_out = self.features(combined_features.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, 7, 7))
        fused = adaptive_pool_flatten(features_out)
        if is_probing:
            return fused
