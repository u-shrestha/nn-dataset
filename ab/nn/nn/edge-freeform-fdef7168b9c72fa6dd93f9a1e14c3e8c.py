...
def depthwise_separable_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=bias), nn.BatchNorm2d(out_channels), nn.ReLU6(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_channels, kernel_size=1, bias=False), nn.BatchNorm2d(expanded_channels), nn.ReLU6(inplace=True), depthwise_separable_conv3x3_block(expanded_channels, expanded_channels, stride=stride, dropout_prob=dropout_prob), nn.Conv2d(expanded_channels, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, (out_ch, stride) in enumerate(zip(channels, strides)):
            self.features.add_module(f'unit{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=2, stride=stride, dropout_prob=dropout_prob))
            curr_ch = out_ch
