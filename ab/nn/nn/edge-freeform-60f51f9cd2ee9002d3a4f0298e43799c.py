...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(DepthwiseSeparableConv(in_channels, hidden_dim, stride=1, padding=1, bias=False, dropout_prob=dropout_prob))
        layers.extend([DepthwiseSeparableConv(hidden_dim, out_channels, stride=stride, padding=1, bias=False, dropout_prob=dropout_prob)])
...
class SEInvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
        ...
        if expand_ratio != 1:
            layers.append(DepthwiseSeparableConv(in_channels, hidden_dim, stride=1, padding=1, bias=False, dropout_prob=dropout_prob))
        layers.extend([DepthwiseSeparableConv(hidden_dim, out_channels, stride=stride, padding=1, bias=False, dropout_prob=dropout_prob), SqueezeExcitation(out_channels)])
        self.conv = nn.Sequential(*layers)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(nn.Conv2d(3, 32, kernel_size=3, stride=2, padding=1, bias=False), nn.BatchNorm2d(32), nn.ReLU6(inplace=True), SEInvertedResidual(32, 16, stride=1, expand_ratio=1, dropout_prob=dropout_prob), SEInvertedResidual(16, 24, stride=2, expand_ratio=6, dropout_prob=dropout_prob), SEInvertedResidual(24, 24, stride=1, expand_ratio=6, dropout_prob=dropout_prob), SEInvertedResidual(24, 40, stride=2, expand_ratio=6, dropout_prob=dropout_prob), SEInvertedResidual(40, 40, stride=1, expand_ratio=6, dropout_prob=dropout_prob), SEInvertedResidual(40, 80, stride=2, expand_ratio=6, dropout_prob=dropout_prob), SEInvertedResidual(80, 80, stride=1, expand_ratio=6, dropout_prob=dropout_prob))
