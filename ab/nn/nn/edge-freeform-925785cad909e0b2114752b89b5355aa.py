...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('shufflenet_v2_x0_5', in_channels=3, unwrap=True, truncate=10).to(device)
        self.backbone_b = TorchVision('mobilenet_v3_small', in_channels=3, unwrap=True, truncate=8).to(device)
        self.features = nn.Sequential()
        curr_ch = 576
        channels = [64, 128, 256]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=2, stride=2))
            self.features.add_module(f'se_{i + 1}', SqueezeExcitation(out_ch))
            self.features.add_module(f'dsconv_{i + 1}', DepthwiseSeparableConv(out_ch, out_ch))
            curr_ch = out_ch
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        fused = torch.nn.functional.interpolate(fused, size=(32, 32), mode='nearest')
        features_out = adaptive_pool_flatten(self.features(fused))
        if is_probing:
            return features_out
        return self.classifier(features_out)
