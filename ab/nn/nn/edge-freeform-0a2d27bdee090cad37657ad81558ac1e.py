...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.ReLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.GELU())
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, expansion_ratio, out_channels, stride):
        ...
        self.expand = nn.LazyConv2d(hidden_dim, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.bn3(x)
        if self.shortcut:
            identity = nn.functional.avg_pool2d(identity, kernel_size=2, stride=2) if self.stride > 1 else identity
            x += identity
        return x
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        ...
        self.fc1 = nn.LazyConv2d(channels // reduction_ratio, kernel_size=1)
...
class EnhancedFractalUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_columns, loc_drop_prob, dropout_prob, expansion_ratio=6):
        super().__init__()
        self.block = nn.Sequential(InvertedResidualBlock(in_channels, expansion_ratio, out_channels, stride=2), DepthwiseSeparableConv(out_channels, out_channels, stride=1), SqueezeExcitation(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        curr_ch = 960
        channels = [128 * 2 ** i for i in range(2)]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', EnhancedFractalUnit(curr_ch, out_ch, 3, 0.15, dropout_prob))
            curr_ch = out_ch
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        x_bf = adaptive_pool_flatten(self.features(x_b))
        fused = torch.cat([x_a, x_bf], dim=1)
        if is_probing:
            return fused
