...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.projector = nn.Conv2d(in_channels, 256, kernel_size=1, bias=False)
        self.bn = nn.BatchNorm2d(256)
        self.relu = nn.ReLU(inplace=True)
        self.shuffle = ShuffleBlock(256, groups=4)
        self.conv = nn.Conv2d(256, 512, kernel_size=3, padding=1, bias=False)
        self.se = SELayer(512, reduction_ratio=16)
...
class ShuffleBlock(nn.Module):
    def __init__(self, channels, groups):
        ...
        self.local_conv = nn.Sequential(
            nn.Conv2d(channels, channels, kernel_size=1, groups=self.groups, bias=False),
...
class ChannelShuffle(nn.Module):
    def __init__(self, groups):
        ...
        assert groups > 1, "groups should be greater than 1"
        self.groups = groups
