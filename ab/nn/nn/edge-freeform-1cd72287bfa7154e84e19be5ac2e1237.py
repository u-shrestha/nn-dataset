...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=1):
    expanded_channels = round(expand_ratio * in_channels)
    layers = []
    if in_channels != expanded_channels:
        layers.append(nn.LazyConv2d(expanded_channels, kernel_size=1, stride=1, bias=False))
    ...
    layers.append(nn.LazyBatchNorm2d())
    layers.append(nn.ReLU6(inplace=True))
    layers.append(nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False))
    layers.append(nn.LazyBatchNorm2d())
    if in_channels == out_channels and stride == 1:
        layers.append(nn.Identity())
    return nn.Sequential(*layers)

def depthwise_separable_conv(in_channels, out_channels, stride=1, padding=1):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=False), nn.LazyBatchNorm2d(), nn.ReLU(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU(inplace=True))

def squeeze_excitation(channels, reduction_ratio=16):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(channels // reduction_ratio, kernel_size=1, stride=1, bias=False), nn.ReLU(inplace=True), nn.LazyConv2d(channels, kernel_size=1, stride=1, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=3, truncate=7).to(device)
        self.backbone_b = TorchVision('shufflenet_v2_x0_5', in_channels=3, truncate=11).to(device)
        self.features = nn.Sequential()
        curr_ch = 1024
        channels = [64 * 2 ** i for i in range(3)]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_residual_{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=2, expand_ratio=2))
            self.features.add_module(f'squeeze_excitation_{i + 1}', squeeze_excitation(out_ch))
            if i < len(channels) - 1:
                self.features.add_module(f'depthwise_separable_{i + 1}', depthwise_separable_conv(out_ch, out_ch))
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        mid_img = torch.nn.functional.interpolate(mid_4d, size=(112, 112), mode='nearest')
