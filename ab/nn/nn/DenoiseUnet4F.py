...
def _reflect_conv(in_ch, out_ch, kernel=3, stride=1):
    # Zero-padded conv (mobile-GPU-clean): the TFLite GPU delegate supports the
    # conv's built-in SAME/zero padding but not REFLECT pad, which forces a CPU
    # fallback. Same receptive field, delegate-friendly.
...
class DualAttentionNet(nn.Module):
    def __init__(self, enc_blocks, dec_blocks, bottom_layers, num_filters):
        ...
        for n_blks in enc_blocks:
            ...
            self.downs.append(
                nn.Conv2d(f, f * 2, 3, stride=2, padding=1),
            )
            f = f * 2
            in_ch = f
        ...
        for i, n_blks in enumerate(dec_blocks):
            ...
            self.skip_convs.append(
                nn.Conv2d(skip_ch, f, 3, padding=1),
            )
            x_ch = f
            group = nn.ModuleList()
            for k in range(n_blks):
        ...
        self.final = nn.Conv2d(x_ch, 3, 3, padding=1)
        self.act = nn.GELU()
