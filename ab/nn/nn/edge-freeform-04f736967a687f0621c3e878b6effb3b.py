...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1):
        ...
        self.stride = stride
        self.residual_path = nn.Sequential(nn.LazyConv2d(self.expanded_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.GELU(), DepthwiseSeparableConvBlock(self.expanded_channels, self.expanded_channels, stride), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d())
        self.identity_path = nn.Sequential()
        if stride != 1 or in_channels != out_channels:
            self.identity_path = nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=1, stride=stride, padding=0, bias=False), nn.LazyBatchNorm2d())
...
class SEBlock(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        super().__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(channels // reduction_ratio, kernel_size=1, stride=1, padding=0), nn.GELU(), nn.LazyConv2d(channels, kernel_size=1, stride=1, padding=0), nn.Sigmoid())
...
class EfficientBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1):
        super().__init__()
        self.inverted_residual = InvertedResidualBlock(in_channels, out_channels, expansion_factor, stride)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', EfficientBlock(curr_ch, out_ch))
            curr_ch = out_ch
