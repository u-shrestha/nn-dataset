...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.ReLU6(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.LazyConv2d(out_channels, kernel_size=3, stride=1, padding=1, bias=bias), nn.ReLU6(inplace=True))

class DepthwiseSeparableBlock(nn.Module):

    def __init__(self, in_channels, out_channels, stride=1, expansion_factor=1, dropout_prob=0.0):
        super().__init__()
        hidden_dim = in_channels * expansion_factor
        self.depthwise = nn.Sequential(nn.Conv2d(in_channels, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True))
        self.pointwise = nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expansion_factor=6, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(nn.LazyConv2d(hidden_dim, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d())
        self.dropout = nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()

    def forward(self, x):
        if self.residual:
            return self.dropout(x + self.conv(x))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_{i + 1}', InvertedResidualBlock(curr_ch, out_ch, stride=1, expansion_factor=2, dropout_prob=dropout_prob))
            self.features.add_module(f'depthwise_{i + 1}', DepthwiseSeparableBlock(out_ch, out_ch, stride=2, expansion_factor=1, dropout_prob=dropout_prob))
            curr_ch = out_ch
