...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=in_shape[1], truncate=3).to(device)
        self.backbone_b = TorchVision('squeezenet1_1', in_channels=in_shape[1], truncate=2).to(device)
        self.features = nn.Sequential(InvertedResidualBlock(in_channels=64, out_channels=128, expansion_factor=6, stride=2, use_se=True, se_ratio=0.25), InvertedResidualBlock(in_channels=128, out_channels=128, expansion_factor=6, stride=1, use_se=True, se_ratio=0.25), DepthwiseSeparableBlock(in_channels=128, out_channels=256, stride=2, padding=1))
