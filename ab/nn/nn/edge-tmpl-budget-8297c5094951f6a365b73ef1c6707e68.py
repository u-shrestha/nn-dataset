...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce = nn.Conv2d(in_channels, 192, kernel_size=1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(192)
        self.depthwise1 = nn.Conv2d(192, 192, kernel_size=3, padding=1, groups=192, bias=False)
        self.bn_depthwise1 = nn.BatchNorm2d(192)
        self.pointwise1 = nn.Conv2d(192, 96, kernel_size=1, bias=False)
        self.bn_pointwise1 = nn.BatchNorm2d(96)
        self.depthwise2 = nn.Conv2d(96, 96, kernel_size=3, padding=1, groups=96, bias=False)
        self.bn_depthwise2 = nn.BatchNorm2d(96)
        self.pointwise2 = nn.Conv2d(96, 48, kernel_size=1, bias=False)
        self.bn_pointwise2 = nn.BatchNorm2d(48)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(48, 6, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(6, 48, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        x = self.bn_reduce(F.relu(self.reduce(x)))
        x = self.bn_depthwise1(F.relu(self.depthwise1(x)))
        x = self.bn_pointwise1(F.relu(self.pointwise1(x)))
        x = self.bn_depthwise2(F.relu(self.depthwise2(x)))
        x = self.bn_pointwise2(F.relu(self.pointwise2(x)))
...
class Net(nn.Module):
    def __init__(self, in_shape, out_shape, prm, device):
        ...
        self.backbone = _edge_backbone('mobilenet_v2', device)
