...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}_a', FractalUnit(curr_ch_a, out_ch, 2, 0.15, self.dropout))
            curr_ch_a = out_ch
    ...
    def infer_dimensions_dynamically(self, in_shape, num_classes):
        ...
        with torch.no_grad():
            ...
            dim_fused = feat_a.shape[1] + feat_b.shape[1]
        self.classifier = nn.Linear(dim_fused, num_classes)
        self.train()

    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        x = x.to(self.device)
        feat_a = adaptive_pool_flatten(self.backbone_a(x))
        feat_b = adaptive_pool_flatten(self.backbone_b(x))
