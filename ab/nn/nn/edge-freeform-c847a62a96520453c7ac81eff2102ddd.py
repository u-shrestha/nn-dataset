...
def depthwise_separable_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=bias), nn.BatchNorm2d(out_channels), nn.ReLU6(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('squeezenet1_1', in_channels=3, truncate=5).to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        channels = [32, 64, 128]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'invres{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=2, stride=2 if i != 0 else 1, dropout_prob=dropout_prob))
            curr_ch = out_ch
        self.backbone_b = TorchVision('shufflenet_v2_x0_5', in_channels=3, truncate=3).to(device)
