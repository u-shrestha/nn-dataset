...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
    ...
    if in_channels != out_channels or stride != 1:
        layers.extend([nn.LazyConv2d(hidden_dim, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d()])
    else:
        layers.extend([nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=1, padding=1, groups=hidden_dim, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()])
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=3, truncate=6).to(device)
        self.backbone_b = TorchVision('shufflenet_v2_x1_0', in_channels=3, truncate=10).to(device)
        ...
        for i, (out_ch, stride, exp_ratio) in enumerate(zip(channels, strides, expand_ratios)):
            self.features.add_module(f'irb_{i + 1}', inverted_residual_block(curr_ch, out_ch, stride, exp_ratio, dropout_prob))
            self.features.add_module(f'seb_{i + 1}', SEBlock(out_ch))
            self.features.add_module(f'dsc_{i + 1}', DepthwiseSeparableConv(out_ch, out_ch, dropout_prob=dropout_prob))
            curr_ch = out_ch
