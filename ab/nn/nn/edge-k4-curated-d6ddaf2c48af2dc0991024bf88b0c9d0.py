...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.output = nn.Sequential(nn.Conv2d(in_channels=in_channels, out_channels=num_classes, kernel_size=1), nn.LeakyReLU(negative_slope=alpha, inplace=True), nn.AdaptiveAvgPool2d(output_size=(1, 1)))
    ...
    def infer_dimensions_dynamically(self):
        dummy = torch.zeros(1, self.in_shape[1], self.image_size, self.image_size).to(self.device)
