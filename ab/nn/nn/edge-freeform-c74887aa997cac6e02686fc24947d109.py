...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride):
        ...
        if expansion_factor != 1:
            layers.append(nn.Conv2d(in_channels, expanded_dim, kernel_size=1, bias=False))
            layers.append(nn.BatchNorm2d(expanded_dim))
            layers.append(nn.ReLU6(inplace=True))
        layers.append(depthwise_separable_conv3x3(expanded_dim, expanded_dim, stride))
        layers.append(nn.Conv2d(expanded_dim, out_channels, kernel_size=1, bias=False))
...
class IRSEBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride):
        ...
        self.ir_block = InvertedResidual(in_channels, out_channels, expansion_factor, stride)
        self.se_block = SEBlock(out_channels)

    def forward(self, x):
        return self.se_block(self.ir_block(x))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(IRSEBlock(3, 32, 1, 1), IRSEBlock(32, 32, 4, 1), IRSEBlock(32, 64, 4, 2), IRSEBlock(64, 64, 4, 1), IRSEBlock(64, 128, 4, 2), nn.AdaptiveAvgPool2d((1, 1)), nn.Flatten()).to(device)
