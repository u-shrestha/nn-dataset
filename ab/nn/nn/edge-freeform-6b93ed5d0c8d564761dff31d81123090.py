...
def inverted_residual_block(in_channels, out_channels, stride=1, expansion_factor=4, dropout_prob=0.0):
    ...
    layers = [nn.LazyConv2d(hidden_dim, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), depthwise_separable_conv(hidden_dim, hidden_dim, stride=stride), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d()]
