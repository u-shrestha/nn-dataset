...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
    ...
    return nn.Sequential(nn.Conv2d(in_channels, hidden_dim, kernel_size=1, stride=1, bias=False), nn.BatchNorm2d(hidden_dim), nn.SiLU(inplace=True), nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim), nn.SiLU(inplace=True), nn.Conv2d(hidden_dim, out_channels, kernel_size=1, stride=1, bias=False), nn.BatchNorm2d(out_channels), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())

def depthwise_separable_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.BatchNorm2d(in_channels), nn.SiLU(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=bias), nn.BatchNorm2d(out_channels), nn.SiLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        self.fc = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(in_channels, reduced_dim, kernel_size=1), nn.ReLU(inplace=True), nn.Conv2d(reduced_dim, in_channels, kernel_size=1), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(inverted_residual_block(3, 24, stride=1, expand_ratio=6, dropout_prob=dropout_prob), SqueezeExcitation(24, reduction_ratio=16), depthwise_separable_block(24, 48, stride=2, padding=1, bias=False, dropout_prob=dropout_prob), SqueezeExcitation(48, reduction_ratio=16), inverted_residual_block(48, 96, stride=2, expand_ratio=6, dropout_prob=dropout_prob), SqueezeExcitation(96, reduction_ratio=16))
