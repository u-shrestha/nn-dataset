...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, padding=1, dropout_prob=0.0):
        super().__init__()
        self.conv = nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels), nn.BatchNorm2d(in_channels), nn.ReLU6(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0), nn.BatchNorm2d(out_channels), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.ReLU6(inplace=True))
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduction_ratio=16):
        super().__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)), nn.Conv2d(in_channels, in_channels // reduction_ratio, kernel_size=1, bias=True), nn.ReLU(inplace=True), nn.Conv2d(in_channels // reduction_ratio, in_channels, kernel_size=1, bias=True), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidual(3, 16, expansion_factor=4, stride=2, dropout_prob=dropout_prob), DepthwiseSeparableConv(16, 32, stride=2, dropout_prob=dropout_prob), InvertedResidual(32, 64, expansion_factor=4, stride=2, dropout_prob=dropout_prob), SqueezeExcitation(64, reduction_ratio=8), DepthwiseSeparableConv(64, 128, stride=2, dropout_prob=dropout_prob), InvertedResidual(128, 128, expansion_factor=4, stride=1, dropout_prob=dropout_prob))
