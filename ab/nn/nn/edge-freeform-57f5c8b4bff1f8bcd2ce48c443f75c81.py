...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.ReLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class SqueezeExcitationBlock(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        super().__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)), nn.LazyConv2d(reduced_dim, kernel_size=1, stride=1, padding=0), nn.ReLU(inplace=True), nn.LazyConv2d(in_channels, kernel_size=1, stride=1, padding=0), nn.Sigmoid())
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, reduction_ratio=4, dropout_prob=0.0):
        super().__init__()
        expanded_dim = in_channels * expand_ratio
        self.residual_connection = stride == 1 and in_channels == out_channels
        self.conv = nn.Sequential(nn.LazyConv2d(expanded_dim, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), DepthwiseSeparableBlock(expanded_dim, expanded_dim, stride=stride), SqueezeExcitationBlock(expanded_dim, expanded_dim // reduction_ratio), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', InvertedResidualBlock(curr_ch, out_ch, dropout_prob=dropout_prob))
            curr_ch = out_ch
