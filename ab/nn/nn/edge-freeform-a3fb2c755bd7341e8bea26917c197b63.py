...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, expansion_factor, out_channels, stride=1, dropout_prob=0.0):
        ...
        self.residual_path = stride == 1 and in_channels == out_channels
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_dim, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(expanded_dim), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_dim, expanded_dim, stride=stride, padding=1, bias=False, dropout_prob=dropout_prob), nn.Conv2d(expanded_dim, out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(DepthwiseSeparableConv(1600, 256, dropout_prob=dropout_prob), InvertedResidualBlock(256, 6, 128, stride=1, dropout_prob=dropout_prob), SqueezeExcitation(128), nn.MaxPool2d(kernel_size=2, stride=2), DepthwiseSeparableConv(128, 256, dropout_prob=dropout_prob), InvertedResidualBlock(256, 6, 64, stride=1, dropout_prob=dropout_prob), SqueezeExcitation(64), nn.MaxPool2d(kernel_size=2, stride=2))
