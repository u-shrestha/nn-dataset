...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_residual_{i + 1}', InvertedResidual(curr_ch, out_ch, stride=1, expand_ratio=2))
            self.features.add_module(f'drop_conv3x3_block_{i + 1}', drop_conv3x3_block(out_ch, out_ch, dropout_prob=dropout_prob))
            curr_ch = out_ch
