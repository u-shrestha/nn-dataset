...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce = nn.Conv2d(in_channels, 160, kernel_size=1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(160)
        self.dwconv1 = nn.Conv2d(160, 160, kernel_size=3, padding=1, groups=160, bias=False)
        self.bn_dw1 = nn.BatchNorm2d(160)
        self.se1 = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(160, 10, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(10, 160, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
        self.dwconv2 = nn.Conv2d(160, 160, kernel_size=3, padding=1, groups=160, bias=False)
        self.bn_dw2 = nn.BatchNorm2d(160)
    ...
    def forward(self, x):
        ...
        x = self.bn_dw1(self.dwconv1(x))
        x = x * self.se1(x)
        x = self.bn_dw2(self.dwconv2(x))
        x = x * self.se2(x)
...
class Net(nn.Module):
    def __init__(self, in_shape, out_shape, prm, device):
        ...
        self.backbone = _edge_backbone('efficientnet_b0', device)
