...
def ds_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(DepthwiseSeparableConv(in_channels, out_channels, stride=stride, padding=padding), nn.Dropout2d(p=dropout_prob))

class DepthwiseSeparableConv(nn.Module):

    def __init__(self, in_channels, out_channels, stride=1, padding=1):
        super().__init__()
        self.depthwise = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels)
...
class EfficientBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=4, stride=1, dropout_prob=0.0):
        super().__init__()
        self.conv = InvertedResidual(in_channels, out_channels, expansion_factor, stride)
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, hidden_dim, kernel_size=1), nn.BatchNorm2d(hidden_dim), nn.SiLU(inplace=True), nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim), nn.BatchNorm2d(hidden_dim), nn.SiLU(inplace=True), nn.Conv2d(hidden_dim, out_channels, kernel_size=1), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(ds_conv3x3_block(3, 16, stride=1, padding=1, bias=False, dropout_prob=dropout_prob), EfficientBlock(16, 24, expansion_factor=6, stride=2, dropout_prob=dropout_prob), EfficientBlock(24, 40, expansion_factor=6, stride=2, dropout_prob=dropout_prob), EfficientBlock(40, 80, expansion_factor=6, stride=2, dropout_prob=dropout_prob), EfficientBlock(80, 96, expansion_factor=6, stride=1, dropout_prob=dropout_prob), EfficientBlock(96, 192, expansion_factor=6, stride=2, dropout_prob=dropout_prob), EfficientBlock(192, 320, expansion_factor=6, stride=1, dropout_prob=dropout_prob))
