    ...
    def learn(self, train_data):
        ...
        if bn_layers and count and (last_noisy is not None):
            ...
            if not torch.isfinite(probe).all():
                print('[bn-heal] eval-mode output non-finite; recalibrating BN running stats', flush=True)
                ...
                with torch.no_grad():
                    probe = self(last_noisy)
                print(f'[bn-heal] post-recalibration finite={torch.isfinite(probe).all().item()}', flush=True)
            self.train()
