...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.depthwise1 = nn.Conv2d(in_channels, in_channels, 3, padding=1, groups=in_channels, bias=False)
        self.bn1 = nn.BatchNorm2d(in_channels)
        self.relu1 = nn.ReLU(inplace=True)
        self.pointwise1 = nn.Conv2d(in_channels, in_channels * 2, 1, bias=False)
        self.bn2 = nn.BatchNorm2d(in_channels * 2)
        self.depthwise2 = nn.Conv2d(in_channels * 2, in_channels * 2, 3, padding=1, groups=in_channels * 2, bias=False)
