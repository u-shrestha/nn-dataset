...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.conv_reduce = nn.Conv2d(in_channels, 128, kernel_size=1, bias=False)
        ...
        self.residual_conv = nn.Conv2d(128, 128, kernel_size=1, bias=False)
        self.bn_residual = nn.BatchNorm2d(128)
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.drop = nn.Dropout(p)
        self.fc = nn.LazyLinear(num_classes)

    def forward(self, x):
        x = self.bn_reduce(self.conv_reduce(x))
        identity = x
        x = self.bn_depthwise(self.depthwise(x))
        x = self.se(x) * x
        x = self.bn_residual(self.residual_conv(x)) + identity
