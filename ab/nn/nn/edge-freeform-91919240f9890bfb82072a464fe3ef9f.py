...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        if inverted_residual_setting is None:
            ...
            inverted_residual_setting = [bneck_conf(16, 3, 16, 16, True, 'RE', 1, 1), bneck_conf(16, 3, 64, 24, False, 'RE', 2, 1), bneck_conf(24, 3, 72, 24, False, 'RE', 1, 1), bneck_conf(24, 5, 72, 40, True, 'HS', 2, 1), bneck_conf(40, 5, 120, 40, True, 'HS', 1, 1), bneck_conf(40, 5, 120, 40, True, 'HS', 1, 1), bneck_conf(40, 5, 120, 48, True, 'HS', 1, 1), bneck_conf(48, 5, 144, 48, True, 'HS', 1, 1), bneck_conf(48, 5, 288, 96 // reduce_divider, True, 'HS', 2, dilation), bneck_conf(96 // reduce_divider, 5, 576 // reduce_divider, 96 // reduce_divider, True, 'HS', 1, dilation), bneck_conf(96 // reduce_divider, 5, 576 // reduce_divider, 96 // reduce_divider, True, 'HS', 1, dilation)]
        ...
        self.classifier = nn.Sequential(nn.Dropout(p=dropout, inplace=True), nn.Linear(lastconv_output_channels, last_channel), nn.Hardswish(inplace=True), nn.Linear(last_channel, num_classes))
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                nn.init.zeros_(m.bias)
        self.to(self.device)
        self.infer_dimensions_dynamically(in_shape)
