...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1):
        ...
        if expansion_factor != 1:
            layers.append(nn.LazyConv2d(hidden_dim, kernel_size=1, bias=False))
        ...
        layers.append(nn.LazyConv2d(out_channels, kernel_size=1, bias=False))
        layers.append(nn.LazyBatchNorm2d())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Ensemble_Depthwise_SE_InvertedResidual'
        ...
        self.features = nn.Sequential(InvertedResidual(1984, 1024, 2, stride=2), InvertedResidual(1024, 512, 2, stride=2), InvertedResidual(512, 256, 2, stride=2), nn.AdaptiveAvgPool2d((1, 1)), nn.Flatten())
