...
def depthwise_separable_block(in_channels, out_channels, stride=1):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False), nn.LazyBatchNorm2d(), nn.ReLU(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU(inplace=True))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidual(496, 256, stride=1), InvertedResidual(256, 128, stride=1), nn.AdaptiveAvgPool2d((1, 1)))
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        mid_img = torch.nn.functional.interpolate(mid_4d, size=(7, 7), mode='nearest')
        fused = self.features(mid_img)
        fused = adaptive_pool_flatten(fused)
        if is_probing:
            return fused
