...
def drop_separable_conv_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    layers = [nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=bias), nn.BatchNorm2d(out_channels), nn.GELU(), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()]
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(drop_separable_conv_block(in_ch_ij, out_channels, stride=1, padding=1, dropout_prob=dropout_prob))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        x = x.to(self.device)
        f_a = global_pool_flatten(self.backbone_a(x))
        x_bf = self.backbone_b(x)
        if x_bf.dim() == 2:
            x_bf = x_bf.unsqueeze(-1).unsqueeze(-1)
        if x_bf.shape[-1] < 14:
            x_bf = F.interpolate(x_bf, size=(14, 14), mode='nearest')
        f_bf = global_pool_flatten(x_bf)
