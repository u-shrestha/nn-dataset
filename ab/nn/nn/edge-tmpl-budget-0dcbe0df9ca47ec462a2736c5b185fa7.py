...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce_dim = nn.Conv2d(in_channels, 192, kernel_size=1, bias=False)
        ...
        self.pointwise1 = nn.Conv2d(192, 384, kernel_size=1, bias=False)
        self.bn_pointwise1 = nn.BatchNorm2d(384)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(384, 96, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(96, 384, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
        self.depthwise2 = nn.Conv2d(384, 384, kernel_size=3, padding=1, groups=384, bias=False)
        self.bn_depthwise2 = nn.BatchNorm2d(384)
        self.pointwise2 = nn.Conv2d(384, 192, kernel_size=1, bias=False)
        self.bn_pointwise2 = nn.BatchNorm2d(192)
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.drop = nn.Dropout(p)
        self.fc = nn.LazyLinear(num_classes)

    def forward(self, x):
        x = F.relu(self.bn_reduce(self.reduce_dim(x)))
        x = F.relu(self.bn_depthwise1(self.depthwise1(x)))
        x = F.relu(self.bn_pointwise1(self.pointwise1(x)))
        se_weights = self.se(x)
        x = x * se_weights
        x = F.relu(self.bn_depthwise2(self.depthwise2(x)))
        x = F.relu(self.bn_pointwise2(self.pointwise2(x)))
...
class Net(nn.Module):
    def __init__(self, in_shape, out_shape, prm, device):
        ...
        self.backbone = _edge_backbone('mobilenet_v3_large', device)
