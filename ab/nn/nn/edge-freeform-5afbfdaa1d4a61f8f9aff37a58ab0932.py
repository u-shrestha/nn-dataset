...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(ds_conv3x3_block(3, 32, stride=2, padding=1, bias=False, dropout_prob=dropout_prob), InvertedResidualBlock(32, 16, stride=1, expand_ratio=1, dropout_prob=dropout_prob), InvertedResidualBlock(16, 24, stride=2, expand_ratio=6, dropout_prob=dropout_prob), InvertedResidualBlock(24, 24, stride=1, expand_ratio=6, dropout_prob=dropout_prob))
        self.infer_dimensions_dynamically(in_shape, out_shape[0])
        self.classifier = nn.Linear(self.dim_fused, out_shape[0]).to(self.device)
        self.to(self.device)
