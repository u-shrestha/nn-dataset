...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.dwconv1 = nn.Conv2d(192, 192, kernel_size=3, padding=1, groups=192, bias=False)
        self.bn_dwconv1 = nn.BatchNorm2d(192)
        self.se_block = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(192, 24, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(24, 192, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
        self.dwconv2 = nn.Conv2d(192, 192, kernel_size=3, padding=1, groups=192, bias=False)
        self.bn_dwconv2 = nn.BatchNorm2d(192)
    ...
    def forward(self, x):
        ...
        x = self.relu(self.bn_dwconv1(self.dwconv1(x)))
        se_weights = self.se_block(x)
        x = x * se_weights
        x = self.relu(self.bn_dwconv2(self.dwconv2(x)))
