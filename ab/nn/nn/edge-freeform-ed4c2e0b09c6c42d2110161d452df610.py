...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(expanded_channels), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_channels, out_channels, stride=stride), nn.BatchNorm2d(out_channels))
...
class SEBlock(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim_ratio=4):
        super().__init__()
        reduced_dim = max(1, in_channels // reduced_dim_ratio)
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)), nn.Conv2d(in_channels, reduced_dim, kernel_size=1, stride=1, padding=0, bias=True), nn.ReLU(inplace=True), nn.Conv2d(reduced_dim, in_channels, kernel_size=1, stride=1, padding=0, bias=True), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=3, unwrap=True, truncate=3).to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        channels = [32, 64, 128]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_residual_{i + 1}', InvertedResidualBlock(curr_ch, out_ch, stride=2))
            self.features.add_module(f'se_{i + 1}', SEBlock(out_ch))
            curr_ch = out_ch
        self.backbone_b = TorchVision('squeezenet1_1', in_channels=3, unwrap=True, truncate=3).to(device)
