...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, padding=1, dilation=1, bias=False, dropout_prob=0.0):
        super(DepthwiseSeparableConv, self).__init__()
        self.depthwise = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, dilation=dilation, groups=in_channels, bias=bias)
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_channels, kernel_size=1, bias=False), nn.BatchNorm2d(expanded_channels), nn.GELU(), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride, padding=1, bias=False, dropout_prob=dropout_prob), nn.BatchNorm2d(expanded_channels), nn.GELU(), nn.Conv2d(expanded_channels, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels))
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(DepthwiseSeparableConv(in_ch_ij, out_channels, dropout_prob=dropout_prob))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_large', in_channels=3, truncate=10).to(device)
        ...
        for i, out_ch in enumerate(channels):
            ...
            curr_ch = out_ch
        self.backbone_b = TorchVision('efficientnet_b0', in_channels=3, truncate=14).to(device)
