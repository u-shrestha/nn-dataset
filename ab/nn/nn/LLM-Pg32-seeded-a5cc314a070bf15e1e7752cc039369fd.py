...
class ContextBranch(nn.Module):
    """Context-aware block with adaptive pooling and convolution."""
    ...
    def forward(self, x):
        ctx = self.pool(x)
        ctx = self.conv(ctx)
        ctx = nn.functional.interpolate(ctx, size=x.shape[-2:], mode='nearest')
        return x + ctx

class Net(nn.Module):
    """U-Net architecture with contextual blocks and skip connections."""
