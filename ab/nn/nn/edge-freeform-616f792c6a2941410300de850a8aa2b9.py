...
def squeeze_excitation_block(in_channels, reduction_ratio=16):
    ...
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(in_channels, reduced_dim, 1, bias=False), nn.SiLU(inplace=True), nn.Conv2d(reduced_dim, in_channels, 1, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(depthwise_separable_block(3, 64, dropout_prob=dropout_prob), inverted_residual_block(64, 64, stride=1, dropout_prob=dropout_prob), squeeze_excitation_block(64))
