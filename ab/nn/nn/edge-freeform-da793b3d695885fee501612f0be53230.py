...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
    ...
    layers = [nn.Conv2d(in_channels, hidden_dim, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(out_channels)]
    if stride == 1 and in_channels == out_channels:
        layers.append(nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
    return nn.Sequential(*layers)

def depthwise_separable_conv(in_channels, out_channels, stride=1):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, 3, stride, 1, groups=in_channels, bias=False), nn.BatchNorm2d(in_channels), nn.ReLU(inplace=True), nn.Conv2d(in_channels, out_channels, 1, 1, 0, bias=False), nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True))

def se_block(channels, reduction_ratio=16):
    return nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)), nn.Conv2d(channels, channels // reduction_ratio, kernel_size=1), nn.ReLU(inplace=True), nn.Conv2d(channels // reduction_ratio, channels, kernel_size=1), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=3, truncate=2).to(device)
        self.backbone_b = TorchVision('efficientnet_b0', in_channels=3, truncate=1).to(device)
        self.features = nn.Sequential(inverted_residual_block(3, 64, stride=2, expand_ratio=6, dropout_prob=dropout_prob), depthwise_separable_conv(64, 128, stride=2), se_block(128, reduction_ratio=16), inverted_residual_block(128, 256, stride=2, expand_ratio=6, dropout_prob=dropout_prob), depthwise_separable_conv(256, 512, stride=2), se_block(512, reduction_ratio=16), inverted_residual_block(512, 1024, stride=2, expand_ratio=6, dropout_prob=dropout_prob), depthwise_separable_conv(1024, 2048, stride=2), se_block(2048, reduction_ratio=16)).to(device)
