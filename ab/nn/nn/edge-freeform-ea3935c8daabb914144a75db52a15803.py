...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1):
        ...
        layers = [nn.Conv2d(in_channels, hidden_dim, kernel_size=1, bias=False), nn.BatchNorm2d(hidden_dim), nn.SiLU(inplace=True), DepthwiseSeparableConv(hidden_dim, hidden_dim, stride=stride), nn.BatchNorm2d(hidden_dim), nn.SiLU(inplace=True), nn.Conv2d(hidden_dim, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels)]
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction=4):
        super().__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)), nn.Conv2d(channels, channels // reduction, kernel_size=1, bias=False), nn.SiLU(inplace=True), nn.Conv2d(channels // reduction, channels, kernel_size=1, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidual(512, 128, expansion_factor=6, stride=1), SqueezeExcitation(128, reduction=4), InvertedResidual(128, 64, expansion_factor=6, stride=1), SqueezeExcitation(64, reduction=4)).to(device)
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        f_bf = adaptive_pool_flatten(self.features(self.backbone_b(x)))
