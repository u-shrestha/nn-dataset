...
def inverted_residual_block(in_channels, out_channels, stride, expand_ratio=6):
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        ...
        reduced_dim = max(channels // reduction_ratio, 8)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(nn.LazyConv2d(reduced_dim, 1, bias=False), nn.ReLU(inplace=True), nn.LazyConv2d(channels, 1, bias=False), nn.Sigmoid())
...
class EnhancedBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.1):
        super(EnhancedBlock, self).__init__()
        self.residual_connection = stride == 1 and in_channels == out_channels
        self.block = nn.Sequential(inverted_residual_block(in_channels, out_channels, stride, expand_ratio), SqueezeExcitation(out_channels), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Optimized_Fractal_Squeeze_Excite'
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', EnhancedBlock(curr_ch, out_ch, expand_ratio=4, dropout_prob=dropout_prob))
            curr_ch = out_ch
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        fused_features = torch.cat([f_a, f_b], dim=1)
        expanded_features = fused_features.unsqueeze(-1).unsqueeze(-1)
        processed_features = self.features(expanded_features)
        avg_pooled_features = adaptive_pool_flatten(processed_features)
        if is_probing:
            return avg_pooled_features
        return self.classifier(avg_pooled_features)
