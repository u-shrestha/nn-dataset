...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dropout_prob=0.0):
        ...
        self.shortcut = nn.Identity() if stride == 1 and in_channels == out_channels else nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=1, stride=stride, padding=0, bias=False), nn.LazyBatchNorm2d())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(InvertedResidualBlock(in_ch_ij, out_channels, expansion_factor=6, stride=1, dropout_prob=dropout_prob))
            blocks.append(level)
