...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
        ...
        self.conv2 = nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False)
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=4, stride=1):
        ...
        self.layers = nn.Sequential(nn.LazyConv2d(expanded_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d())
...
class SEBlock(nn.Module):
    ...
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        self.fc = nn.Sequential(nn.LazyConv2d(in_channels // reduction_ratio, kernel_size=1, bias=False), nn.ReLU(inplace=True), nn.LazyConv2d(in_channels, kernel_size=1, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, (out_ch, stride, exp_ratio) in enumerate(zip(channels, strides, expand_ratios)):
            self.features.add_module(f'inverted_residual_{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=exp_ratio, stride=stride))
            self.features.add_module(f'seblock_{i + 1}', SEBlock(out_ch))
            curr_ch = out_ch
