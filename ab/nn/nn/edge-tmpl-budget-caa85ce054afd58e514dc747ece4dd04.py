...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce = nn.Conv2d(in_channels, 256, kernel_size=1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(256)
        self.depthwise = nn.Conv2d(256, 256, kernel_size=3, padding=1, groups=256, bias=False)
        self.bn_depthwise = nn.BatchNorm2d(256)
        self.squeeze = nn.Conv2d(256, 32, kernel_size=1, bias=False)
        self.excitation = nn.Conv2d(32, 256, kernel_size=1, bias=False)
        self.conv1x1 = nn.Conv2d(256, 128, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        se_weights = self.excitation(F.relu(self.squeeze(nn.functional.adaptive_avg_pool2d(x, 1))))
        x = x * torch.sigmoid(se_weights)
