...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.SiLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.Conv2d(out_channels, out_channels, kernel_size=1, groups=out_channels, bias=bias), nn.Conv2d(out_channels, out_channels, kernel_size=3, groups=out_channels, stride=1, padding=1), nn.BatchNorm2d(out_channels), nn.SiLU(inplace=True))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Optimized_Efficient_Edge_Deployment'
        ...
        self.backbone_b = TorchVision('mobilenet_v3_small', in_channels=3, truncate=12).to(device)
