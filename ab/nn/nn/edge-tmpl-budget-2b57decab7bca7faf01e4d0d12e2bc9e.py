...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce = nn.Conv2d(in_channels, 64, kernel_size=1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(64)
        self.depthwise = nn.Conv2d(64, 64, kernel_size=3, padding=1, groups=64, bias=False)
        self.bn_depthwise = nn.BatchNorm2d(64)
        self.squeeze = nn.Conv2d(64, 16, kernel_size=1, bias=False)
        self.bn_squeeze = nn.BatchNorm2d(16)
        self.expand1 = nn.Conv2d(16, 32, kernel_size=1, bias=False)
        self.bn_expand1 = nn.BatchNorm2d(32)
        self.expand2 = nn.Conv2d(16, 32, kernel_size=1, bias=False)
        self.bn_expand2 = nn.BatchNorm2d(32)
    ...
    def forward(self, x):
        ...
        se_weights = self.relu(self.bn_squeeze(self.squeeze(x)))
        expand1 = self.relu(self.bn_expand1(self.expand1(se_weights)))
        expand2 = self.relu(self.bn_expand2(self.expand2(se_weights)))
        x = torch.cat([expand1, expand2], dim=1)
