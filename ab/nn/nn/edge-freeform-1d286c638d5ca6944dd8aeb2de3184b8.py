...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        super().__init__()
        self.fc1 = nn.LazyConv2d(reduced_dim, kernel_size=1)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.LazyConv2d(in_channels, kernel_size=1)
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1, padding=1, dropout_prob=0.0):
        super().__init__()
        expanded_dim = in_channels * expansion_factor
        self.expansion = nn.LazyConv2d(expanded_dim, kernel_size=1, bias=False)
        self.dw_conv = DepthwiseSeparableBlock(expanded_dim, expanded_dim, stride, padding)
        self.se = SqueezeExcitation(expanded_dim, expanded_dim // 16)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        expansion_factors = [4, 4]
        strides = [1, 2]
        for i, (out_ch, exp_fact, stride) in enumerate(zip(channels, expansion_factors, strides)):
            self.features.add_module(f'unit{i + 1}', InvertedResidualBlock(curr_ch, out_ch, exp_fact, stride, dropout_prob=dropout_prob))
            curr_ch = out_ch
