...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce = nn.Conv2d(in_channels, 96, kernel_size=1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(96)
        self.depthwise = nn.Conv2d(96, 96, kernel_size=3, padding=1, groups=96, bias=False)
        self.bn_depthwise = nn.BatchNorm2d(96)
        self.squeeze = nn.Conv2d(96, 24, kernel_size=1, bias=False)
        self.bn_squeeze = nn.BatchNorm2d(24)
        self.expand1 = nn.Conv2d(24, 48, kernel_size=1, bias=False)
        self.bn_expand1 = nn.BatchNorm2d(48)
        self.expand2 = nn.Conv2d(24, 48, kernel_size=3, padding=1, bias=False)
        self.bn_expand2 = nn.BatchNorm2d(48)
        self.relu = nn.ReLU(inplace=True)
        self.se_block = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(96, 12, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(12, 96, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        x = self.relu(self.bn_squeeze(self.squeeze(x)))
        x1 = self.relu(self.bn_expand1(self.expand1(x)))
        x2 = self.relu(self.bn_expand2(self.expand2(x)))
...
class Net(nn.Module):
    def __init__(self, in_shape, out_shape, prm, device):
        ...
        self.backbone = _edge_backbone('squeezenet1_1', device)
