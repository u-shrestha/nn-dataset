...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=6):
    ...
    if stride != 1 or in_channels != out_channels:
        ...
        layers.append(nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False))
        layers.append(nn.LazyBatchNorm2d())
        layers.append(nn.Dropout2d(p=0.1) if in_channels == out_channels else nn.Identity())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=1, expand_ratio=2))
            curr_ch = out_ch
