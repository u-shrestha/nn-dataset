...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=1, dropout_prob=0.0):
    ...
    layers = []
    if expand_ratio != 1:
        layers.append(nn.LazyConv2d(hidden_dim, kernel_size=1, stride=1, bias=False))
        layers.append(nn.LazyBatchNorm2d())
        layers.append(nn.SiLU(inplace=True))
    layers.extend([nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d()])
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, (out_ch, stride, exp_ratio) in enumerate(zip(channels, strides, expand_ratios)):
            ...
            self.features.add_module(f'seblock{i + 1}', SEBlock(out_ch))
            curr_ch = out_ch
        self.features.add_module('depthwise_separable', DepthwiseSeparableConv(64, 128))
        self.features.add_module('final_seblock', SEBlock(128))
        self.features.add_module('maxpool', MaxPool2d(kernel_size=2, stride=2))
