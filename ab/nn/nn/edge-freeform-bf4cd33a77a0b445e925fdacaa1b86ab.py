...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=4, dropout_prob=0.0):
    ...
    if in_channels != out_channels or stride != 1:
        layers.append(nn.Conv2d(in_channels, out_channels, 1, stride, 0, bias=False))
        layers.append(nn.BatchNorm2d(out_channels))
    layers.append(nn.Conv2d(out_channels, out_channels, 3, 1, 1, groups=out_channels, bias=False))
    layers.append(nn.BatchNorm2d(out_channels))
    layers.append(nn.SiLU(inplace=True))
    layers.append(nn.Conv2d(out_channels, out_channels, 1, 1, 0, bias=False))
    layers.append(nn.BatchNorm2d(out_channels))
    if dropout_prob > 0:
...
def se_block(in_channels, reduction_ratio=16):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(in_channels, in_channels // reduction_ratio, 1, bias=False), nn.ReLU(inplace=True), nn.Conv2d(in_channels // reduction_ratio, in_channels, 1, bias=False), nn.Sigmoid())
