...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Mobile_Net_Style'
        self.use_amp = prm.get('use_amp', False)
        dropout_prob = float(prm['dropout'])
        self.backbone = TorchVision('mobilenet_v3_large', in_channels=3).to(device)
        self.features = nn.Sequential(DepthwiseSeparableBlock(960, 128, stride=2), DepthwiseSeparableBlock(128, 64, stride=1), nn.AdaptiveAvgPool2d((1, 1)), nn.Flatten(), nn.Linear(64, out_shape[0]))
