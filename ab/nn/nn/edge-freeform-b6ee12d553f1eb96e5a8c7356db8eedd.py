...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.LazyBatchNorm2d(), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.ReLU(inplace=True))

def depthwise_separable_conv3x3(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.LazyConv2d(out_channels, kernel_size=1, bias=bias), nn.LazyBatchNorm2d(), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.ReLU(inplace=True))

class InvertedResidualBlock(nn.Module):

    def __init__(self, in_channels, out_channels, expansion_ratio=6, stride=1, dropout_prob=0.0):
        super().__init__()
        self.expanded_channels = expansion_ratio * in_channels
        self.stride = stride
        self.use_res_connect = self.stride == 1 and in_channels == out_channels
        self.layers = nn.Sequential(nn.LazyConv2d(self.expanded_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.Conv2d(self.expanded_channels, self.expanded_channels, kernel_size=3, stride=stride, padding=1, groups=self.expanded_channels, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Hybrid_Backbones_with_DWS_and_IR'
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res{i + 1}', InvertedResidualBlock(curr_ch, out_ch, dropout_prob=dropout_prob))
            curr_ch = out_ch
        self.features.add_module('depthwise_sep', depthwise_separable_conv3x3(curr_ch, curr_ch // 2, dropout_prob=dropout_prob))
        self.features.add_module('pool', MaxPool2d(kernel_size=2, stride=2))
