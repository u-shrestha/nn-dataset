...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=4, dropout_prob=0.0):
    ...
    if in_channels == out_channels and stride == 1:
        layers.insert(0, nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        super().__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(in_channels, reduced_dim, 1), nn.ReLU(inplace=True), nn.Conv2d(reduced_dim, in_channels, 1), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        curr_ch = 3
        channels = [24 * 2 ** i for i in range(3)]
        for i, out_ch in enumerate(channels):
            ...
            curr_ch = out_ch
        self.backbone_b = TorchVision('mobilenet_v3_large', in_channels=3, truncate=10).to(device)
