...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1):
        ...
        self.expansion = expansion_factor * in_channels
        self.conv1 = nn.Conv2d(in_channels, self.expansion, kernel_size=1, stride=1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.expansion)
        self.dwconv = nn.Conv2d(self.expansion, self.expansion, kernel_size=3, stride=stride, padding=1, groups=self.expansion, bias=False)
        self.bn2 = nn.BatchNorm2d(self.expansion)
        self.conv2 = nn.Conv2d(self.expansion, out_channels, kernel_size=1, stride=1, bias=False)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(nn.Conv2d(512, 128, kernel_size=1, bias=False), nn.BatchNorm2d(128), nn.ReLU(inplace=True), DepthwiseSeparableConv(128, 128), InvertedResidualBlock(128, 64, expansion_factor=2), SqueezeExcitation(64, 16))
