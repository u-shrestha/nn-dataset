...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', nn.Sequential(DepthwiseSeparableConv(curr_ch, out_ch, stride=2), SqueezeExcitation(out_ch), InvertedResidual(out_ch, out_ch, stride=1)))
            curr_ch = out_ch
