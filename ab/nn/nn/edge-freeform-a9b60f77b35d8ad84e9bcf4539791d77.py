...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=4, stride=1, use_se=False):
        super().__init__()
        self.use_se = use_se
        expanded_channels = in_channels * expansion_factor
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(expanded_channels), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride), SEBlock(expanded_channels) if use_se else nn.Identity(), nn.Conv2d(expanded_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_large', in_channels=3, truncate=11).to(device)
        self.backbone_b = TorchVision('squeezenet1_1', in_channels=3, truncate=9).to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        channels = [16, 32, 64]
        for i, out_ch in enumerate(channels):
            if i == 0:
                self.features.add_module(f'dsconv{i + 1}', DepthwiseSeparableConv(curr_ch, out_ch, stride=2))
            else:
                self.features.add_module(f'irb{i + 1}', InvertedResidual(curr_ch, out_ch, expansion_factor=4, stride=2, use_se=True))
            curr_ch = out_ch
        self.infer_dimensions_dynamically(in_shape, out_shape[0])
        self.classifier = nn.Linear(self.fused_dim, out_shape[0])
        self.to(self.device)
