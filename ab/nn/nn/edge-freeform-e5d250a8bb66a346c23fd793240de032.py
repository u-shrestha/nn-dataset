...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channel, reduction=16):
        ...
        self.fc = nn.Sequential(nn.LazyConv2d(channel // reduction, 1, bias=False), nn.SiLU(inplace=True), nn.LazyConv2d(channel, 1, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Split_A_Parallel_BF_SE'
        ...
        self.features = nn.Sequential(inverted_residual_block(in_channels=3, out_channels=16, stride=1, expand_ratio=1, dropout_prob=dropout_prob), SqueezeExcitation(channel=16), inverted_residual_block(in_channels=16, out_channels=32, stride=2, expand_ratio=4, dropout_prob=dropout_prob), SqueezeExcitation(channel=32), inverted_residual_block(in_channels=32, out_channels=64, stride=2, expand_ratio=6, dropout_prob=dropout_prob), SqueezeExcitation(channel=64))
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        if x_b.shape[-1] < 14:
            x_b = torch.nn.functional.interpolate(x_b, size=(14, 14), mode='nearest')
        x_b = adaptive_pool_flatten(self.features(x_b))
