...
def _conv_gn_silu(in_ch, out_ch, kernel=3, stride=1, groups=1):
    # BatchNorm (mobile-GPU-clean: folds into conv on the TFLite GPU delegate;
    # GroupNorm is not natively supported and forces a CPU fallback).
...
class _ResidualCRBlock(nn.Module):
    """3x3 -> 3x3 residual block with channel recalibration, BatchNorm and SiLU."""
...
class Net(nn.Module):
    """
    Pyramid-context denoising U-Net with BatchNorm/SiLU residual blocks,
