...
class ReflectPadBlock(nn.Module):
    """ReflectionPad + valid conv + Softplus residual block."""
    ...
    def forward(self, x):
        h = self.act(self.conv1(self.pad(x)))
        return self.conv2(self.pad(h)) + x


class Net(nn.Module):
    """ReflectPad — introduces op types absent from the pool; crossover-capable channel-preserving block."""

    def __init__(self, in_shape=(1, 3, 512, 512), out_shape=None, prm={}, device="cuda"):
        super().__init__()
        self.device = device
        Block = ReflectPadBlock
