...
def depthwise_separable_conv3x3(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=bias), nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, self.expanded_channels, kernel_size=1, bias=False), nn.BatchNorm2d(self.expanded_channels), nn.ReLU6(inplace=True), nn.Conv2d(self.expanded_channels, self.expanded_channels, kernel_size=3, stride=stride, padding=1, groups=self.expanded_channels, bias=False), nn.BatchNorm2d(self.expanded_channels), nn.ReLU6(inplace=True), nn.Conv2d(self.expanded_channels, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels))
        self.shortcut = nn.Sequential() if stride == 1 and in_channels == out_channels else nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False), nn.BatchNorm2d(out_channels))
        self.dropout = nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()

    def forward(self, x):
        if self.stride == 1:
            return self.dropout(self.conv(x) + self.shortcut(x))
        else:
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(InvertedResidualBlock(in_ch_ij, out_channels, stride=1, dropout_prob=dropout_prob))
            blocks.append(level)
