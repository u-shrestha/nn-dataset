...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6):
        super().__init__()
        expanded_dim = int(round(in_channels * expand_ratio))
        self.conv = nn.Sequential(nn.LazyConv2d(expanded_dim, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_dim, expanded_dim, stride=stride), nn.LazyConv2d(out_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d())
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        super().__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(reduced_dim, kernel_size=1), nn.ReLU(inplace=True), nn.LazyConv2d(in_channels, kernel_size=1), nn.Hardsigmoid(inplace=True))
...
class EfficientBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio, se_ratio):
        super().__init__()
        reduced_dim = max(1, int(in_channels * se_ratio))
        self.conv = nn.Sequential(nn.LazyConv2d(in_channels * expand_ratio, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), DepthwiseSeparableConv(in_channels * expand_ratio, in_channels * expand_ratio, stride=stride), SqueezeExcitation(in_channels * expand_ratio, reduced_dim), nn.LazyConv2d(out_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d())
        self.use_res_connect = stride == 1 and in_channels == out_channels
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Efficient_Hybrid_Backbones'
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_residual{i + 1}', InvertedResidualBlock(curr_ch, out_ch, stride=2))
            self.features.add_module(f'efficient_block{i + 1}', EfficientBlock(out_ch, out_ch, stride=1, expand_ratio=6, se_ratio=0.25))
            curr_ch = out_ch
