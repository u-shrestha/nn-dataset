...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv2d(in_channels // 2, in_channels // 2, 3, stride=1, padding=1, groups=in_channels // 2, bias=False),
            nn.BatchNorm2d(in_channels // 2), nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // 2, in_channels, 1, bias=False),
            nn.BatchNorm2d(in_channels), nn.ReLU(inplace=True))
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channels, in_channels // 16, 1, bias=False), nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // 16, in_channels, 1, bias=False), nn.Sigmoid())
