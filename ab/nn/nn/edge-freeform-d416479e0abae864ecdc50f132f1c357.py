...
class SEBlock(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        ...
        self.fc = nn.Sequential(nn.LazyConv2d(channels // reduction_ratio, kernel_size=1, bias=False), nn.ReLU(inplace=True), nn.LazyConv2d(channels, kernel_size=1, bias=False), nn.Sigmoid())

    def forward(self, x):
        se_weights = self.avg_pool(x)
        se_weights = self.fc(se_weights)
        return x * se_weights
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', InvertedResidual(curr_ch, out_ch, stride=2, expansion_factor=6))
