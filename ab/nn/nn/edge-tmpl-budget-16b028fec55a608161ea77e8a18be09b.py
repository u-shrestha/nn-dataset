...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.dwconv = nn.Conv2d(96, 96, kernel_size=3, padding=1, groups=96, bias=False)
        self.bn_dwconv = nn.BatchNorm2d(96)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(96, 8, kernel_size=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(8, 96, kernel_size=1, bias=True),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        x = self.bn_dwconv(self.dwconv(x))
