...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=1, dropout_prob=0.0):
    ...
    layers.extend([nn.Conv2d(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim), nn.SiLU(inplace=True), nn.Conv2d(hidden_dim, out_channels, 1, 1, 0, bias=False), nn.BatchNorm2d(out_channels)])
...
def se_block(in_channels, reduction_ratio=16):
    ...
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(in_channels, hidden_dim, 1, bias=False), nn.ReLU(inplace=True), nn.Conv2d(hidden_dim, in_channels, 1, bias=False), nn.Sigmoid())
