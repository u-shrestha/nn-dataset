...
class SEBlock(nn.Module):
    ...
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        self.fc = nn.Sequential(nn.LazyLinear(in_channels // reduction_ratio), nn.ReLU(inplace=True), nn.LazyLinear(in_channels), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('shufflenet_v2_x0_5', in_channels=3, truncate=8).to(device)
        self.features = nn.Sequential(FractalUnit(576, 128, 2, 0.15, dropout_prob), DepthwiseSeparableConv(128, 64), SEBlock(64))
