...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_ratio=6, stride=1, dropout_prob=0.0):
        ...
        self.conv1 = nn.LazyConv2d(self.expanded_channels, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn1 = nn.LazyBatchNorm2d()
        self.conv2 = nn.LazyConv2d(self.expanded_channels, kernel_size=3, stride=stride, padding=1, groups=self.expanded_channels, bias=False)
        self.bn2 = nn.LazyBatchNorm2d()
        self.conv3 = nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn3 = nn.LazyBatchNorm2d()
        self.relu = nn.ReLU(inplace=True)
        self.dropout = nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()
        self.shortcut = nn.Sequential() if stride == 1 and in_channels == out_channels else nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=1, stride=stride, padding=0, bias=False), nn.LazyBatchNorm2d())
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=4):
        ...
        self.fc1 = nn.LazyLinear(channels // reduction_ratio)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.LazyLinear(channels)
...
class CombinedBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_ratio=6, stride=1, dropout_prob=0.0, reduction_ratio=4):
        super().__init__()
        self.irb = InvertedResidualBlock(in_channels, out_channels, expansion_ratio, stride, dropout_prob)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', CombinedBlock(curr_ch, out_ch, dropout_prob=dropout_prob))
            curr_ch = out_ch
