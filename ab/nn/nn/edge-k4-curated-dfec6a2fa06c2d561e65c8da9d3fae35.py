...
def infer_dimensions_dynamically():
    dummy = torch.zeros(1, 3, 256, 256).to('cuda')
    model = Net((3, 256, 256), (10,), {}, torch.device('cuda'))
    features = model.features(dummy)
    print(features.shape[1:], end='')
...
class BagNetBottleneck(nn.Module):
    ...
    def conv_block(self, in_channels, out_channels, kernel_size, stride):
        ...
        return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, bias=False), nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape, out_shape, prm, device):
        ...
        self.features = nn.Sequential(nn.Conv2d(channel_number, 64, kernel_size=5, stride=2, padding=3, bias=False), nn.BatchNorm2d(64), nn.ReLU(inplace=True), nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
        in_channels = 64
