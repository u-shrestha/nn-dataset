...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False, dropout_prob=0.0):
        super(DepthwiseSeparableConv, self).__init__()
        self.depthwise = nn.Conv2d(in_channels, in_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=in_channels, bias=bias)
        self.pointwise = nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=bias)
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=2, stride=1, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(nn.LazyConv2d(in_channels * expansion_factor, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), DepthwiseSeparableConv(in_channels * expansion_factor, out_channels, stride=stride, dropout_prob=dropout_prob), nn.LazyBatchNorm2d())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(InvertedResidualBlock(in_ch_ij, out_channels, dropout_prob=dropout_prob))
            blocks.append(level)
