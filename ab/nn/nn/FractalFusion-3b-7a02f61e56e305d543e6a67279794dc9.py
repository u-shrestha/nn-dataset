...
def supported_hyperparameters():
    return {'lr', 'dropout', 'momentum', 'max_steps', 'pretrained', 'freeze_backbones'}
...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.freeze_backbones = float(prm.get("freeze_backbones", 0.0)) > 0
        ...
        self.backbones = nn.ModuleList([
            ...
            for name in ["mobilenet_v3_large", "shufflenet_v2_x1_0", "shufflenet_v2_x2_0"]
        ])
        if self.freeze_backbones:
            self.backbones.requires_grad_(False)
            self.backbones.eval()
            assert not any(parameter.requires_grad for parameter in self.backbones.parameters())
