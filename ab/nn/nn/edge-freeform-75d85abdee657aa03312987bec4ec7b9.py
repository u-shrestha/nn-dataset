...
def se_block(in_channels, reduction_ratio=16):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.LazyLinear(in_channels // reduction_ratio), nn.ReLU(inplace=True), nn.LazyLinear(in_channels), nn.Sigmoid(), nn.Unflatten(1, (in_channels, 1, 1)))

def depthwise_separable_conv(in_channels, out_channels, stride=1):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            if i == 0:
                self.features.add_module(f'dsconv{i + 1}', depthwise_separable_conv(curr_ch, out_ch, stride=2))
            else:
                self.features.add_module(f'irb{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=2, dropout_prob=dropout_prob))
            self.features.add_module(f'se{i + 1}', se_block(out_ch))
            curr_ch = out_ch
