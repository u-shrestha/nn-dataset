...
def inverted_residual_block(in_channels, out_channels, stride, expand_ratio=1, dropout_prob=0.0):
    expanded_channels = round(expand_ratio * in_channels)
    layers = []
    if in_channels != expanded_channels:
        layers.append(nn.Conv2d(in_channels, expanded_channels, kernel_size=1, stride=1, bias=False))
    ...
    layers.append(nn.BatchNorm2d(expanded_channels))
    layers.append(nn.ReLU6(inplace=True))
    layers.append(nn.Conv2d(expanded_channels, out_channels, kernel_size=1, stride=1, bias=False))
...
def se_block(channel, reduction=4):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(channel, channel // reduction, kernel_size=1, bias=False), nn.ReLU(inplace=True), nn.Conv2d(channel // reduction, channel, kernel_size=1, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('squeezenet1_1', in_channels=3, truncate=2).to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        channels = [32, 64, 128]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=2, expand_ratio=2, dropout_prob=dropout_prob))
            curr_ch = out_ch
