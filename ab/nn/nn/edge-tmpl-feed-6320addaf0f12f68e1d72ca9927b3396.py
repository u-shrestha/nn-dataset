...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.conv1 = nn.Conv2d(in_channels, in_channels, 3, padding=1, groups=in_channels, bias=False)
        ...
        self.se_block = nn.Sequential(
            ...
            nn.Conv2d(in_channels * 2, in_channels // 8, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // 8, in_channels * 2, 1, bias=False),
            nn.Sigmoid()
        )
        self.dw_conv = nn.Conv2d(in_channels * 2, in_channels * 2, 3, padding=1, groups=in_channels * 2, bias=False)
