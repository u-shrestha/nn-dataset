...
class DepthwiseSeparableConv(nn.Module):
    ...
    def forward(self, x):
        return self.relu(self.dropout(self.pointwise(self.depthwise(x))))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dropout_prob=0.0):
        ...
        self.relu = nn.ReLU6(inplace=True)
        self.dws_conv = DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride, dropout_prob=dropout_prob)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Hybrid_Efficient_MobileNet'
        self.use_amp = prm.get('use_amp', False)
        dropout_prob = float(prm.get('dropout', 0.1))
        self.backbone_a = TorchVision('efficientnet_b0', in_channels=3, truncate=4).to(device)
        self.features = nn.Sequential()
        curr_ch = 1280
        channels = [64 * 2 ** i for i in range(2)]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'irb{i + 1}', InvertedResidualBlock(curr_ch, out_ch, dropout_prob=dropout_prob))
            curr_ch = out_ch
        self.backbone_b = TorchVision('mobilenet_v3_small', in_channels=3, truncate=2).to(device)
