...
def se_block(channel, reduction=4):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyLinear(channel // reduction, bias=False), nn.ReLU(inplace=True), nn.LazyLinear(channel, bias=False), nn.Sigmoid())

def se_inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
    hidden_dim = int(round(in_channels * expand_ratio))
    layers = [nn.LazyConv2d(hidden_dim, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), se_block(hidden_dim), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d()]
    if dropout_prob > 0 and stride == 1 and (in_channels == out_channels):
        layers.append(nn.Dropout2d(p=dropout_prob))
    return nn.Sequential(*layers)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(inverted_residual_block(3, 16, stride=1, expand_ratio=6, dropout_prob=dropout_prob), se_inverted_residual_block(16, 24, stride=2, expand_ratio=6, dropout_prob=dropout_prob), se_inverted_residual_block(24, 24, stride=1, expand_ratio=6, dropout_prob=dropout_prob), se_inverted_residual_block(24, 32, stride=2, expand_ratio=6, dropout_prob=dropout_prob), se_inverted_residual_block(32, 32, stride=1, expand_ratio=6, dropout_prob=dropout_prob))
