...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.LazyBatchNorm2d(), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.LazyConv2d(out_channels, kernel_size=3, stride=1, padding=1, bias=bias), nn.SiLU(inplace=True))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride, dropout_prob):
        super().__init__()
        ...
        self.dwconv = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False)
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=4):
        ...
        self.fc = nn.Sequential(nn.LazyLinear(channels // reduction_ratio), nn.ReLU(inplace=True), nn.LazyLinear(channels), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_large', in_channels=in_shape[1]).to(device)
        self.backbone_b = TorchVision('shufflenet_v2_x1_0', in_channels=in_shape[1]).to(device)
        self.features = nn.Sequential()
        curr_ch = 576
        channels = [96, 192]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'irb{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=2, stride=2, dropout_prob=dropout_prob))
            self.features.add_module(f'se{i + 1}', SqueezeExcitation(out_ch))
            curr_ch = out_ch
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        fused = torch.cat([f_a, f_b, x_f], dim=1)
        if is_probing:
            return fused
