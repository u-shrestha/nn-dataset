        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(nn.Sequential(DepthwiseSeparableConv(in_ch_ij, out_channels), SqueezeExcitation(out_channels, out_channels // 16)))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Hybrid_Efficient_Mobile_Fractal'
        ...
        curr_ch = 3
        channels = [48 * 2 ** i for i in range(3)]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'depthwise_{i + 1}', DepthwiseSeparableConv(curr_ch, out_ch))
            self.features.add_module(f'fractal_unit_{i + 1}', FractalUnit(out_ch, out_ch, 2, dropout_prob))
            curr_ch = out_ch
