...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.project = nn.Conv2d(in_channels, 256, kernel_size=1, bias=False)
        self.shuffle = ShuffleBlock(groups=256)
        self.conv = nn.Conv2d(256, 256, kernel_size=3, padding=1, groups=256, bias=False)
        self.bn = nn.BatchNorm2d(256)
        self.se = SqueezeExcitation(256, reduction_ratio=16)
...
class ShuffleBlock(nn.Module):
    def __init__(self, groups):
        ...
        self.grouped_conv = nn.Conv2d(groups, groups, kernel_size=3, stride=1, padding=1, groups=groups, bias=False)
