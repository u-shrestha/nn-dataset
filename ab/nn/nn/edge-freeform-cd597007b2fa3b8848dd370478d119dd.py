...
class FractalUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_columns, loc_drop_prob, dropout_prob):
        super().__init__()
        self.block = depthwise_separable_block(in_channels, out_channels, dropout_prob=dropout_prob)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidual(3, 64, stride=1, dropout_prob=dropout_prob), FractalUnit(64, 128, num_columns=2, loc_drop_prob=0.15, dropout_prob=dropout_prob), SEBlock(128))
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        mid_4d = f_a.unsqueeze(-1).unsqueeze(-1)
