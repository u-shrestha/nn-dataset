...
def ds_conv_block(in_channels, out_channels, stride=1, padding=1):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels), nn.BatchNorm2d(in_channels), nn.ReLU(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1), nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, expansion_factor, out_channels, stride):
        ...
        layers.append(ds_conv_block(hidden_dim, hidden_dim, stride=stride))
...
class FractalBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_blocks, stride=1, expansion_factor=6, se_ratio=4):
        ...
        self.se = SEBlock(out_channels, reduction_ratio=se_ratio)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            if i == 0:
                self.features.add_module(f'dsconv{i + 1}', ds_conv_block(curr_ch, out_ch, stride=2))
            else:
                self.features.add_module(f'irb{i + 1}', InvertedResidualBlock(curr_ch, expansion_factor=6, out_channels=out_ch, stride=2))
            self.features.add_module(f'se{i + 1}', SEBlock(out_ch, reduction_ratio=16))
            curr_ch = out_ch
