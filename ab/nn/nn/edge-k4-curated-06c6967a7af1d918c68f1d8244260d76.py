...
def depthwise_separable_conv3x3(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=bias), nn.ReLU(inplace=True), nn.Dropout(p=dropout_prob))
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(depthwise_separable_conv3x3(in_ch_ij, out_channels, dropout_prob=dropout_prob))
            blocks.append(level)
