...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.depthwise1 = nn.Conv2d(160, 160, kernel_size=3, padding=1, groups=160, bias=False)
        ...
        self.depthwise2 = nn.Conv2d(160, 160, kernel_size=3, padding=1, groups=160, bias=False)
    ...
    def forward(self, x):
        x = self.act1(self.bn1(self.reduce(x)))
        x = self.act1(self.bn2(self.depthwise1(x)))
        se_weights = self.se(x)
        x = x * se_weights
        x = self.act2(self.bn3(self.depthwise2(x)))
