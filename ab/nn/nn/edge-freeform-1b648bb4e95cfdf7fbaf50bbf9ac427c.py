...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expansion_factor=6, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(DepthwiseSeparableBlock(in_channels, hidden_dim, stride=stride), SqueezeExcitation(hidden_dim, hidden_dim // 16), DepthwiseSeparableBlock(hidden_dim, out_channels, stride=1))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Efficient_Hybrid_SE_Architecture'
