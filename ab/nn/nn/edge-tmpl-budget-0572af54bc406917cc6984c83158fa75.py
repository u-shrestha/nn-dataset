...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce_dim = nn.Conv2d(in_channels, 128, 1, bias=False)
        self.bn1 = nn.BatchNorm2d(128)
        self.relu = nn.ReLU(inplace=True)
        self.depthwise_conv = nn.Conv2d(128, 128, 3, padding=1, groups=128, bias=False)
        self.bn2 = nn.BatchNorm2d(128)
        self.squeeze = nn.Conv2d(128, 32, 1, bias=False)
        self.bn_squeeze = nn.BatchNorm2d(32)
        self.excite = nn.Conv2d(32, 128, 1, bias=False)
    ...
    def forward(self, x):
        ...
        se_weights = torch.sigmoid(self.bn_excite(self.excite(se_weights)))
        x = x * se_weights
