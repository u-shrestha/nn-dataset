...
def depthwise_separable_conv3x3(in_channels, out_channels, stride=1):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False), nn.BatchNorm2d(in_channels), nn.ReLU(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=False), nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True))
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio):
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(depthwise_separable_conv3x3(in_channels, hidden_dim, 1))
        layers.extend([nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, out_channels, kernel_size=1, stride=1, bias=False), nn.BatchNorm2d(out_channels)])
...
class EfficientBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, reduction=4):
        ...
        self.use_se = stride == 1 and in_channels == out_channels
        self.conv = nn.Sequential(depthwise_separable_conv3x3(in_channels, in_channels * expand_ratio, stride=stride), SqueezeExcitation(in_channels * expand_ratio, reduction=reduction) if self.use_se else nn.Identity(), nn.Conv2d(in_channels * expand_ratio, out_channels, kernel_size=1, stride=1, bias=False), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Ensemble_Backbones_to_EfficientBlocks'
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', EfficientBlock(curr_ch, out_ch, stride=2 if i == 0 else 1))
            curr_ch = out_ch
