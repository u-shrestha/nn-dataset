...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio=6, dropout_prob=0.0):
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Ensemble_Backbones_to_InvertedResidual'
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res{i + 1}', InvertedResidualBlock(curr_ch, out_ch, stride=1, dropout_prob=dropout_prob))
            curr_ch = out_ch
