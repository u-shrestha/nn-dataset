...
class SEBlock(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        ...
        self.fc = nn.Sequential(nn.Conv2d(channels, channels // reduction_ratio, kernel_size=1, bias=True), nn.ReLU(inplace=True), nn.Conv2d(channels // reduction_ratio, channels, kernel_size=1, bias=True), nn.Hardsigmoid(inplace=True))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio, dropout_prob=0.0):
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(DSConv(in_channels, hidden_dim, stride=1, dropout_prob=dropout_prob))
        layers.append(DSConv(hidden_dim, out_channels, stride=stride, dropout_prob=dropout_prob))
...
class FractalUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_blocks, stride=1, expansion_factor=6, dropout_prob=0.0):
        super(FractalUnit, self).__init__()
        layers = [InvertedResidualBlock(in_channels, out_channels, stride, expansion_factor, dropout_prob)]
        for _ in range(1, num_blocks):
            layers.append(InvertedResidualBlock(out_channels, out_channels, 1, expansion_factor, dropout_prob))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        curr_ch = 3
        channels = [24, 40, 80]
        expansion_factors = [2, 1, 2]
        strides = [1, 2, 2]
        for i, (out_ch, exp_factor, stride) in enumerate(zip(channels, expansion_factors, strides)):
            self.features.add_module(f'fractal_unit{i + 1}', FractalUnit(curr_ch, out_ch, num_blocks=2, stride=stride, expansion_factor=exp_factor, dropout_prob=dropout_prob))
            curr_ch = out_ch
