...
class EfficientBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=4, stride=1, dropout_prob=0.0):
        ...
        self.se = SqueezeExcitation(out_channels, reduced_dim=out_channels // 4)
        self.dropout = nn.Dropout2d(p=dropout_prob)
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, hidden_dim, kernel_size=1, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim), nn.ReLU6(inplace=True), nn.Conv2d(hidden_dim, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels))
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        super().__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(in_channels, reduced_dim, 1, bias=True), nn.ReLU(inplace=True), nn.Conv2d(reduced_dim, in_channels, 1, bias=True), nn.Hardsigmoid(inplace=True))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(DepthwiseSeparableConv(3, 16, stride=2, padding=1, dropout_prob=dropout_prob), EfficientBlock(16, 24, expansion_factor=6, stride=2, dropout_prob=dropout_prob), EfficientBlock(24, 24, expansion_factor=6, stride=1, dropout_prob=dropout_prob), EfficientBlock(24, 40, expansion_factor=6, stride=2, dropout_prob=dropout_prob), EfficientBlock(40, 40, expansion_factor=6, stride=1, dropout_prob=dropout_prob), SqueezeExcitation(40, reduced_dim=10), nn.MaxPool2d(kernel_size=2, stride=2))
