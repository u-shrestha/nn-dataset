...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.depthwise1 = nn.Conv2d(64, 64, kernel_size=3, padding=1, groups=64, bias=False)
        self.bn_depthwise1 = nn.BatchNorm2d(64)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(64, 8, kernel_size=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(8, 64, kernel_size=1, bias=True),
            nn.Sigmoid()
        )
        self.depthwise2 = nn.Conv2d(64, 64, kernel_size=3, padding=1, groups=64, bias=False)
        self.bn_depthwise2 = nn.BatchNorm2d(64)
        self.pointwise = nn.Conv2d(64, 32, kernel_size=1, bias=False)
        self.bn_pointwise = nn.BatchNorm2d(32)
