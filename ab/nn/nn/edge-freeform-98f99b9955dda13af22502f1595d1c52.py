...
class SEBlock(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        ...
        self.fc = nn.Sequential(nn.LazyConv2d(reduced_dim, kernel_size=1, bias=True), nn.ReLU(inplace=True), nn.LazyConv2d(channels, kernel_size=1, bias=True), nn.Sigmoid())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(nn.Sequential(InvertedResidualBlock(in_ch_ij, out_channels, dropout_prob=dropout_prob), SEBlock(out_channels)))
            blocks.append(level)
...
class EfficientFractalUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_columns, dropout_prob):
        super().__init__()
        self.block = EfficientFractalBlock(in_channels, out_channels, num_columns, dropout_prob)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', EfficientFractalUnit(curr_ch, out_ch, 2, dropout_prob))
            curr_ch = out_ch
