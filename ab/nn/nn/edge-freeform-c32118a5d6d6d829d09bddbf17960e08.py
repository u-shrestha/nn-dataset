...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
        ...
        self.hidden_dim = round(in_channels * expand_ratio)
        self.use_res_connect = self.stride == 1 and in_channels == out_channels
        layers = []
        if expand_ratio != 1:
            layers.append(nn.Conv2d(in_channels, self.hidden_dim, kernel_size=1, bias=False))
            layers.append(nn.BatchNorm2d(self.hidden_dim))
            layers.append(nn.ReLU(inplace=True))
        layers.append(DepthwiseSeparableConv(self.hidden_dim, self.hidden_dim, stride=stride, dropout_prob=dropout_prob))
        layers.append(nn.Conv2d(self.hidden_dim, out_channels, kernel_size=1, bias=False))
