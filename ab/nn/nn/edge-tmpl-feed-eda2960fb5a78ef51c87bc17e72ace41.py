...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.depthwise_conv1 = nn.Conv2d(in_channels, in_channels, 3, padding=1, groups=in_channels, bias=False)
        self.bn1 = nn.BatchNorm2d(in_channels)
        self.pointwise_conv1 = nn.Conv2d(in_channels, in_channels // 2, 1, bias=False)
        self.bn2 = nn.BatchNorm2d(in_channels // 2)
        self.relu = nn.ReLU(inplace=True)
        self.depthwise_conv2 = nn.Conv2d(in_channels // 2, in_channels // 2, 3, padding=1, groups=in_channels // 2, bias=False)
        self.bn3 = nn.BatchNorm2d(in_channels // 2)
        self.se_block = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channels // 2, in_channels // 32, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // 32, in_channels // 2, 1, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        x = self.relu(self.bn1(self.depthwise_conv1(x)))
        x = self.relu(self.bn2(self.pointwise_conv1(x)))
        x = self.relu(self.bn3(self.depthwise_conv2(x)))
