...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.dwconv1 = nn.Conv2d(96, 96, kernel_size=3, padding=1, groups=96, bias=False)
        self.bn_dwconv1 = nn.BatchNorm2d(96)
        self.se1 = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(96, 12, kernel_size=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(12, 96, kernel_size=1, bias=True),
            nn.Sigmoid()
        )
        self.dwconv2 = nn.Conv2d(96, 96, kernel_size=3, padding=1, groups=96, bias=False)
        self.bn_dwconv2 = nn.BatchNorm2d(96)
    ...
    def forward(self, x):
        ...
        x = self.bn_dwconv1(self.dwconv1(x))
        se_weights1 = self.se1(x)
        x = x * se_weights1
        x = self.bn_dwconv2(self.dwconv2(x))
        se_weights2 = self.se2(x)
        x = x * se_weights2
