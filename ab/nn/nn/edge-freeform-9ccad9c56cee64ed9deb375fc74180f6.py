...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_large', in_channels=3, truncate=12).to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        channels = [16, 24, 32]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=6, dropout_prob=dropout_prob))
            curr_ch = out_ch
            if i < len(channels) - 1:
