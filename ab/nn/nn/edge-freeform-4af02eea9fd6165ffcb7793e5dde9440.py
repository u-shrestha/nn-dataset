...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=bias), nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expansion=6, reduction_ratio=16):
        super(InvertedResidualBlock, self).__init__()
        self.stride = stride
        self.expanded_channels = expansion * in_channels
        ...
        self.dws_conv = DepthwiseSeparableConv(self.expanded_channels, self.expanded_channels, stride=stride)
        self.se = SqueezeExcitation(self.expanded_channels, reduction_ratio)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res{i + 1}', InvertedResidualBlock(curr_ch, out_ch, stride=2 if i > 0 else 1, expansion=6, reduction_ratio=16))
            curr_ch = out_ch
            if i < len(channels) - 1:
