...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1):
        ...
        self.expansion_conv = DepthwiseSeparableConv(in_channels, expanded_dim)
        self.bottleneck_conv = nn.Conv2d(expanded_dim, out_channels, kernel_size=1, stride=1, padding=0, bias=False)
...
class SqueezeExcitationBlock(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        super().__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(channels, channels // reduction_ratio, kernel_size=1, stride=1, padding=0, bias=False), nn.ReLU(inplace=True), nn.Conv2d(channels // reduction_ratio, channels, kernel_size=1, stride=1, padding=0, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidualBlock(3, 16, expansion_factor=1, stride=1), SqueezeExcitationBlock(16), DepthwiseSeparableConv(16, 32, stride=2), nn.MaxPool2d(kernel_size=2, stride=2), InvertedResidualBlock(32, 64, expansion_factor=2, stride=1), SqueezeExcitationBlock(64), DepthwiseSeparableConv(64, 128, stride=2), nn.MaxPool2d(kernel_size=2, stride=2))
