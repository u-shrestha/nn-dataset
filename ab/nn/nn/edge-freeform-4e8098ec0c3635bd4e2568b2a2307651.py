...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.LazyBatchNorm2d(), nn.ReLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, expansion_factor, out_channels, stride, dropout_prob=0.0):
        ...
        self.layers = nn.Sequential(nn.LazyConv2d(expanded_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d())
        self.dropout = nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()

    def forward(self, x):
        return x + self.dropout(self.layers(x)) if self.identity else self.layers(x)
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        ...
        self.fc = nn.Sequential(nn.LazyLinear(reduced_dim), nn.ReLU(inplace=True), nn.LazyLinear(in_channels), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', InvertedResidualBlock(curr_ch, expansion_factor=2, out_channels=out_ch, stride=2, dropout_prob=dropout_prob))
            self.features.add_module(f'se{i + 1}', SqueezeExcitation(out_ch, out_ch // 4))
            curr_ch = out_ch
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        x_features = adaptive_pool_flatten(self.features(fused.unsqueeze(-1).unsqueeze(-1)))
        if is_probing:
            return x_features
