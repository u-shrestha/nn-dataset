...
def depthwise_separable_conv_block(in_channels, out_channels, stride=1, dropout_prob=0.0):
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(nn.LazyConv2d(expanded_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), depthwise_separable_conv_block(expanded_channels, out_channels, stride=stride, dropout_prob=dropout_prob))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=3, unwrap=False, truncate=4).to(device)
        self.features = nn.Sequential()
        curr_ch = 160
        channels = [128 * 2 ** i for i in range(2)]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res_block_{i + 1}', InvertedResidualBlock(curr_ch, out_ch, stride=2, dropout_prob=dropout_prob))
            curr_ch = out_ch
        self.backbone_b = TorchVision('shufflenet_v2_x1_0', in_channels=3, unwrap=False, truncate=3).to(device)
