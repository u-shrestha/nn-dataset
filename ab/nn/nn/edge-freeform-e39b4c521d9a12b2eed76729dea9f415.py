...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=4, dropout_prob=0.0):
    ...
    if dropout_prob > 0:
        layers.insert(7, nn.Dropout2d(p=dropout_prob))
    if in_channels != out_channels or stride != 1:
        layers.insert(0, nn.LazyConv2d(out_channels, 1, stride, 0, bias=False))
        layers.insert(1, nn.LazyBatchNorm2d())
    return nn.Sequential(*layers)

def depthwise_separable_block(in_channels, out_channels, stride=1):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, 3, stride, 1, groups=in_channels, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, 1, 1, 0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True))
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(inverted_residual_block(in_ch_ij, out_channels, dropout_prob=dropout_prob))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('shufflenet_v2_x0_5', in_channels=in_shape[1]).to(device)
        self.features = nn.Sequential()
        curr_ch = in_shape[1]
        channels = [64 * 2 ** i for i in range(2)]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', FractalUnit(curr_ch, out_ch, 2, dropout_prob))
            curr_ch = out_ch
