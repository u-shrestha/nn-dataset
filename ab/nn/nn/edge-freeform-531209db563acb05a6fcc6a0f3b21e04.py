...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio):
        ...
        layers.append(DepthwiseSeparableConv(hidden_dim, hidden_dim, stride))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.transform = 'norm_256_flip'
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=in_shape[1]).to(self.device)
        self.backbone_b = TorchVision('shufflenet_v2_x1_0', in_channels=in_shape[1]).to(self.device)
        self.features = nn.Sequential(nn.Conv2d(in_shape[1], 32, kernel_size=3, stride=2, padding=1, bias=False), nn.BatchNorm2d(32), nn.ReLU6(inplace=True), InvertedResidual(32, 16, 1, 1), InvertedResidual(16, 24, 2, 6), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), InvertedResidual(24, 24, 1, 6), InvertedResidual(24, 40, 2, 6), SqueezeExcitation(40), InvertedResidual(40, 40, 1, 6), InvertedResidual(40, 40, 1, 6), InvertedResidual(40, 80, 2, 6), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), InvertedResidual(80, 80, 1, 6), InvertedResidual(80, 80, 1, 6), InvertedResidual(80, 80, 1, 6), InvertedResidual(80, 112, 1, 6), SqueezeExcitation(112), InvertedResidual(112, 112, 1, 6), InvertedResidual(112, 160, 2, 6), InvertedResidual(160, 160, 1, 6), SqueezeExcitation(160), InvertedResidual(160, 160, 1, 6), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.AdaptiveAvgPool2d((1, 1)), nn.Flatten())
