...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.dwconv1 = nn.Conv2d(in_channels // 2, in_channels // 2, 3, padding=1, groups=in_channels // 2, bias=False)
        self.bn2 = nn.BatchNorm2d(in_channels // 2)
        self.dwconv2 = nn.Conv2d(in_channels // 2, in_channels // 2, 3, padding=1, groups=in_channels // 2, bias=False)
        self.bn3 = nn.BatchNorm2d(in_channels // 2)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channels // 2, in_channels // 16, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // 16, in_channels // 2, 1, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        x = self.relu(self.bn2(self.dwconv1(x)))
        x = self.relu(self.bn3(self.dwconv2(x)))
