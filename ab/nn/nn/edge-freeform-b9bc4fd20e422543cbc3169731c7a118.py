...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio, dropout_prob=0.0):
        ...
        layers.extend([ds_conv3x3_block(hidden_dim, out_channels, stride=stride, bias=False, dropout_prob=dropout_prob), SqueezeExcitation(out_channels) if stride == 1 and in_channels == out_channels else nn.Identity()])
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(ds_conv3x3_block(3, 32, stride=1, padding=1, bias=False, dropout_prob=dropout_prob), InvertedResidualBlock(32, 64, stride=2, expand_ratio=2, dropout_prob=dropout_prob), SqueezeExcitation(64), InvertedResidualBlock(64, 128, stride=2, expand_ratio=2, dropout_prob=dropout_prob), SqueezeExcitation(128)).to(device)
