...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_residual_{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=4, stride=1 if i == 0 else 2))
            curr_ch = out_ch
