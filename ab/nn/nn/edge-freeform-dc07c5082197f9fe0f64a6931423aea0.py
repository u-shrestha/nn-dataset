...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio, dropout_prob=0.0):
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(depthwise_separable_conv(in_channels, hidden_dim, bias=False))
        layers.extend([depthwise_separable_conv(hidden_dim, out_channels, stride=stride, bias=False), nn.Dropout2d(p=dropout_prob)])
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(depthwise_separable_conv(3, 16, stride=1, padding=1, bias=False), InvertedResidualBlock(16, 24, stride=2, expand_ratio=6, dropout_prob=dropout_prob), SqueezeExcitation(24), InvertedResidualBlock(24, 32, stride=2, expand_ratio=6, dropout_prob=dropout_prob), SqueezeExcitation(32), InvertedResidualBlock(32, 64, stride=2, expand_ratio=6, dropout_prob=dropout_prob), SqueezeExcitation(64))
        self.to(self.device)
