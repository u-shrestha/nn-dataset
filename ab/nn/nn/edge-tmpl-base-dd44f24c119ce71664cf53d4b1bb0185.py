...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.squeeze = nn.Conv2d(in_channels, in_channels // 4, 1)
        self.excite = nn.Conv2d(in_channels // 4, in_channels, 1)
    ...
    def forward(self, x):
        ...
        x = self.depthwise(x)
        x = self.bn(x)
        se = F.relu(self.squeeze(F.adaptive_avg_pool2d(x, 1)))
        se = torch.sigmoid(self.excite(se))
        x = x * se.expand_as(x) + identity
