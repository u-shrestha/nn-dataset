...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        ...
        self.fc = nn.Sequential(nn.Linear(in_channels, reduced_dim, bias=False), nn.ReLU(inplace=True), nn.Linear(reduced_dim, in_channels, bias=False), nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        out = identity * out.expand_as(identity)
        return out

class SEInvertedResidualBlock(nn.Module):

    def __init__(self, in_channels, out_channels, expansion_factor=4, stride=1, reduction_ratio=4):
        super(SEInvertedResidualBlock, self).__init__()
        self.residual_block = InvertedResidualBlock(in_channels, out_channels, expansion_factor, stride)
        self.se_block = SqueezeExcitation(out_channels, out_channels // reduction_ratio)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', SEInvertedResidualBlock(curr_ch, out_ch, stride=2 if i > 0 else 1, expansion_factor=2, reduction_ratio=4))
            curr_ch = out_ch
