...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expansion_factor=6, dropout_prob=0.0):
        ...
        self.dwconv = nn.LazyConv2d(self.expanded_channels, kernel_size=3, stride=stride, padding=1, groups=self.expanded_channels, bias=False)
        self.bn2 = nn.LazyBatchNorm2d()
        self.se = SEBlock(self.expanded_channels)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res_block{i + 1}', InvertedResidualBlock(curr_ch, out_ch, dropout_prob=dropout_prob))
            curr_ch = out_ch
