...
def se_block(channel, reduction=4):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(channel // reduction, kernel_size=1, padding=0, bias=False), nn.SiLU(inplace=True), nn.LazyConv2d(channel, kernel_size=1, padding=0, bias=False), nn.Sigmoid())

class SEInvertedResidualBlock(nn.Module):

    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0, reduction=4):
        super().__init__()
        self.conv = inverted_residual_block(in_channels, out_channels, stride, expand_ratio, dropout_prob)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'se_ir_unit{i + 1}', SEInvertedResidualBlock(curr_ch, out_ch, dropout_prob=dropout_prob))
            curr_ch = out_ch
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        f_bf = adaptive_pool_flatten(self.backbone_b(x))
        mid = torch.cat([f_a, f_bf], dim=1)
