...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias=False):
        ...
        self.pointwise = nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=bias)
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride):
        ...
        self.conv = nn.Sequential(nn.LazyConv2d(self.expanded_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), DepthwiseSeparableConv(self.expanded_channels, self.expanded_channels, kernel_size=3, stride=stride, padding=1), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d())
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=4):
        ...
        self.sequential = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(reduced_channels, kernel_size=1, stride=1, padding=0), nn.ReLU(inplace=True), nn.LazyConv2d(channels, kernel_size=1, stride=1, padding=0), nn.Sigmoid())

    def forward(self, x):
        return x * self.sequential(x)

class EfficientFractalSEBlock(nn.Module):

    def __init__(self, in_channels, out_channels, expansion_factor=4, stride=1, se_reduction_ratio=4, num_columns=2, dropout_prob=0.0):
        super(EfficientFractalSEBlock, self).__init__()
        self.fractal_units = nn.ModuleList()
        for _ in range(num_columns):
            self.fractal_units.append(InvertedResidualBlock(in_channels, out_channels, expansion_factor, stride))
        self.se_block = SqueezeExcitation(out_channels, se_reduction_ratio)
        self.dropout = nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()

    def forward(self, x):
        outputs = [fractal_unit(x) for fractal_unit in self.fractal_units]
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('efficientnet_b0', in_channels=in_shape[1], truncate=1).to(device)
        self.features = nn.Sequential()
        curr_ch = 16
        channels = [32 * 2 ** i for i in range(2)]
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', EfficientFractalSEBlock(curr_ch, out_ch, expansion_factor=4, stride=1, se_reduction_ratio=4, num_columns=2, dropout_prob=dropout_prob))
            curr_ch = out_ch
