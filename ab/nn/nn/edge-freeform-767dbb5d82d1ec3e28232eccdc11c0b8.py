...
def ds_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.BatchNorm2d(in_channels), nn.ReLU6(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=bias), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio, dropout_prob=0.0):
        ...
        layers.extend([ds_conv3x3_block(hidden_dim, out_channels, stride=stride, bias=False, dropout_prob=dropout_prob), SqueezeExcitation(out_channels)])
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(ds_conv3x3_block(3, 64, stride=2, padding=1, bias=False, dropout_prob=dropout_prob), InvertedResidualBlock(64, 64, stride=1, expand_ratio=2, dropout_prob=dropout_prob), SqueezeExcitation(64), InvertedResidualBlock(64, 128, stride=2, expand_ratio=2, dropout_prob=dropout_prob), SqueezeExcitation(128))
        self.to(self.device)
