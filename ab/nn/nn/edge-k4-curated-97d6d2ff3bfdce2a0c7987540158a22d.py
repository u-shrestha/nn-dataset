...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.SiLU(inplace=True), nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=bias), nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=bias))
...
class Net(nn.Module):
    ...
    def infer_dimensions_dynamically(self, in_shape, num_classes):
        ...
        with torch.no_grad():
            ...
            x_f = adaptive_pool_flatten(output_feat)
            x_a = adaptive_pool_flatten(self.backbone_a(dummy))
            x_b = adaptive_pool_flatten(self.backbone_b(dummy))
            merged = torch.cat([x_f, x_a, x_b], dim=1)
            dim_fused = merged.shape[1]
