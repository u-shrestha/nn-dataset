...
def depthwise_separable_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=bias), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())

def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=4, dropout_prob=0.0):
    hidden_dim = int(round(in_channels * expand_ratio))
    layers = [nn.LazyConv2d(hidden_dim, 1, 1, 0, bias=False), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True), nn.Conv2d(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True), nn.LazyConv2d(out_channels, 1, 1, 0, bias=False), nn.LazyBatchNorm2d()]
...
def se_block(in_channels, reduction_ratio=16):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(in_channels // reduction_ratio, 1, bias=False), nn.SiLU(inplace=True), nn.LazyConv2d(in_channels, 1, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, (out_ch, stride, exp_ratio) in enumerate(zip(channels, strides, expand_ratios)):
            self.features.add_module(f'inverted_resblock{i + 1}', inverted_residual_block(curr_ch, out_ch, stride, exp_ratio, dropout_prob))
            self.features.add_module(f'se_block{i + 1}', se_block(out_ch))
            curr_ch = out_ch
