...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0):
        ...
        layers.append(DepthwiseSeparableConv(hidden_dim, out_channels, stride=stride, dropout_prob=dropout_prob))
...
class SEBlock(nn.Module):
    ...
    def __init__(self, channel, reduction=16):
        ...
        self.fc = nn.Sequential(nn.LazyConv2d(channel // reduction, kernel_size=1, padding=0, bias=False), nn.ReLU(inplace=True), nn.LazyConv2d(channel, kernel_size=1, padding=0, bias=False), nn.Sigmoid())
...
class SEInvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, dropout_prob=0.0, se_reduction=16):
        super().__init__()
        hidden_dim = round(in_channels * expand_ratio)
        self.use_res_connect = stride == 1 and in_channels == out_channels
        layers = []
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'seinverted{i + 1}', SEInvertedResidualBlock(curr_ch, out_ch, stride=2 if i == 0 else 1, dropout_prob=dropout_prob))
            curr_ch = out_ch
