...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(nn.Conv2d(3, 32, kernel_size=3, stride=2, padding=1, bias=False), nn.BatchNorm2d(32), nn.ReLU6(inplace=True))
        curr_ch = 32
        self.features.add_module('irb1', InvertedResidual(curr_ch, 16, stride=1, expand_ratio=1, dropout_prob=dropout_prob))
        curr_ch = 16
        self.features.add_module('irb2', SEInvertedResidual(curr_ch, 24, stride=2, expand_ratio=6, dropout_prob=dropout_prob))
        curr_ch = 24
        self.features.add_module('irb3', SEInvertedResidual(curr_ch, 24, stride=1, expand_ratio=6, dropout_prob=dropout_prob))
