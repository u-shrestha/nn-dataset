...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio=4, dropout_prob=0.0):
        super(InvertedResidualBlock, self).__init__()
        self.stride = stride
        expanded_dim = in_channels * expand_ratio
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_dim, kernel_size=1, bias=False), nn.BatchNorm2d(expanded_dim), nn.ReLU(inplace=True), DepthwiseSeparableConv(expanded_dim, expanded_dim, stride=stride), nn.Conv2d(expanded_dim, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels))
    ...
    def forward(self, x):
        ...
        x = self.conv(x)
        x = self.dropout(x)
        return nn.functional.relu6(x + residual) if self.stride == 1 else x
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        ...
        self.fc = nn.Sequential(nn.Conv2d(in_channels, reduced_dim, kernel_size=1), nn.ReLU(inplace=True), nn.Conv2d(reduced_dim, in_channels, kernel_size=1), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidualBlock(3, 32, stride=1, expand_ratio=1, dropout_prob=dropout_prob), InvertedResidualBlock(32, 64, stride=2, expand_ratio=4, dropout_prob=dropout_prob), SqueezeExcitation(64, reduced_dim=16))
