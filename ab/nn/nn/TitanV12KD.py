# Model Architecture: TitanV12_KD
# NPU-Optimized Super-Resolution | KD Student (λ=0.1)
# 8 ResBlocks | 64 Channels
...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple,
        ...
        # Body: 8 x ResBlock (TitanV12 depth)
    ...
    def _load_teacher(self):
        if self.teacher is None:
            # We load HSRv4 dynamically to avoid modifying core scripts
            from ab.nn.nn.HSRv4 import Net as TeacherNet
            # Creating dummy shapes and prm, as they might not be fully needed for inference
            self.teacher = TeacherNet((3, 48, 48), (3, 144, 144), {}, self.device).to(self.device)
            ckpt_path = os.path.join("out", "ckpt", "HSRv4", "best_model.pth")
            
            if os.path.exists(ckpt_path):
                self.teacher.load_state_dict(torch.load(ckpt_path, map_location=self.device))
                print(f"[KD] Successfully loaded teacher HSRv4 from {ckpt_path}")
            else:
                print(f"[KD] WARNING: Teacher checkpoint NOT FOUND at {ckpt_path}")
    ...
    def train_setup(self, prm):
        ...
        max_epochs = prm.get("epoch_max", 400)
    ...
    def learn(self, data_roll: DataRoll):
        # Load teacher if not loaded yet
        self._load_teacher()
        ...
        for inputs, labels in data_roll:
            ...
            # Student output
            student_out = self.forward(inputs)
            
            # Task Loss (Charbonnier on HR)
            task_loss = self.criterion(student_out, labels)
            
            # KD Loss (MSE on Teacher's soft targets)
            kd_loss = 0.0
            if self.teacher is not None:
                with torch.no_grad():
                    teacher_out = self.teacher(inputs)
                kd_loss = F.mse_loss(student_out, teacher_out)
            
            # Total Loss
            loss = task_loss + self.kd_lambda * kd_loss
