...
class SEInvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio=1, reduction=16):
        ...
        layers.append(nn.LazyBatchNorm2d())
        layers.append(nn.SiLU(inplace=True))
        layers.append(se_block(expanded_channels, reduction))
        layers.append(nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False))
        layers.append(nn.LazyBatchNorm2d())
        ...
        if in_channels == out_channels and stride == 1:
            ...
            shortcut_layers = [nn.LazyConv2d(out_channels, kernel_size=1, stride=stride, bias=False), nn.LazyBatchNorm2d()]
            self.shortcut = nn.Sequential(*shortcut_layers)

    def forward(self, x):
        out = self.block(x)
        out = out * self.block[-4](out)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', SEInvertedResidual(curr_ch, out_ch, stride=2, expand_ratio=2))
            curr_ch = out_ch
