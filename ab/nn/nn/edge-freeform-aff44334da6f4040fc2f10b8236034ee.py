...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            if i == 0:
                self.features.add_module(f'conv{i + 1}', nn.Conv2d(curr_ch, out_ch, kernel_size=3, stride=2, padding=1, bias=False))
                self.features.add_module(f'bn{i + 1}', nn.BatchNorm2d(out_ch))
                self.features.add_module(f'relu{i + 1}', nn.ReLU6(inplace=True))
            else:
                self.features.add_module(f'irb{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=2 if i == 1 else 1))
            curr_ch = out_ch
