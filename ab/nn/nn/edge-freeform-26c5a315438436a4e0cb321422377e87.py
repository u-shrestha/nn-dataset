...
class DSConv3x3Block(nn.Module):

    def __init__(self, in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
        super().__init__()
        self.conv = nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.BatchNorm2d(in_channels), nn.ReLU6(inplace=True), nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=bias), nn.Dropout2d(p=dropout_prob))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio, dropout_prob=0.0):
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(DSConv3x3Block(in_channels, hidden_dim, bias=False, dropout_prob=dropout_prob))
        layers.extend([DSConv3x3Block(hidden_dim, out_channels, stride=stride, bias=False, dropout_prob=dropout_prob), SEBlock(out_channels)])
