...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=4, dropout_prob=0.0):
    ...
    if in_channels != out_channels or stride != 1:
        convs.append(nn.Conv2d(in_channels, hidden_dim, kernel_size=1, stride=1, bias=False))
        convs.append(nn.BatchNorm2d(hidden_dim))
        convs.append(nn.ReLU6(inplace=True))
    convs.append(nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, groups=hidden_dim, padding=1, bias=False))
    convs.append(nn.BatchNorm2d(hidden_dim))
    convs.append(nn.ReLU6(inplace=True))
    convs.append(nn.Conv2d(hidden_dim, out_channels, kernel_size=1, stride=1, bias=False))
