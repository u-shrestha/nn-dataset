...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.depthwise = nn.Conv2d(192, 192, kernel_size=3, padding=1, groups=192, bias=False)
        self.bn_depthwise = nn.BatchNorm2d(192)
        self.squeeze = nn.Conv2d(192, 32, kernel_size=1, bias=False)
        self.bn_squeeze = nn.BatchNorm2d(32)
        self.excite = nn.Conv2d(32, 192, kernel_size=1, bias=False)
        self.bn_excite = nn.BatchNorm2d(192)
        self.conv1x1 = nn.Conv2d(192, 96, kernel_size=1, bias=False)
        self.bn_conv1x1 = nn.BatchNorm2d(96)
    ...
    def forward(self, x):
        ...
        se = F.relu(self.bn_squeeze(self.squeeze(x)))
        se = torch.sigmoid(self.bn_excite(self.excite(se)))
        x = x * se
        x = F.relu(self.bn_conv1x1(self.conv1x1(x)))
...
class Net(nn.Module):
    def __init__(self, in_shape, out_shape, prm, device):
        ...
        self.backbone = _edge_backbone('mnasnet0_5', device)
