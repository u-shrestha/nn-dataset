...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio, dropout_prob=0.0):
        ...
        layers.append(SqueezeExcitation(out_channels, out_channels // 16))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_res_{i + 1}', InvertedResidualBlock(curr_ch, out_ch, stride=2, expand_ratio=2, dropout_prob=dropout_prob))
            curr_ch = out_ch
