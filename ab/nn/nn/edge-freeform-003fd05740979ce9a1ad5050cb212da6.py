...
class Net(nn.Module):
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        fused_4d = fused.unsqueeze(-1).unsqueeze(-1)
        fused_img = torch.nn.functional.interpolate(fused_4d, size=(14, 14), mode='nearest')
        final_feat = adaptive_pool_flatten(self.features(fused_img))
        if is_probing:
            return final_feat
