...
class EdgePreservingLoss(nn.Module):
    ...
    def forward(self, pred, target):
        return F.l1_loss(pred, target) + self.edge_weight * self._grad_mag(pred - target)
    ...
    def __init__(self, enc_blocks, dec_blocks, bottom_layers, num_filters):
        ...
        for n_blks in enc_blocks:
            ...
            for k in range(n_blks):
                group.append(nn.Sequential(nn.Conv2d(in_ch if k == 0 else f, f, 3, padding=1), nn.ELU()))
                in_ch = f
        ...
        for k in range(bottom_layers):
            self.bottom.append(nn.Sequential(nn.Conv2d(in_ch if k == 0 else f, f, 3, padding=1), nn.ELU()))
            in_ch = f
        ...
        for i, n_blks in enumerate(dec_blocks):
            ...
            group = nn.ModuleList()
            for k in range(n_blks):
                group.append(nn.Sequential(nn.Conv2d(cat_ch if k == 0 else f, f, 3, padding=1), nn.ELU()))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape=(1, 3, 512, 512), out_shape=None, prm={}, device='cuda'):
        super().__init__()
        self.device = device
        self.net = LightFusionNet(enc_blocks=[2, 2, 2, 2], dec_blocks=[2, 2, 2, 2], bottom_layers=2, num_filters=32)
    ...
    def train_setup(self, prm):
        ...
        self.scheduler = optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=t_max, eta_min=1e-05)
    ...
    def learn(self, train_data):
        ...
        for noisy, clean in train_data:
            ...
            preds = self(noisy)
            loss = torch.mean(torch.sqrt((preds - clean) ** 2 + 0.001 ** 2))
            loss.backward()
