...
class InvertedResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out += self.shortcut(identity) if self.use_residual else 0
        return out
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidualBlock(in_shape[1], 64, expansion_factor=1, stride=1, dropout_prob=dropout_prob), InvertedResidualBlock(64, 128, expansion_factor=2, stride=2, dropout_prob=dropout_prob), SEBlock(128), DSConv(128, 128, stride=1, dropout_prob=dropout_prob), nn.AdaptiveAvgPool2d((1, 1)), nn.Flatten())
        self.infer_dimensions_dynamically(in_shape, out_shape[0])
        self._scaler = make_scaler(enabled=True)
