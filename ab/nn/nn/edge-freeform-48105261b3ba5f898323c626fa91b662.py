...
def inverted_residual_block(in_channels, out_channels, stride=1, expansion=6):
    ...
    if stride != 1 or in_channels != out_channels:
        layers.append(nn.LazyConv2d(out_channels, kernel_size=1, stride=stride, padding=0, bias=False))
    return nn.Sequential(*layers)

class FractalBlock(nn.Module):

    def __init__(self, in_channels, out_channels, num_columns, expansion=6):
        super().__init__()
        self.layers = nn.ModuleList([inverted_residual_block(in_channels, out_channels, stride=1, expansion=expansion) for _ in range(num_columns)])
...
class FractalUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_columns, expansion=6):
        super().__init__()
        self.block = FractalBlock(in_channels, out_channels, num_columns, expansion)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', FractalUnit(curr_ch, out_ch, num_columns=2, expansion=6))
            curr_ch = out_ch
    ...
    def _norm4d(self, x: torch.Tensor) -> torch.Tensor:
        if x.dim() == 4:
            return x
        if x.dim() == 5:
            B, T, C, H, W = x.shape
