...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expansion_factor):
        ...
        if expansion_factor != 1:
            layers.append(DepthwiseSeparableConv(in_channels, hidden_dim))
        layers.extend([nn.LazyConv2d(hidden_dim, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True), DepthwiseSeparableConv(hidden_dim, out_channels, stride=stride), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True)])
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Efficient_Mobile_Fractal_SE'
        ...
        strides = [1, 2]
        for i, (out_ch, ef, s) in enumerate(zip(channels, expansion_factors, strides)):
            self.features.add_module(f'inverted_residual{i + 1}', InvertedResidual(curr_ch, out_ch, s, ef))
