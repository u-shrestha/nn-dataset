...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.reduce = nn.Conv2d(in_channels, 192, 1, bias=False)
        self.bn_reduce = nn.BatchNorm2d(192)
        self.depthwise = nn.Conv2d(192, 192, 3, padding=1, groups=192, bias=False)
        self.bn_depthwise = nn.BatchNorm2d(192)
        self.pointwise = nn.Conv2d(192, 96, 1, bias=False)
        self.bn_pointwise = nn.BatchNorm2d(96)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(96, 24, 1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(24, 96, 1, bias=True),
            nn.Sigmoid()
        )
