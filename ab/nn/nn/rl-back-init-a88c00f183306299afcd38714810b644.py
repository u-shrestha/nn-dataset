        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % (2 ** j) == 0:
                    ...
                    # [MODIFIED] Use the generated block instead of hardcoded Conv2d
                    generated_layer = drop_conv3x3_block(
                        in_channels=in_ch_ij,
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        N = 3
        num_columns = 2
        element_list = ('nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()', 'nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()', 'nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias)') # Just for logging
        backbone_a_name = "regnet_y_32gf"
        backbone_b_name = "resnet18"
