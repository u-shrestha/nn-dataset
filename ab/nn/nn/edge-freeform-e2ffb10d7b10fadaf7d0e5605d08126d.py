...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=6):
    ...
    if in_channels == out_channels and stride == 1:
        layers += [SqueezeExcitation(out_channels)]
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(inverted_residual_block(3, 32, stride=1, expand_ratio=1), inverted_residual_block(32, 64, stride=2, expand_ratio=6), inverted_residual_block(64, 128, stride=2, expand_ratio=6), depthwise_separable_conv(128, 160, stride=1), depthwise_separable_conv(160, 160, stride=1))
