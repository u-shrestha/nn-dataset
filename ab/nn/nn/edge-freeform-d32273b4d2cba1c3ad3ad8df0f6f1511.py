...
class FractalUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_blocks, stride=1, expansion_factor=6):
        ...
        layers = [InvertedResidual(in_channels, out_channels, stride=stride, expansion_factor=expansion_factor)]
        for _ in range(1, num_blocks):
            layers.append(InvertedResidual(out_channels, out_channels, stride=1, expansion_factor=expansion_factor))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(DepthwiseSeparableConv(in_shape[1], 32, stride=1), SqueezeExcitation(32), FractalUnit(32, 64, num_blocks=2, stride=2), FractalUnit(64, 128, num_blocks=2, stride=2))
