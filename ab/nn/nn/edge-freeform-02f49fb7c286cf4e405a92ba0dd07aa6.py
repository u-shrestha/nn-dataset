...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio):
        ...
        layers.append(depthwise_separable_block(hidden_dim, hidden_dim, stride=stride))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('efficientnet_b0', in_channels=3, truncate=10).to(device)
        ...
        for i, out_ch in enumerate(channels):
            ...
            curr_ch = out_ch
            if i < len(channels) - 1:
                self.features.add_module(f'dsconv{i + 1}', depthwise_separable_block(out_ch, out_ch))
