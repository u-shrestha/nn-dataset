...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expansion_factor=6, dropout_prob=0.0):
        ...
        self.se = SqueezeExcitation(hidden_dim, hidden_dim // 16)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('mobilenet_v3_small', in_channels=3, truncate=3).to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        channels = [24, 48, 96]
        for i, out_ch in enumerate(channels):
            if i == 0:
                self.features.add_module(f'dsconv{i + 1}', DepthwiseSeparableBlock(curr_ch, out_ch, stride=2, dropout_prob=dropout_prob))
            else:
                self.features.add_module(f'irb{i + 1}', InvertedResidualBlock(curr_ch, out_ch, stride=2, dropout_prob=dropout_prob))
