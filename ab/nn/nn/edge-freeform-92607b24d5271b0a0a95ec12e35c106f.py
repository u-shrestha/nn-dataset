...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.LazyConv2d(out_channels, kernel_size=3, stride=1, padding=1, bias=bias))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Optimized_Multi_Block_Efficient_Edge'
        self.use_amp = prm.get('use_amp', False)
        dropout_prob = float(prm['dropout'])
        self.backbone_a = TorchVision('shufflenet_v2_x1_0', in_channels=3, unwrap=True, truncate=10).to(device)
        self.backbone_b = TorchVision('mobilenet_v3_small', in_channels=3, unwrap=True, truncate=10).to(device)
        ...
        for i, out_ch in enumerate(channels):
            ...
            curr_ch = out_ch
            self.features.add_module(f'dsconv{i + 1}', depthwise_separable_block(out_ch, out_ch, dropout_prob=dropout_prob))
