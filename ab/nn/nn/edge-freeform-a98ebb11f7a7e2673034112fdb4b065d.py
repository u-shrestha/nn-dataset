...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Efficient_Ensemble'
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=4, stride=1, dropout_prob=dropout_prob))
            curr_ch = out_ch
