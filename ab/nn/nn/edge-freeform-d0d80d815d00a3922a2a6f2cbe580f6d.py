...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.BatchNorm2d(out_channels), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=bias), nn.ReLU(inplace=True))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_channels, kernel_size=1, bias=False), nn.BatchNorm2d(expanded_channels), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride), nn.Conv2d(expanded_channels, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(nn.Conv2d(3, 32, kernel_size=3, stride=2, padding=1, bias=False), nn.BatchNorm2d(32), nn.ReLU6(inplace=True), InvertedResidualBlock(32, 16, expansion_factor=1), InvertedResidualBlock(16, 24, expansion_factor=6, stride=2), InvertedResidualBlock(24, 24, expansion_factor=6), InvertedResidualBlock(24, 32, expansion_factor=6, stride=2), InvertedResidualBlock(32, 32, expansion_factor=6), InvertedResidualBlock(32, 64, expansion_factor=6, stride=2), InvertedResidualBlock(64, 64, expansion_factor=6), InvertedResidualBlock(64, 96, expansion_factor=6), InvertedResidualBlock(96, 96, expansion_factor=6), InvertedResidualBlock(96, 160, expansion_factor=6, stride=2), InvertedResidualBlock(160, 160, expansion_factor=6), InvertedResidualBlock(160, 160, expansion_factor=6), InvertedResidualBlock(160, 320, expansion_factor=6))
