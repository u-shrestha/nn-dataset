...
class SEIRBlock(nn.Module):

    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, reduction=4, dropout_prob=0.0):
        super(SEIRBlock, self).__init__()
        self.block = inverted_residual_block(in_channels, out_channels, stride, expand_ratio, dropout_prob)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'seir_unit{i + 1}', SEIRBlock(curr_ch, out_ch, dropout_prob=dropout_prob))
            curr_ch = out_ch
