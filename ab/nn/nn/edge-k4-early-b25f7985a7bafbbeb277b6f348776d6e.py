...
def supported_hyperparameters():
    return {'lr', 'dropout', 'momentum', 'use_amp'}

def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=bias), nn.BatchNorm2d(out_channels), nn.SiLU(inplace=True))
...
class FractalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for level_block in self.blocks:
            temp_outs = [blk(inp) for blk, inp in zip(level_block, outs)]
            merged = torch.stack(temp_outs, dim=0).mean(dim=0)
            outs = [merged for _ in range(len(level_block))]
        return outs[0]
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.use_amp = prm.pop('use_amp', False)
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        if x_bf.dim() == 2:
            x_bf = x_bf.unsqueeze(-1).unsqueeze(-1)
        if x_bf.shape[-1] < 14:
            x_bf = nn.functional.interpolate(x_bf, size=(14, 14), mode='nearest', align_corners=False)
        f_bf = adaptive_pool_flatten(self.features(x_bf))
        fused = torch.cat([f_a, f_bf], dim=1)
        if is_probing:
            return fused
