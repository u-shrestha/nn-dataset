...
class AttentionResidualBlock(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.attention = MDTA(dim, 2)
        self.residual = ResidualBlock(dim)

    def forward(self, x):
        return self.attention(x) + self.residual(x)
...
class Net(nn.Module):
    def __init__(self, in_shape=(1, 3, 256, 256), out_shape=None, prm={}, device="cuda"):
        ...
        self.enc0 = AttentionResidualBlock(dim)
        self.down0 = _down(dim)
        self.enc1 = AttentionResidualBlock(2 * dim)
        self.down1 = _down(2 * dim)
        self.latent = AttentionResidualBlock(4 * dim)
        self.up1 = _up(4 * dim)
        self.reduce1 = nn.Conv2d(4 * dim, 2 * dim, 1)
        self.dec1 = AttentionResidualBlock(2 * dim)
        self.up0 = _up(2 * dim)
        self.reduce0 = nn.Conv2d(2 * dim, dim, 1)
        self.dec0 = AttentionResidualBlock(dim)
