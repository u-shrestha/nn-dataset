...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.channel_squeeze = nn.Conv2d(in_channels * 2, in_channels // 4, 1, bias=False)
        self.channel_expand = nn.Conv2d(in_channels // 4, in_channels * 2, 1, bias=False)
    ...
    def forward(self, x):
        ...
        squeezed = self.channel_squeeze(x)
        expanded = self.channel_expand(squeezed)
        scale = self.sigmoid(expanded)
        x = x * scale
