...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.SiLU(inplace=True), nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=bias))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Triple_Backbones'
        self.use_amp = prm.get('use_amp', False)
        dropout_prob = float(prm.get('dropout', 0.1))
        self.backbone_a = TorchVision('squeezenet1_1', in_channels=3, truncate=1).to(device)
        self.backbone_b = TorchVision('efficientnet_b0', in_channels=3, truncate=3).to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        channels = [16, 32, 64]
        for i, out_ch in enumerate(channels):
            unit = FractalUnit(curr_ch, out_ch, num_columns=2, loc_drop_prob=0.15, dropout_prob=dropout_prob)
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        x_main = adaptive_pool_flatten(self.features(x))
        fused = torch.cat([x_main, x_a, x_b], dim=1)
        if is_probing:
            return fused
