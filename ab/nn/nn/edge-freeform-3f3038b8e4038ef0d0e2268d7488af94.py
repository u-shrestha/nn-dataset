...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(DepthwiseSeparableConv(in_channels, expanded_channels, stride=1), DepthwiseSeparableConv(expanded_channels, out_channels, stride=stride))
        self.dropout = nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(EfficientBlock(3, 64, stride=1), EfficientBlock(64, 128, stride=2), EfficientBlock(128, 128, stride=1)).to(device)
