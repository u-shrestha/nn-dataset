...
class DepthwiseSeparableBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, dropout_prob=0.0):
        super().__init__()
        self.depthwise = nn.Sequential(nn.LazyConv2d(in_channels, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True))
        self.pointwise = nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.SiLU(inplace=True))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(EfficientUnit(576, 128, 2, 6, 4, dropout_prob), EfficientUnit(128, 64, 2, 6, 4, dropout_prob))
