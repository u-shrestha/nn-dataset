...
class DepthwiseSeparableBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(self.bn2(self.dwconv(x)))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion=6, stride=1, dropout_prob=0.0):
        super().__init__()
        self.expansion = expansion
        expanded_channels = in_channels * expansion
        self.conv1 = nn.LazyConv2d(expanded_channels, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.relu(self.bn1(self.conv1(x)))
        x = self.relu(self.bn2(self.dwconv(x)))
        x = self.dropout(self.bn3(self.conv2(x)))
        if self.residual:
            x += identity
        return x
...
class SEBlock(nn.Module):
    ...
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        self.fc1 = nn.LazyConv2d(reduced_channels, kernel_size=1, stride=1, padding=0, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.LazyConv2d(in_channels, kernel_size=1, stride=1, padding=0, bias=False)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(DepthwiseSeparableBlock(512, 128, expansion=2), InvertedResidualBlock(128, 128, expansion=4, dropout_prob=dropout_prob), SEBlock(128), DepthwiseSeparableBlock(128, 256, stride=2, expansion=2), InvertedResidualBlock(256, 256, expansion=4, dropout_prob=dropout_prob), SEBlock(256))
