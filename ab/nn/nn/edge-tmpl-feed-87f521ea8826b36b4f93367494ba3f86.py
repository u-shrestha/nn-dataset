...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.pointwise_conv1 = nn.Conv2d(in_channels, in_channels // 4, 1, bias=False)
        self.bn2 = nn.BatchNorm2d(in_channels // 4)
        self.depthwise_conv2 = nn.Conv2d(in_channels // 4, in_channels // 4, 3, padding=1, groups=in_channels // 4, bias=False)
        self.bn3 = nn.BatchNorm2d(in_channels // 4)
        self.se_block = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channels // 4, in_channels // 16, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // 16, in_channels // 4, 1, bias=False),
            nn.Sigmoid()
        )
