...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Optimized_Edge_Architecture'
        ...
        self.features = nn.Sequential(inverted_residual_block(in_channels=1280, out_channels=512, stride=1, expand_ratio=4, dropout_prob=dropout_prob), depthwise_separable_block(in_channels=512, out_channels=256, stride=1, padding=1, bias=False, dropout_prob=dropout_prob), squeeze_excitation_block(in_channels=256, reduction_ratio=16))
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        features = adaptive_pool_flatten(features)
        if is_probing:
            return features
