...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1, dropout_prob=0.0):
        ...
        self.conv1 = drop_conv3x3_block(in_channels, expanded_channels, stride=1, padding=0, bias=False)
        self.dwconv2 = nn.Sequential(nn.Conv2d(expanded_channels, expanded_channels, kernel_size=3, stride=stride, padding=1, groups=expanded_channels, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True))
        self.conv3 = nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Efficient_Mobilenet_Shuffle'
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=2, stride=2, dropout_prob=dropout_prob))
            curr_ch = out_ch
