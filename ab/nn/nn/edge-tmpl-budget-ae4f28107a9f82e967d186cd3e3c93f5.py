...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.depthwise2 = nn.Conv2d(192, 192, kernel_size=3, padding=1, groups=192, bias=False)
        self.bn_depthwise2 = nn.BatchNorm2d(192)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(192, 48, kernel_size=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(48, 192, kernel_size=1, bias=True),
            nn.Sigmoid()
        )
        self.pointwise = nn.Conv2d(192, 96, kernel_size=1, bias=False)
        self.bn_pointwise = nn.BatchNorm2d(96)
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.drop = nn.Dropout(p)
        self.fc = nn.LazyLinear(num_classes)

    def forward(self, x):
        x = self.relu(self.bn_reduce(self.reduce_dim(x)))
        x = self.relu(self.bn_depthwise1(self.depthwise1(x)))
        x = self.relu(self.bn_depthwise2(self.depthwise2(x)))
        se_weights = self.se(x)
        x = x * se_weights
        x = self.relu(self.bn_pointwise(self.pointwise(x)))
