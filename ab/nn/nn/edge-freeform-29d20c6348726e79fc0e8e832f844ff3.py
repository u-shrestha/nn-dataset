...
def inverted_residual_block(in_channels, out_channels, stride=1, expansion_factor=6, dropout_prob=0.0):
    ...
    if in_channels != hidden_dim:
        layers.append(nn.Conv2d(in_channels, hidden_dim, kernel_size=1, stride=1, bias=False))
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            if i == 0:
                ...
                self.features.add_module(f'pwconv{i + 1}', nn.Conv2d(out_ch, out_ch, kernel_size=1, bias=False))
                self.features.add_module(f'seblock{i + 1}', se_block(out_ch))
            else:
                self.features.add_module(f'irb{i + 1}', inverted_residual_block(curr_ch, out_ch, stride=2, expansion_factor=6, dropout_prob=dropout_prob))
                self.features.add_module(f'seblock{i + 1}', se_block(out_ch))
            curr_ch = out_ch
