...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=4):
        ...
        self.conv = nn.Sequential(nn.LazyConv2d(self.hidden_dim, 1, 1, 0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.Conv2d(self.hidden_dim, self.hidden_dim, 3, stride, 1, groups=self.hidden_dim, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, 1, 1, 0, bias=False), nn.LazyBatchNorm2d())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(InvertedResidualBlock(576, 128, stride=2), DepthwiseSeparableConv(128, 256), SqueezeExcitation(256, 64), nn.MaxPool2d(kernel_size=2, stride=2))
