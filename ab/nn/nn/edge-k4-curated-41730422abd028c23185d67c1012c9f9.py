...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('efficientnet_b0', in_channels=3, weights='DEFAULT').to(device)
        self.backbone_b = TorchVision('shufflenet_v2_x0_5', in_channels=3, weights='DEFAULT').to(device)
        self.features = nn.Sequential()
        curr_ch = 3
        self.features.add_module('unit1', FractalUnit(curr_ch, 64, num_columns=2, loc_drop_prob=0.15, dropout_prob=dropout_prob))
        curr_ch = 64
