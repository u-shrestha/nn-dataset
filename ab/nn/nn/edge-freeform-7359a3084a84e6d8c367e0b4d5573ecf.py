...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm2d(16), nn.ReLU6(inplace=True), InvertedResidualBlock(16, 32, stride=2, expansion=6, reduction_ratio=16, dropout_prob=dropout_prob), InvertedResidualBlock(32, 64, stride=2, expansion=6, reduction_ratio=16, dropout_prob=dropout_prob))
        self.backbone_a = TorchVision('shufflenet_v2_x1_0', in_channels=3, truncate=3).to(device)
        self.backbone_b = TorchVision('mobilenet_v3_small', in_channels=3, truncate=10).to(device)
        self.to(self.device)
