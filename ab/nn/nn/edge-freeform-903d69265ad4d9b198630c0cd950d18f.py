...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        super().__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(reduced_dim, 1), nn.ReLU(inplace=True), nn.LazyConv2d(in_channels, 1), nn.Sigmoid())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(nn.Sequential(inverted_residual_block(in_ch_ij, out_channels, dropout_prob=dropout_prob), SqueezeExcitation(out_channels, out_channels // 4)))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_large', in_channels=3, truncate=6).to(device)
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        x_bf = adaptive_pool_flatten(x_b)
        mid_img = torch.nn.functional.interpolate(x_b, size=(14, 14), mode='nearest')
        f_mid = adaptive_pool_flatten(self.features(mid_img))
        fused = torch.cat([f_a, f_mid, x_bf], dim=1)
        if is_probing:
            return fused
