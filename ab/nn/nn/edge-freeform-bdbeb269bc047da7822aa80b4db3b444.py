...
def optimized_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expand_ratio=6, stride=1, dropout_prob=0.0):
        super().__init__()
        self.stride = stride
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(optimized_conv3x3_block(in_channels, hidden_dim, bias=False))
        layers.extend([nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False), nn.LazyBatchNorm2d(), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()])
