...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1, dropout_prob=0.0):
        ...
        self.conv = nn.Sequential(nn.LazyConv2d(expanded_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride, dropout_prob=dropout_prob), SEBlock(expanded_channels), nn.LazyConv2d(out_channels, kernel_size=1, stride=1, padding=0, bias=False), nn.LazyBatchNorm2d())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Combined_Efficient_MobileNet'
        ...
        self.features = nn.Sequential(InvertedResidualBlock(1280, 256, expansion_factor=4, stride=1, dropout_prob=dropout_prob), InvertedResidualBlock(256, 128, expansion_factor=4, stride=1, dropout_prob=dropout_prob))
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        fused = torch.cat([f_a, f_b], dim=1).unsqueeze(-1).unsqueeze(-1)
        fused = torch.nn.functional.interpolate(fused, size=(14, 14), mode='nearest')
        fused = adaptive_pool_flatten(self.features(fused))
        if is_probing:
            return fused
