...
def enhanced_drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.LazyConv2d(out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.LazyBatchNorm2d(), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.SiLU(inplace=True), nn.LazyConv2d(out_channels, kernel_size=3, stride=1, padding=1, bias=bias), nn.SiLU(inplace=True))

class DepthwiseSeparableBlock(nn.Module):

    def __init__(self, in_channels, out_channels, stride=1, expansion_factor=2, dropout_prob=0.0):
        super().__init__()
        mid_channels = in_channels * expansion_factor
        self.depthwise = nn.Conv2d(in_channels, mid_channels, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False)
        self.bn1 = nn.LazyBatchNorm2d()
        self.pointwise = nn.LazyConv2d(out_channels, kernel_size=1, stride=1, bias=False)
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expansion_factor, dropout_prob=0.0):
        ...
        self.stride = stride
        mid_channels = in_channels * expansion_factor
        self.bottleneck = nn.LazyConv2d(mid_channels, kernel_size=1, stride=1, bias=False)
        self.bn1 = nn.LazyBatchNorm2d()
    ...
    def forward(self, x):
        identity = x
        x = self.activation(self.bn1(self.bottleneck(x)))
        x = self.activation(self.bn2(self.depthwise(x)))
        x = self.dropout(x)
...
class SqueezeExcitationBlock(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=4):
        ...
        self.fc = nn.Sequential(nn.LazyConv2d(channels // reduction_ratio, kernel_size=1), nn.SiLU(inplace=True), nn.LazyConv2d(channels, kernel_size=1), nn.Sigmoid())
        ...
        for i in range(depth):
            ...
            for j in range(self.num_columns):
                if (i + 1) % 2 ** j == 0:
                    ...
                    level.append(enhanced_drop_conv3x3_block(in_ch_ij, out_channels, dropout_prob=dropout_prob))
            blocks.append(level)
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=3, truncate=8).to(device)
        ...
        for i, out_ch in enumerate(channels):
            ...
            curr_ch = out_ch
        self.backbone_b = TorchVision('shufflenet_v2_x1_0', in_channels=3, truncate=7).to(device)
