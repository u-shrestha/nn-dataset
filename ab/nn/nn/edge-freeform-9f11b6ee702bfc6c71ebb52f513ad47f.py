...
def se_block(in_channels, reduction_ratio=16):
    return nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.LazyConv2d(in_channels // reduction_ratio, 1, bias=False), nn.ReLU(inplace=True), nn.LazyConv2d(in_channels, 1, bias=False), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        self.pattern = 'Hybrid_Architecture'
        self.use_amp = prm.get('use_amp', False)
        dropout_prob = float(prm.get('dropout', 0.2))
        self.backbone_a = TorchVision('mobilenet_v3_small', in_channels=3, unwrap=True, truncate=2).to(device)
        self.backbone_b = TorchVision('shufflenet_v2_x1_0', in_channels=3, unwrap=True, truncate=2).to(device)
