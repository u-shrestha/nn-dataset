...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, expansion_factor, out_channels, stride, use_se=False, se_ratio=0.25):
        super().__init__()
        self.stride = stride
        ...
        layers.append(DepthwiseSeparableConv(hidden_dim, hidden_dim, stride=stride))
        if use_se:
            layers.append(SEBlock(hidden_dim, int(hidden_dim * se_ratio)))
...
class SEBlock(nn.Module):
    ...
    def __init__(self, in_channels, reduced_dim):
        ...
        self.fc = nn.Sequential(nn.Linear(in_channels, reduced_dim), nn.ReLU(inplace=True), nn.Linear(reduced_dim, in_channels), nn.Sigmoid())
...
class FractalUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_blocks, stride=1, expansion_factor=6, se_ratio=0.25):
        super(FractalUnit, self).__init__()
        layers = [InvertedResidualBlock(in_channels, expansion_factor, out_channels, stride, use_se=True, se_ratio=se_ratio)]
        for _ in range(1, num_blocks):
            layers.append(InvertedResidualBlock(out_channels, expansion_factor, out_channels, 1, use_se=True, se_ratio=se_ratio))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(FractalUnit(3, 32, num_blocks=2, stride=1, expansion_factor=6, se_ratio=0.25), FractalUnit(32, 64, num_blocks=2, stride=2, expansion_factor=6, se_ratio=0.25), FractalUnit(64, 128, num_blocks=2, stride=2, expansion_factor=6, se_ratio=0.25))
    ...
    def infer_dimensions_dynamically(self, in_shape, num_classes):
        ...
        with torch.no_grad():
            ...
            fused = torch.cat([x_f, f_a, f_b], dim=1)
            self.feat_dim = fused.shape[1]
        self.train()
