...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.depthwise1 = nn.Conv2d(96, 96, kernel_size=3, padding=1, groups=96, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.relu(self.bn_dw1(self.depthwise1(x)))
        se_weight1 = self.se1(x)
        x = x * se_weight1
        x = self.relu(self.bn_dw2(self.depthwise2(x)))
        se_weight2 = self.se2(x)
        x = x * se_weight2
