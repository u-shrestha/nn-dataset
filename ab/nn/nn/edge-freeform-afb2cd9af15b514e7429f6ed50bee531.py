...
def depthwise_separable_conv(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias), nn.LazyBatchNorm2d(), nn.ReLU6(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, bias=bias), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())

class SqueezeExcitation(nn.Module):

    def __init__(self, channels, reduction_ratio=16):
        super(SqueezeExcitation, self).__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)), nn.LazyConv2d(channels // reduction_ratio, kernel_size=1), nn.ReLU(inplace=True), nn.LazyConv2d(channels, kernel_size=1), nn.Sigmoid())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(depthwise_separable_conv(3, 32, stride=1, padding=1, bias=False, dropout_prob=self.dropout), inverted_residual_block(32, 64, stride=2, expand_ratio=6, dropout_prob=self.dropout), inverted_residual_block(64, 128, stride=2, expand_ratio=6, dropout_prob=self.dropout), SqueezeExcitation(128))
