...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride):
        super().__init__()
        self.conv = nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False), nn.LazyBatchNorm2d(), nn.ReLU(inplace=True), nn.LazyConv2d(out_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU(inplace=True))
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride, dropout_prob):
        ...
        self.conv = nn.Sequential(nn.LazyConv2d(expanded_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.ReLU(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride), nn.LazyConv2d(out_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'inverted_residual_{i + 1}', InvertedResidualBlock(curr_ch, out_ch, expansion_factor=2, stride=2, dropout_prob=dropout_prob))
            curr_ch = out_ch
