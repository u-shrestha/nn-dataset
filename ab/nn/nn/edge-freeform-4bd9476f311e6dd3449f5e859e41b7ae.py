...
def inverted_residual_block(in_channels, out_channels, stride=1, expand_ratio=1, dropout_prob=0.0):
    ...
    return nn.Sequential(nn.Conv2d(in_channels, expanded_dim, kernel_size=1, bias=False), nn.BatchNorm2d(expanded_dim), nn.ReLU6(inplace=True), nn.Conv2d(expanded_dim, expanded_dim, kernel_size=3, stride=stride, padding=1, groups=expanded_dim, bias=False), nn.BatchNorm2d(expanded_dim), nn.ReLU6(inplace=True), nn.Conv2d(expanded_dim, out_channels, kernel_size=1, bias=False), nn.BatchNorm2d(out_channels), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity())
