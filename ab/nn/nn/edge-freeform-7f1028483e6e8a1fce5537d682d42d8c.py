...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
        super().__init__()
        self.depthwise = nn.LazyConv2d(in_channels, kernel_size=3, stride=stride, padding=padding, groups=in_channels, bias=bias)
        self.pointwise = nn.LazyConv2d(out_channels, kernel_size=1, bias=bias)
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=4, stride=1, dropout_prob=0.0):
        super().__init__()
        self.expansion = nn.Sequential(nn.LazyConv2d(expansion_factor * in_channels, kernel_size=1, bias=False), nn.LazyBatchNorm2d(), nn.SiLU(inplace=True))
        self.dw_separable = DepthwiseSeparableConv(expansion_factor * in_channels, out_channels, stride=stride, dropout_prob=dropout_prob)
...
class SEBlock(nn.Module):
    ...
    def __init__(self, in_channels, reduction_ratio=4):
        ...
        self.excitation = nn.Sequential(nn.LazyLinear(reduced_dim), nn.ReLU(inplace=True), nn.LazyLinear(in_channels), nn.Sigmoid())
...
class HybridBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_blocks=2, expansion_factor=4, stride=1, dropout_prob=0.0, se_reduction=4):
        super().__init__()
        blocks = [InvertedResidualBlock(in_channels, out_channels, expansion_factor, stride, dropout_prob)]
        for _ in range(1, num_blocks):
            blocks.append(InvertedResidualBlock(out_channels, out_channels, expansion_factor, 1, dropout_prob))
            blocks.append(SEBlock(out_channels, se_reduction))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('squeezenet1_1', in_channels=3, truncate=8).to(device)
        self.features = nn.Sequential(HybridBlock(576, 64, num_blocks=2, expansion_factor=4, stride=2, dropout_prob=dropout_prob), HybridBlock(64, 128, num_blocks=2, expansion_factor=4, stride=2, dropout_prob=dropout_prob))
