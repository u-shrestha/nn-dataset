...
def se_ir_block(in_channels, out_channels, stride=1, expand_ratio=6, reduction=4, dropout_prob=0.0):
    return nn.Sequential(inverted_residual_block(in_channels, out_channels, stride, expand_ratio, dropout_prob), se_block(out_channels, reduction))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(se_ir_block(512, 128, dropout_prob=dropout_prob), se_ir_block(128, 64, dropout_prob=dropout_prob))
    ...
    def forward(self, x: torch.Tensor, is_probing: bool=False) -> torch.Tensor:
        ...
        fused_img = fused.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, 14, 14)
        output_features = self.features(fused_img)
        output_flat = adaptive_pool_flatten(output_features)
        if is_probing:
            return output_flat
        return self.classifier(output_flat)
