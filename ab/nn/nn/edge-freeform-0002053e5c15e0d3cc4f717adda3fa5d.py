...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor=4, stride=1, use_se=False):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, expanded_channels, 1, 1, 0, bias=False), nn.BatchNorm2d(expanded_channels), nn.ReLU6(inplace=True), DepthwiseSeparableConv(expanded_channels, expanded_channels, stride=stride), SEBlock(expanded_channels) if use_se else nn.Identity(), nn.Conv2d(expanded_channels, out_channels, 1, 1, 0, bias=False), nn.BatchNorm2d(out_channels))
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            self.features.add_module(f'unit{i + 1}', InvertedResidual(curr_ch, out_ch, expansion_factor=4, use_se=True))
            curr_ch = out_ch
