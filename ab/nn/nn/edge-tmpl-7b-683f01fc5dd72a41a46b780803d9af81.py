...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        super().__init__()
        p = float(prm['dropout'])
        self.projector = nn.Conv2d(in_channels, 320, kernel_size=1, bias=False)
        self.bn = nn.BatchNorm2d(320)
        self.relu = nn.ReLU(inplace=True)
        self.depthwise_conv = nn.Conv2d(320, 320, 3, padding=1, groups=320, bias=False)
        self.pointwise_conv = nn.Conv2d(320, 128, kernel_size=1, bias=False)
        self.se_layer = SELayer(128, reduction_ratio=16)
