...
class TorchVision(nn.Module):
    ...
    def __init__(self, model: str, weights: str='DEFAULT', unwrap: bool=True, truncate: int=1, in_channels: int=3):
        super().__init__()
        self.adapter = nn.LazyConv2d(3, kernel_size=1, bias=False) if in_channels != 3 else nn.Identity()
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        for i, out_ch in enumerate(channels):
            if i == 0:
                self.features.add_module(f'conv{i + 1}', nn.LazyConv2d(out_ch, kernel_size=3, stride=2, padding=1, bias=False))
                self.features.add_module(f'bn{i + 1}', nn.LazyBatchNorm2d())
                self.features.add_module(f'relu{i + 1}', nn.ReLU6(inplace=True))
            else:
                self.features.add_module(f'se_irb{i + 1}', SEInvertedResidualBlock(curr_ch, out_ch, stride=2 if i == 1 else 1, dropout_prob=dropout_prob))
            curr_ch = out_ch
