...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes, prm):
        ...
        self.pointwise = nn.Conv2d(in_channels, in_channels * 2, 1, bias=False)
        self.bn_pointwise = nn.BatchNorm2d(in_channels * 2)
        self.relu = nn.ReLU(inplace=True)
        self.squeeze = nn.Conv2d(in_channels * 2, in_channels // 4, 1, bias=False)
        self.excite = nn.Conv2d(in_channels // 4, in_channels * 2, 1, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        se_weights = self.sigmoid(self.excite(self.relu(self.squeeze(self.avg_pool(x)))))
        x = x * se_weights
