...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.backbone_b = TorchVision('efficientnet_b0', in_channels=3, truncate=4).to(device)
        self.features = nn.Sequential(ds_conv3x3_block(3, 32, stride=1, padding=1, bias=False, dropout_prob=dropout_prob), InvertedResidualBlock(32, 64, stride=2, expand_ratio=2, dropout_prob=dropout_prob), InvertedResidualBlock(64, 128, stride=2, expand_ratio=2, dropout_prob=dropout_prob), nn.AdaptiveAvgPool2d((1, 1)))
        self.to(self.device)
