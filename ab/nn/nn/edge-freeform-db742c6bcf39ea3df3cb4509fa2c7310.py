...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias), nn.BatchNorm2d(out_channels), nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity(), nn.ReLU6(inplace=True))
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, in_channels * expand_ratio, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(in_channels * expand_ratio), nn.ReLU6(inplace=True), DepthwiseSeparableConv(in_channels * expand_ratio, out_channels, stride))
